// 显式 import attribute：Node ESM（含 node --test 直跑 .ts）要求 JSON 模块声明 type。
import modelProfilesRaw from './modelProfiles.json' with { type: 'json' };
...
const inferForcedNameCapabilities = (modelId: string): ModelCapability[] => {
    ...
    if (normalized.includes('embedding') || /\bembed\b/.test(normalized)) return ['embedding'];
