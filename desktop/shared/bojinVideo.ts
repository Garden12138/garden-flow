...
export type BojinOfficialVideoMode =
  | 'text-to-video'
  | 'reference-guided'
  | 'first-last-frame'
  | 'continuation';
...
export function getBojinOfficialVideoModel(mode: BojinOfficialVideoMode): string {
  return REDBOX_OFFICIAL_VIDEO_MODELS[mode];
}

export function isBojinOfficialVideoModel(model: string): boolean {
