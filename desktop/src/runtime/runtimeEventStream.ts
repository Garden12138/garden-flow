...
function parseRuntimeEnvelopeRecord(record: UnknownRecord, eventType: RuntimeEventType): RuntimeUnifiedEvent {
  ...
  return {
    eventId: toOptionalText(record.eventId || record.id),
    eventType,
    sessionId: toText(record.sessionId) || null,
    runId: toOptionalText(record.runId || payload.runId) || null,
    userMessageId: toOptionalText(record.userMessageId || payload.userMessageId) || null,
    assistantMessageId: toOptionalText(record.assistantMessageId || payload.assistantMessageId) || null,
    sequence: toOptionalNumber(record.sequence ?? payload.sequence) ?? null,
    phase: toOptionalText(record.phase) || null,
    status: toOptionalText(record.status) || null,
...
function dispatchRuntimeEnvelope(handlers: RuntimeEventStreamHandlers, envelope: RuntimeUnifiedEvent): void {
  ...
  const runtimeMeta: RuntimeEnvelopeMeta = {
    eventId: toOptionalText(envelope.eventId),
    runId: toOptionalText(envelope.runId),
    userMessageId: toOptionalText(envelope.userMessageId),
    assistantMessageId: toOptionalText(envelope.assistantMessageId),
    sequence: toOptionalNumber(envelope.sequence),
