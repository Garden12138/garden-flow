...
export interface AuthoringTaskHints {
    ...
    generationToolConstraints?: {
        ...
        videoGenerate?: {
            operation?: 'generate' | 'generate-note';
...
const PLATFORM_SAVE_RULE: Record<AuthoringPlatform, string> = {
    xiaohongshu: '使用 `app_cli(command="manuscripts note-save --note-type image|video", payload={ document: {...} })` 创建结构化 `.redpost` 或 `.redvideo` 工程。必须保存标题候选、最终标题、正文、标签、封面文案、逐页方案/口播分镜和媒体槽位；禁止用普通 Write 或 manuscripts write 覆盖结构化工程。',
...
export function buildGardenFlowAuthoringMessage(input: BuildAuthoringMessageInput) {
    ...
    return {
        ...
        taskHints: {
            ...
            activeSkills: input.platform === 'xiaohongshu' ? ['writing-style', 'xhs-title'] : ['writing-style'],
