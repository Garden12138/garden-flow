import type { ChatComposerModeGroup, ChatShortcut, ChatShortcutContext } from '../Chat';
import { APP_BRAND } from '../../config/brand';
import type { ScheduleDraft, ScheduleTemplate } from './types';
import {
    GARDENFLOW_ATTACHMENT_ACTIONS_BY_SCENE,
    resolveGardenFlowAttachmentShortcutScene,
    type GardenFlowAttachmentShortcutScene,
    type GardenFlowComposerShortcutInput,
} from '../chat/attachment-actions/gardenflowAttachmentActions';

export type { GardenFlowComposerShortcutInput } from '../chat/attachment-actions/gardenflowAttachmentActions';

export const GARDENFLOW_CONTEXT_ID = 'gardenflow-singleton';
export const GARDENFLOW_CONTEXT_TYPE = 'gardenflow';
export const GARDENFLOW_DISPLAY_NAME = APP_BRAND.aiDisplayName;
export const GARDENFLOW_CONTEXT = [
    `${GARDENFLOW_DISPLAY_NAME} 是一个面向自媒体内容生产与运营的 AI 工作台。`,
    '工作目标：基于用户目标推进选题、内容、配图、发布与复盘，并给出可执行的工作流建议。',
    '默认输出结构：目标拆解、内容策略、执行步骤、风险提示。',
    '当产出、保存或更新可交付文件时，必须先通过工具成功写入或生成文件，再用 Markdown 链接报告路径；优先使用 workspace://、media://、manuscripts://、knowledge://、cover:// 或 gardenflow:// 这类 app 内虚拟路径。未成功写入文件时，不得说“已保存”。',
    '不要建议用户切换到视频剪辑页面或音频剪辑页面；当前产品没有这类可导航页面。视频分析、字幕提取、切片或音频整理都应在当前 AI 对话里通过可用工具完成或说明缺失能力。',
].join('\n');

export type GardenFlowComposerShortcutScene =
    | GardenFlowAttachmentShortcutScene
    | 'empty_new_chat'
    | 'member_mention'
    | 'knowledge_context';

export const GARDENFLOW_DEFAULT_COMPOSER_SHORTCUT_INPUTS: GardenFlowComposerShortcutInput[] = [
...
export const GARDENFLOW_XHS_COMPOSER_MODE_GROUP: ChatComposerModeGroup = {
    label: '小红书卡片',
    options: [
        {
            id: 'xiaohongshu-image-note',
            label: '图文笔记',
            description: '5 个标题、正文、标签、封面文案和 6 页图文方案',
            placeholder: '输入一个选题或想法，例如：租房卧室的低成本改造',
            icon: 'image',
            taskHints: {
        ...
        {
            id: 'xiaohongshu-video-note',
            label: '视频笔记',
            description: '标题、正文、标签、封面文案、口播稿、分镜和字幕',
            placeholder: '输入一个视频想法，例如：讲清新手如何挑咖啡豆',
            icon: 'video',
            parameters: [
                {
                    id: 'video-duration',
                    label: '时长',
                    taskHintKey: 'xhsVideoDurationSeconds',
                    valueType: 'number',
                    defaultValue: '45',
                    options: [
                        { value: '15', label: '15 秒' },
                ...
                },
                {
                    ...
                    label: '比例',
                    taskHintKey: 'xhsVideoAspectRatio',
                    defaultValue: '9:16',
                    options: [
...
export const GARDENFLOW_COMPOSER_SHORTCUT_INPUTS_BY_SCENE: Record<GardenFlowComposerShortcutScene, GardenFlowComposerShortcutInput[]> = {
    ...GARDENFLOW_ATTACHMENT_ACTIONS_BY_SCENE,
    empty_new_chat: GARDENFLOW_DEFAULT_COMPOSER_SHORTCUT_INPUTS,
    member_mention: [
...
export function resolveGardenFlowComposerShortcutScene(context: ChatShortcutContext): GardenFlowComposerShortcutScene {
    if (context.attachment) {
        return resolveGardenFlowAttachmentShortcutScene(context);
    }
...
export function createGardenFlowComposerShortcuts(
    inputs: GardenFlowComposerShortcutInput[] = GARDENFLOW_DEFAULT_COMPOSER_SHORTCUT_INPUTS,
): ChatShortcut[] {
    return inputs
        .map((item) => ({
...
export function createGardenFlowComposerShortcutsForContext(context: ChatShortcutContext): ChatShortcut[] {
    const scene = resolveGardenFlowComposerShortcutScene(context);
    return createGardenFlowComposerShortcuts(GARDENFLOW_COMPOSER_SHORTCUT_INPUTS_BY_SCENE[scene]);
}

export const GARDENFLOW_SHORTCUTS = createGardenFlowComposerShortcuts();

export const GARDENFLOW_WELCOME_SHORTCUTS = GARDENFLOW_SHORTCUTS;
...
export const GARDENFLOW_SIDEBAR_MIN_WIDTH = 300;
export const GARDENFLOW_SIDEBAR_MAX_WIDTH = 560;
export const GARDENFLOW_SIDEBAR_DEFAULT_WIDTH = 380;
export const GARDENFLOW_WELCOME_ICON_SRC = APP_BRAND.logoSrc;
