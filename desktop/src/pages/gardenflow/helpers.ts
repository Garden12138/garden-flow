...
import { GARDENFLOW_CONTEXT, GARDENFLOW_CONTEXT_ID, GARDENFLOW_DISPLAY_NAME, WEEKDAY_OPTIONS } from './config';
...
export function buildGardenFlowContextId(activeSpaceId: string): string {
    return `${GARDENFLOW_CONTEXT_ID}:${activeSpaceId}`;
}

export function buildGardenFlowSessionTitle(spaceName: string): string {
    return `${GARDENFLOW_DISPLAY_NAME} · ${spaceName}`;
}

export function buildGardenFlowInitialContext(spaceName: string, activeSpaceId: string): string {
    return `${GARDENFLOW_CONTEXT}\n当前空间: ${spaceName} (${activeSpaceId})`;
}

export function buildGardenFlowRuntimeMetadata(activeSpaceId: string, spaceName?: string): Record<string, unknown> {
    const spaceId = String(activeSpaceId || 'default').trim() || 'default';
    const contextId = buildGardenFlowContextId(spaceId);
    return {
        surface: 'gardenflow',
        runtimeSurface: 'gardenflow',
