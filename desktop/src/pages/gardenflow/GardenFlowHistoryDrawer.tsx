...
export type GardenFlowHistorySurface = 'gardenflow' | 'advisor' | 'room' | 'external';

export interface GardenFlowHistoryListItem extends ContextChatSessionListItem {
    surface: GardenFlowHistorySurface;
    speakerLabel: string;
    advisorId?: string;
    roomId?: string;
}

export type GardenFlowHistorySessionActivity = 'running' | 'unread-complete' | 'failed';

type GardenFlowSidebarTab = 'chat' | 'manuscripts' | 'archived';

type GardenFlowManuscriptNode = {
    name: string;
    path: string;
    isDirectory: boolean;
    children?: GardenFlowManuscriptNode[];
    title?: string;
    draftType?: string;
    updatedAt?: number;
};
...
function isAutomationHistorySession(session: GardenFlowHistoryListItem): boolean {
    if (session.surface !== 'gardenflow') return false;
...
interface GardenFlowHistorySidebarSectionProps {
    historyLoading: boolean;
    sessionList: GardenFlowHistoryListItem[];
    activeSessionId: string | null;
    teamRooms?: GardenFlowTeamRoom[];
    activeRoomId?: string | null;
    activeSurface?: 'gardenflow' | 'advisor' | 'room';
    sessionActivityById?: Record<string, GardenFlowHistorySessionActivity>;
    onCreateRoom?: () => void;
    onSwitchRoom?: (roomId: string) => void;
    onDeleteRoom?: (room: GardenFlowTeamRoom) => void | Promise<void>;
    onSwitchSession: (session: GardenFlowHistoryListItem) => void;
    onSetSessionUnread?: (sessionId: string, unread: boolean) => void;
    onArchiveSession?: (session: GardenFlowHistoryListItem) => void | Promise<void>;
    archivedSessionList?: GardenFlowHistoryListItem[];
    archivedSessionsLoading?: boolean;
    onLoadArchivedSessions?: () => void | Promise<void>;
    onRestoreArchivedSession?: (session: GardenFlowHistoryListItem) => void | Promise<void>;
    onDeleteArchivedSession?: (session: GardenFlowHistoryListItem) => void | Promise<void>;
    onRenameSession?: (session: GardenFlowHistoryListItem, title: string) => void | Promise<void>;
    onOpenManuscript?: (filePath: string) => void;
    activeManuscriptPath?: string | null;
}

const PINNED_ROOM_IDS_STORAGE_KEY = 'gardenflow:gardenflow:pinned-room-ids:v1';
const PINNED_SESSION_IDS_STORAGE_KEY = 'gardenflow:gardenflow:pinned-session-ids:v1';
...
type ManuscriptDialogState =
    ...
    | { mode: 'rename'; node: GardenFlowManuscriptNode };

type ManuscriptContextMenuState = {
    x: number;
    y: number;
    parentPath: string;
    node?: GardenFlowManuscriptNode;
};
...
function manuscriptNodeLabel(node: GardenFlowManuscriptNode): string {
...
function sortManuscriptNodes(nodes: GardenFlowManuscriptNode[]): GardenFlowManuscriptNode[] {
...
export function GardenFlowHistorySidebarSection({
    historyLoading,
    sessionList,
    activeSessionId,
    teamRooms = [],
...
}: GardenFlowHistorySidebarSectionProps) {
    const [activeTab, setActiveTab] = useState<GardenFlowSidebarTab>('chat');
    const [renameTarget, setRenameTarget] = useState<GardenFlowHistoryListItem | null>(null);
    const [renameTitle, setRenameTitle] = useState('');
    const [renameError, setRenameError] = useState('');
    const [isRenaming, setIsRenaming] = useState(false);
    const [archivedActionSessionId, setArchivedActionSessionId] = useState('');
    const [archivedActionKind, setArchivedActionKind] = useState<'restore' | 'delete' | ''>('');
    ...
    const [manuscriptTree, setManuscriptTree] = useState<GardenFlowManuscriptNode[]>([]);
    ...
    }, [activeTab, onLoadArchivedSessions]);
    ...
    const loadManuscripts = useCallback(async () => {
        ...
        setManuscriptsError('');
        try {
            const tree = await window.ipcRenderer.manuscripts.list() as GardenFlowManuscriptNode[];
            ...
            if (requestId !== manuscriptRequestIdRef.current) return;
            console.error('Failed to load GardenFlow manuscript tree:', error);
    ...
    const deleteManuscriptNode = async (node: GardenFlowManuscriptNode) => {
        setMenuTarget(null);
        setManuscriptContextMenu(null);
    ...
    const handleManuscriptDragStart = (event: DragEvent<HTMLElement>, node: GardenFlowManuscriptNode) => {
        event.stopPropagation();
        setMenuTarget(null);
        setManuscriptContextMenu(null);
        ...
        event.dataTransfer.setData('application/x-gardenflow-manuscript-path', node.path);
    };

    const handleManuscriptDragOver = (event: DragEvent<HTMLElement>, targetDir: string) => {
        const sourcePath = draggedManuscriptPath || event.dataTransfer.getData('application/x-gardenflow-manuscript-path') || event.dataTransfer.getData('text/plain');
    ...
    const renderManuscriptNode = (node: GardenFlowManuscriptNode, depth = 0): ReactNode => {
        const label = manuscriptNodeLabel(node);
    ...
    const sessionWorkingDirectory = (session: GardenFlowHistoryListItem): string => {
    ...
    const sessionAcpLabel = (session: GardenFlowHistoryListItem): string => {
        const metadata = session.metadata && typeof session.metadata === 'object'
            ? session.metadata as Record<string, unknown>
            : null;
    ...
    const openSessionContextMenu = (
        event: MouseEvent<HTMLElement>,
        session: GardenFlowHistoryListItem,
    ...
    const openRenameDialog = (session: GardenFlowHistoryListItem) => {
    ...
    const restoreArchivedSession = async (session: GardenFlowHistoryListItem) => {
        if (!onRestoreArchivedSession || archivedActionSessionId) return;
        setArchivedActionSessionId(session.id);
        setArchivedActionKind('restore');
        try {
            await onRestoreArchivedSession(session);
        } catch (error) {
            void appAlert(error instanceof Error ? error.message : '恢复对话失败');
        } finally {
            setArchivedActionSessionId('');
            setArchivedActionKind('');
        }
    };

    const archiveSession = async (session: GardenFlowHistoryListItem) => {
        if (!onArchiveSession || archivingSessionId) return;
        const title = displaySessionTitle(session.chatSession?.title?.trim() || '未命名会话', session.surface);
        setArchivingSessionId(session.id);
        try {
            const confirmed = await appConfirm(
                `归档对话“${title}”？归档后可在“已归档”中恢复。`,
                { title: '归档对话', confirmLabel: '归档' }
            );
            if (!confirmed) return;
            await onArchiveSession(session);
        } catch (error) {
            void appAlert(error instanceof Error ? error.message : '归档对话失败');
        } finally {
            setArchivingSessionId('');
        }
    };

    const deleteArchivedSession = async (session: GardenFlowHistoryListItem) => {
        if (!onDeleteArchivedSession || archivedActionSessionId) return;
        const title = displaySessionTitle(session.chatSession?.title?.trim() || '未命名会话', session.surface);
        const confirmed = await appConfirm(
            `永久删除对话“${title}”及其全部消息？此操作无法撤销。`,
            { title: '永久删除对话', confirmLabel: '删除', tone: 'danger' }
        );
        if (!confirmed) return;
        setArchivedActionSessionId(session.id);
        setArchivedActionKind('delete');
        try {
            await onDeleteArchivedSession(session);
        } catch (error) {
            void appAlert(error instanceof Error ? error.message : '删除对话失败');
        } finally {
            setArchivedActionSessionId('');
            setArchivedActionKind('');
        }
    ...
    return (
        <div className="flex h-full min-h-0 flex-col overflow-hidden border-t border-border/70 pt-3">
            ...
            <div className="gardenflow-history-scroll min-h-0 flex-1 overflow-y-auto px-2 custom-scrollbar">
                        ...
                        <div className="mb-2 flex items-center justify-between px-1">
                            <div className="flex items-center gap-2 text-[11px] font-bold text-text-tertiary">
                                ...
                                {archivedSessionList.length > 0 && (
                                    <span className="rounded-full bg-surface-secondary px-1.5 py-0.5 text-[9px] text-text-tertiary">
                                        {archivedSessionList.length}
                                    </span>
                                )}
                            </div>
                            <button
                                type="button"
                                onClick={() => void onLoadArchivedSessions?.()}
                                className="flex h-6 w-6 items-center justify-center rounded-md text-text-tertiary transition-colors hover:bg-surface-secondary hover:text-text-primary disabled:opacity-50"
                                title="刷新"
                                aria-label="刷新已归档对话"
                                disabled={archivedSessionsLoading}
                            >
                                <RefreshCw className={clsx('h-3.5 w-3.5', archivedSessionsLoading && 'animate-spin')} />
                            </button>
                        </div>
                        {archivedSessionsLoading && archivedSessionList.length === 0 ? (
                            <div className="flex items-center justify-center py-10">
                                <Loader2 className="h-5 w-5 animate-spin text-accent-primary/50" />
                            </div>
                        ) : archivedSessionList.length === 0 ? (
                            <div className="mx-3 rounded-lg border border-dashed border-border/80 px-3 py-6 text-center text-[11px] text-text-tertiary">
                                暂无已归档对话
                            </div>
                        ) : (
                            <div className="space-y-1">
                                {archivedSessionList.map((session) => {
                                    const title = displaySessionTitle(session.chatSession?.title?.trim() || '未命名会话', session.surface);
                                    const isActing = archivedActionSessionId === session.id;
                                    const updatedAt = String(session.chatSession?.updatedAt || session.chatSession?.createdAt || '').trim();
                                    ...
                                    return (
                                        <div
                                            key={session.id}
                                            className="group rounded-lg border border-border/60 bg-surface-primary/60 px-3 py-2.5"
                                        >
                                            <div className="flex items-start gap-2">
                                                <Archive className="mt-0.5 h-3.5 w-3.5 shrink-0 text-text-tertiary" />
                                                <div className="min-w-0 flex-1">
                                                    <div className="flex min-w-0 items-center gap-1.5">
                                                        ...
                                                        <span className="truncate text-[13px] font-semibold text-text-secondary">{title}</span>
                                                    </div>
                                                    {updatedLabel && (
                                                        <div className="mt-1 truncate text-[9px] text-text-tertiary">{updatedLabel}</div>
                                                    )}
                                                </div>
                                            </div>
                                            <div className="mt-2 flex items-center justify-end gap-1">
                                                <button
                                                    ...
                                                    disabled={Boolean(archivedActionSessionId)}
                                                    className="inline-flex h-7 items-center gap-1 rounded-lg px-2.5 text-[11px] font-medium text-text-secondary transition-colors hover:bg-surface-secondary hover:text-text-primary disabled:opacity-50"
                                                >
                                                    {isActing && archivedActionKind === 'restore' && <Loader2 className="h-3 w-3 animate-spin" />}
                                                    恢复
                                                </button>
                                                <button
                                                    type="button"
                                                    ...
                                                    disabled={Boolean(archivedActionSessionId)}
                                                    className="inline-flex h-7 items-center gap-1 rounded-lg px-2.5 text-[11px] font-medium text-red-500 transition-colors hover:bg-red-500/10 disabled:opacity-50"
                                                >
                                                    {isActing && archivedActionKind === 'delete'
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="flex items-center justify-between gap-2">
                                        ...
                                        {activity === 'failed' && !isUnread && (
                                            <span
                                                className="absolute right-4 top-1/2 h-2.5 w-2.5 -translate-y-1/2 rounded-full bg-red-500 shadow-[0_0_0_3px_rgba(239,68,68,0.14)] transition-opacity group-hover:opacity-0"
                                                aria-label="执行失败"
                                            />
                                        )}
            ...
            {renameTarget && (
                <div
                    className="gardenflow-rename-dialog-backdrop fixed inset-0 z-[130] flex items-center justify-center px-4"
                    onMouseDown={closeRenameDialog}
                >
                    <div
                        className="gardenflow-rename-dialog w-full max-w-[420px] rounded-2xl p-5"
                        onMouseDown={(event) => event.stopPropagation()}
                    >
                        <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0">
                                <h3 className="gardenflow-rename-dialog-title text-lg font-bold">重命名对话</h3>
                            </div>
                            <button
                                type="button"
                                onClick={closeRenameDialog}
                                disabled={isRenaming}
                                className="gardenflow-rename-dialog-close flex h-8 w-8 shrink-0 items-center justify-center rounded-lg transition-colors disabled:opacity-50"
                                title="关闭"
                                aria-label="关闭"
                            >
                                <X className="h-4 w-4" />
                        ...
                        <input
                            ...
                            disabled={isRenaming}
                            className="gardenflow-rename-dialog-input mt-5 h-11 w-full rounded-xl px-3 text-sm outline-none transition disabled:opacity-60"
                            maxLength={80}
                        />
                        {renameError && (
                            <div className="gardenflow-rename-dialog-error mt-2 text-xs">{renameError}</div>
                        )}
                        <div className="mt-5 flex items-center justify-end gap-2">
                            <button
                                ...
                                onClick={closeRenameDialog}
                                disabled={isRenaming}
                                className="gardenflow-rename-dialog-secondary h-9 rounded-xl px-4 text-sm transition-colors disabled:opacity-50"
                            >
                                取消
                            </button>
                            <button
                                ...
                                className="gardenflow-rename-dialog-primary inline-flex h-9 items-center gap-2 rounded-xl px-4 text-sm font-medium transition-colors disabled:opacity-50"
                            >
            ...
            {manuscriptDialog && (
                <div
                    className="gardenflow-manuscript-dialog-backdrop fixed inset-0 z-[130] flex items-center justify-center bg-black/30 px-4"
                    onMouseDown={closeManuscriptDialog}
                >
                    <div
                        className="gardenflow-manuscript-dialog w-full max-w-[420px] rounded-2xl p-5"
                        onMouseDown={(event) => event.stopPropagation()}
                    >
                        <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0">
                                <h3 className="gardenflow-manuscript-dialog-title text-lg font-bold">
                            ...
                            <button
                                ...
                                className="gardenflow-manuscript-dialog-close flex h-8 w-8 shrink-0 items-center justify-center rounded-lg transition-colors disabled:opacity-50"
                                title="关闭"
                                aria-label="关闭"
                            >
                                <X className="h-4 w-4" />
                        ...
                        {manuscriptDialog.mode !== 'create-file' && (
                            <input
                                ...
                                disabled={isSubmittingManuscriptDialog}
                                className="gardenflow-manuscript-dialog-input mt-5 h-11 w-full rounded-xl px-3 text-sm outline-none transition disabled:opacity-60"
                        ...
                        {manuscriptDialog.mode !== 'create-file' && (
                            <div className="mt-5 flex items-center justify-end gap-2">
                                <button
                                    ...
                                    onClick={closeManuscriptDialog}
                                    disabled={isSubmittingManuscriptDialog}
                                    className="gardenflow-manuscript-dialog-secondary h-9 rounded-xl px-4 text-sm transition-colors disabled:opacity-50"
                                >
                                    取消
                                </button>
                                <button
                                    ...
                                    className="gardenflow-manuscript-dialog-primary inline-flex h-9 items-center gap-2 rounded-xl px-4 text-sm font-medium transition-colors disabled:opacity-50"
                                >
