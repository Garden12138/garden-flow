...
export function MediaLibrary({
    ...
    const useAssetAsVideoReference = useCallback((asset: MediaAsset) => {
        if (!onNavigateToGenerationStudio || !asset.exists || isVideoAsset(asset) || isAudioAsset(asset)) return;
        const source = String(asset.absolutePath || asset.previewUrl || asset.relativePath || '').trim();
        if (!source) {
            void appAlert('该素材没有可读取的图片文件');
            return;
        }
        const fileName = source.split(/[\\/]/).filter(Boolean).pop() || asset.id;
        onNavigateToGenerationStudio({
            mode: 'video',
            source: 'media-library',
            sourceTitle: asset.title || fileName,
            preset: { videoGenerationMode: 'reference-guided' },
            assetReferences: [{
                assetId: asset.id,
                name: fileName,
    ...
    return (
        <div className="workbench-resources workbench-resources--media min-h-full flex flex-col bg-background text-text-primary">
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div className="p-3 space-y-2">
                                                ...
                                                {!isVideoAsset(asset) && !isAudioAsset(asset) && asset.exists && onNavigateToGenerationStudio && (
                                                    <button
                                                        type="button"
                                                        onClick={(event) => {
                                                            event.stopPropagation();
                                                            useAssetAsVideoReference(asset);
                                                        }}
                                                        className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-border bg-surface-secondary px-2.5 text-[11px] font-medium text-text-secondary transition-colors hover:border-accent-primary/35 hover:text-accent-primary"
                                                    >
                    ...
                    {!isVideoAsset(contextMenu.asset) && !isAudioAsset(contextMenu.asset) && contextMenu.asset.exists && onNavigateToGenerationStudio && (
                        <button
                            ...
                            onClick={() => {
                                ...
                                if (asset) useAssetAsVideoReference(asset);
                            }}
