...
function mediaAssetMatchesAllowedExtensions(
    asset: Pick<MediaAsset, 'previewUrl' | 'absolutePath' | 'relativePath'>,
    ...
    const source = mediaAssetSourceUrl(asset).split(/[?#]/, 1)[0] || '';
...
interface SubjectsProps {
    ...
    onClose?: () => void;
    variant?: 'page' | 'modal';
    referencePicker?: Pick<GenerationAssetPickerRequest, 'mediaKind' | 'maxCount' | 'allowedExtensions' | 'onSelect'>;
}

export function Subjects({ isActive = true, onReturnHome, onClose, variant = 'page', referencePicker }: SubjectsProps) {
    const isModalVariant = variant === 'modal';
    const [selectedReferences, setSelectedReferences] = useState<GenerationAssetReference[]>([]);
    ...
    const addReferenceSelection = useCallback((references: GenerationAssetReference[]) => {
        if (!referencePicker || references.length === 0) return;
        setSelectedReferences((current) => {
            const referenceKey = (reference: GenerationAssetReference) => (
                reference.absolutePath || reference.previewUrl || `${reference.assetId || ''}:${reference.name}`
            );
            const incomingKeys = new Set(references.map(referenceKey));
            const isEntireSelectionActive = references.every((reference) => (
                current.some((item) => referenceKey(item) === referenceKey(reference))
            ));
            if (isEntireSelectionActive) {
                return current.filter((item) => !incomingKeys.has(referenceKey(item)));
            }

            const next = [...current];
            for (const reference of references) {
                const key = referenceKey(reference);
                if (!next.some((item) => referenceKey(item) === key) && next.length < referencePicker.maxCount) {
                    next.push(reference);
                }
            }
            return next.slice(0, referencePicker.maxCount);
        });
    }, [referencePicker]);

    const isReferenceSelected = useCallback((assetId: string) => (
        selectedReferences.some((reference) => reference.assetId === assetId)
    ), [selectedReferences]);

    const selectMediaReference = useCallback((asset: MediaAsset) => {
        if (!referencePicker || !asset.exists) return;
        const matchesRequestedKind = referencePicker.mediaKind === 'audio'
            ? isAudioAsset(asset)
            : !isVideoAsset(asset) && !isAudioAsset(asset);
        if (!matchesRequestedKind || !mediaAssetMatchesAllowedExtensions(asset, referencePicker.allowedExtensions)) return;
        const source = mediaAssetSourceUrl(asset).trim();
        if (!source) return;
        const name = source.split(/[\\/]/).filter(Boolean).pop() || asset.title || asset.id;
        addReferenceSelection([{
            assetId: asset.id,
            name,
            mimeType: asset.mimeType || `${referencePicker.mediaKind}/*`,
            absolutePath: asset.absolutePath,
            previewUrl: asset.previewUrl || asset.relativePath,
        }]);
    }, [addReferenceSelection, referencePicker]);

    const selectSubjectReferences = useCallback((subject: SubjectRecord) => {
        if (!referencePicker || referencePicker.mediaKind !== 'image') return;
        const absolutePaths = Array.isArray(subject.absoluteImagePaths) ? subject.absoluteImagePaths : [];
        const previewUrls = Array.isArray(subject.previewUrls) ? subject.previewUrls : [];
        const relativePaths = Array.isArray(subject.imagePaths) ? subject.imagePaths : [];
        const count = Math.min(
            referencePicker.maxCount,
            Math.max(absolutePaths.length, previewUrls.length, relativePaths.length),
        );
        const references: GenerationAssetReference[] = Array.from({ length: count }).flatMap((_, index) => {
            const source = absolutePaths[index] || previewUrls[index] || relativePaths[index] || '';
            if (!source) return [];
            return [{
                assetId: subject.id,
                name: relativePaths[index]?.split(/[\\/]/).filter(Boolean).pop() || `${subject.name}-${index + 1}.png`,
                mimeType: 'image/*',
                absolutePath: absolutePaths[index],
                previewUrl: previewUrls[index] || relativePaths[index],
            }];
        });
        if (references.length > 0) addReferenceSelection(references);
    }, [addReferenceSelection, referencePicker]);
    ...
    const availableDetailPlatforms = useMemo(() => {
        ...
        return ALL_ECOMMERCE_PLATFORMS.filter((platform) => sourcePlatformIds.has(platform.id));
        ...
        return mediaAssets.filter((asset) => {
            if (referencePicker) {
                ...
                const matchesRequestedKind = referencePicker.mediaKind === 'audio'
                    ? isAudioAsset(asset)
                    : !isVideoAsset(asset) && !isAudioAsset(asset);
                if (!matchesRequestedKind || !mediaAssetMatchesAllowedExtensions(asset, referencePicker.allowedExtensions)) return false;
            }
            if (!keyword) return true;
            const haystack = [
                asset.title || '',
    ...
    }, [mediaAssets, query, referencePicker]);
    ...
    const openProductDetailPage = useCallback((brand: BrandWorkspaceBrand, productBundle: BrandWorkspaceProductBundle) => {
        ...
        setSelectedDetailPlatformId(productBundle.sourceSnapshots?.[0]?.platform || '');
        setError('');
    }, []);
            ...
            <div className="fixed inset-0 z-[10020] flex min-h-0 flex-col bg-white">
                <div className={clsx('shrink-0 border-b border-[rgb(var(--color-border))] bg-white', isModalVariant ? 'px-5 pb-4 pt-14' : 'px-8 pb-5 pt-14')}>
                    <div className="flex min-w-0 items-center gap-3">
                        <button
                            ...
                            className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-[rgb(var(--color-border))] bg-white text-[rgb(var(--color-text-primary))] shadow-sm transition hover:bg-[rgb(var(--color-surface-primary))]"
                            aria-label="返回资产库"
                            title="返回"
                        >
                        ...
                        <div className="flex shrink-0 items-center gap-2">
                            <button
                                ...
                                className="inline-flex h-9 items-center gap-1.5 rounded-lg bg-black px-3 text-sm font-semibold text-white transition hover:bg-black/85"
                            >
                ...
                <div className="flex min-h-0 flex-1 bg-[rgb(var(--color-surface-primary))]">
                    <aside className={clsx('hidden min-h-0 w-[250px] shrink-0 overflow-y-auto border-r border-[rgb(var(--color-border))] bg-[rgb(var(--color-surface-primary))] p-4 lg:block', isModalVariant && 'w-[220px]')}>
                        <div className="mb-3 text-xs font-semibold text-[rgb(var(--color-text-secondary))]">采集来源</div>
                        {hasPlatforms ? (
                                ...
                                {availableDetailPlatforms.map((platform) => {
                                    ...
                                    return (
                                        <button
                                            key={platform.id}
                                            type="button"
                                            onClick={() => setSelectedDetailPlatformId(platform.id)}
                                            className={clsx(
                    ...
                    <main className="min-w-0 flex-1 overflow-y-auto bg-white">
                        <div className={clsx('mx-auto min-h-full w-full max-w-[1180px] space-y-5 bg-white py-5 pb-10', isModalVariant ? 'px-5' : 'px-8')}>
                            ...
                            {!activeProductSourceSnapshot && (
                                <section className="rounded-xl border border-[rgb(var(--color-border))] bg-white p-5">
                                    <div className="text-sm font-semibold text-[rgb(var(--color-text-primary))]">商品资料</div>
                                    <div className="mt-1 text-xs leading-5 text-[rgb(var(--color-text-secondary))]">
                                        这个商品暂无网页采集快照，以下是资产库中保存的人工资料。
                                    </div>
                                    <div className="mt-5 grid gap-5 lg:grid-cols-[220px_minmax(0,1fr)]">
                                        <div className="grid grid-cols-2 gap-2">
                                            {productInfoImages.map((asset) => (
                                                <div key={asset.id} className="aspect-square overflow-hidden rounded-lg bg-[rgb(var(--color-surface-secondary))]">
                                                    <img src={resolveAssetUrl(asset.path)} alt="" className="h-full w-full object-cover" />
                                                </div>
                                            ))}
                                            {productInfoImages.length === 0 && (
                                                <div className="col-span-2 flex aspect-[4/3] items-center justify-center rounded-lg bg-[rgb(var(--color-surface-secondary))] text-[rgb(var(--color-text-tertiary))]">
                                        ...
                                        <div className="space-y-4">
                                            {activeDetailProductBundle.product.description && (
                                                <p className="text-sm leading-7 text-[rgb(var(--color-text-secondary))]">
                                                    {activeDetailProductBundle.product.description}
                                                </p>
                                            )}
                                            {(activeDetailProductBundle.product.facts || []).length > 0 && (
                                                <dl className="grid gap-x-6 rounded-xl border border-[rgb(var(--color-border))] px-4 md:grid-cols-2">
                                                    {(activeDetailProductBundle.product.facts || []).map((fact) => (
                                                        <div key={`${fact.key}-${fact.value}`} className="grid grid-cols-[96px_minmax(0,1fr)] gap-3 border-b border-black/[0.04] py-3 text-sm leading-6">
                                                            <dt className="text-[rgb(var(--color-text-tertiary))]">{fact.key}</dt>
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                {activeDetailProductBundle.skus.length === 0 ? (
                                                    <span className="rounded-md bg-[rgb(var(--color-surface-secondary))] px-2 py-1 text-xs text-[rgb(var(--color-text-secondary))]">暂无 SKU</span>
                                                ) : activeDetailProductBundle.skus.map((sku) => (
                                                    <span key={sku.id} className="rounded-md bg-[rgb(var(--color-surface-secondary))] px-2 py-1 text-xs text-[rgb(var(--color-text-secondary))]">
                                                        {sku.name}{sku.variantText ? ` · ${sku.variantText}` : ''}
                                                    </span>
                                                ))}
                                            </div>
                                        </div>
                    ...
                    {referencePicker && (
                        <div className="flex items-center gap-2">
                            <span className="rounded-full bg-violet-50 px-3 py-1 text-xs font-semibold text-violet-700">
                                已选 {selectedReferences.length}/{referencePicker.maxCount} {referencePicker.mediaKind === 'audio' ? '段' : '张'}
                            </span>
                            <button
                                type="button"
                                disabled={selectedReferences.length === 0}
                                onClick={() => referencePicker.onSelect(selectedReferences)}
                                className="inline-flex h-8 items-center rounded-lg bg-violet-600 px-3 text-xs font-semibold text-white transition hover:bg-violet-700 disabled:cursor-not-allowed disabled:opacity-40"
                            >
                                确认引用
                            </button>
                        </div>
                                ...
                                {filteredMediaAssets.map((asset) => {
                                    ...
                                    return (
                                        <div
                                            ...
                                            role="button"
                                            tabIndex={0}
                                            onClick={() => referencePicker ? selectMediaReference(asset) : openMediaPreview(asset)}
                                            onContextMenu={referencePicker ? undefined : (event) => openMediaContextMenu(event, asset)}
                                            onKeyDown={(event) => {
                                                if (event.key !== 'Enter' && event.key !== ' ') return;
                                                event.preventDefault();
                                                if (referencePicker) selectMediaReference(asset);
                                                else openMediaPreview(asset);
                                            }}
                                            className={clsx(
                                                'overflow-hidden rounded-lg border bg-surface-primary text-left shadow-sm transition hover:shadow-md',
                                                referencePicker && isReferenceSelected(asset.id) ? 'border-violet-500 ring-2 ring-violet-200' : 'border-border',
                                            )}
                                        >
                                ...
                                {filteredMediaAssets.map((asset) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            onContextMenu={referencePicker ? undefined : (event) => openMediaContextMenu(event, asset)}
                                            className={clsx(
                                                'flex w-full items-center gap-3 px-3 py-2 text-left transition hover:bg-[rgb(var(--color-surface-primary))]',
                                                referencePicker && isReferenceSelected(asset.id) && 'bg-violet-50 ring-1 ring-inset ring-violet-300',
                                            )}
                                        >
                                                ...
                                                {products.length === 0 ? (
                                                    ...
                                                    <div className="space-y-1.5">
                                                        {products.map((productBundle) => {
                                                            ...
                                                            const sourceCount = productBundle.sourceSnapshots?.length || 0;
                                                            return (
                                                            <div
                                                                key={product.id}
                                                                role="button"
                                                            ...
                                                            >
                                                                ...
                                                                <div className="hidden min-w-0 shrink-0 items-center gap-1 sm:flex">
                                                                    <span className="rounded-md bg-[rgb(var(--color-surface-secondary))] px-2 py-1 text-[11px] text-[rgb(var(--color-text-secondary))]">
                                                                        {sourceCount > 0 ? `${sourceCount} 个采集来源` : '人工资料'}
                                                                    </span>
                                                                    <button
                                                                        type="button"
                                                                        onClick={(event) => {
                    ...
                    <div className={clsx('grid gap-3', isModalVariant ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4' : 'grid-cols-2 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5')}>
                        {filteredSubjects.map((subject) => {
                            ...
                            <div
                                key={subject.id}
                                className={clsx(
                                    'overflow-hidden rounded-lg border bg-surface-primary shadow-sm transition-shadow hover:shadow-md',
                                    referencePicker && isReferenceSelected(subject.id) ? 'border-violet-500 ring-2 ring-violet-200' : 'border-border',
                                )}
                            >
                                <button
                                    type="button"
                                    onClick={() => referencePicker ? selectSubjectReferences(subject) : openEditModal(subject)}
                                    disabled={Boolean(referencePicker) && !subject.primaryPreviewUrl}
                    ...
                    <div className="divide-y divide-[rgb(var(--color-border))] rounded-xl border border-[rgb(var(--color-border))] bg-white">
                        {filteredSubjects.map((subject) => {
                            ...
                            <button
                                ...
                                disabled={Boolean(referencePicker) && !subject.primaryPreviewUrl}
                                className={clsx(
                                    'flex w-full items-center gap-3 px-3 py-2 text-left transition hover:bg-[rgb(var(--color-surface-primary))] disabled:cursor-not-allowed disabled:opacity-50',
                                    referencePicker && isReferenceSelected(subject.id) && 'bg-violet-50 ring-1 ring-inset ring-violet-300',
                                )}
                            >
                                <div className="h-12 w-12 shrink-0 overflow-hidden rounded-lg bg-[rgb(var(--color-surface-secondary))]">
                                    {subject.primaryPreviewUrl ? (
