...
import type { GenerationAssetPickerRequest, GenerationIntent, PendingChatMessage } from '../features/app-shell/types';
...
import {
    ...
    isSuccessfulAgentGenerationEntry,
...
interface GenerationStudioProps {
    ...
    onReturnHome?: () => void;
    onOpenAssets?: (request?: GenerationAssetPickerRequest) => void;
}
...
function isAudioReference(item: ReferenceItem | null | undefined): boolean {
    return String(item?.dataUrl || '').startsWith('data:audio/');
}
...
function applyIntentPreset(
    ...
    if (intent.preset?.videoGenerationMode) {
        setters.setVideoMode(intent.preset.videoGenerationMode);
    }
}

async function generationIntentReferenceItems(intent: GenerationIntent): Promise<ReferenceItem[]> {
    const references = Array.isArray(intent.assetReferences) ? intent.assetReferences : [];
    return Promise.all(references.slice(0, 9).map(async (reference, index) => {
        const source = String(reference.absolutePath || reference.previewUrl || '').trim();
        if (!source) throw new Error(`素材 ${reference.name || index + 1} 没有可读取的文件路径`);
        if (source.startsWith('data:')) {
            return { name: reference.name || `素材 ${index + 1}`, dataUrl: source };
        }
        const response = await fetch(resolveAssetUrl(source));
        if (!response.ok) throw new Error(`素材 ${reference.name || index + 1} 读取失败`);
        return {
...
function UploadPreviewCard({
    ...
    const leadIsAudio = isAudioReference(lead);
                ...
                {hasItems ? (
                    <>
                        ...
                        <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent px-2 pb-2 pt-4 text-center">
                            ...
                            {items.length > 1 && (
                                <div className="mt-0.5 text-[10px] text-white/90">{items.length} {leadIsAudio ? '段' : '张'}</div>
                            )}
                        </div>
                    </>
                ) : (
...
function requestReferenceItems(request: GenerationRequest): ReferenceItem[] {
    ...
    if (request.type === 'video') {
        items.push(...request.referenceAudios);
...
function ReferencePreviewStrip({ request }: { request: GenerationRequest }) {
    ...
    return (
        <div className="flex max-w-[680px] items-center gap-2 overflow-x-auto">
            <div className="shrink-0 text-[11px] text-text-tertiary">参考素材</div>
            <div className="flex min-w-0 items-center gap-2">
...
export function GenerationStudio({
    ...
    const lastSettingsVideoModelRef = useRef('');
    ...
    const [isPreparingAgentMessage, setIsPreparingAgentMessage] = useState(false);
    ...
    const [videoReferenceAudios, setVideoReferenceAudios] = useState<ReferenceItem[]>([]);
    ...
    const visibleFeedItems = useMemo(
        () => feedEntries
            ...
            .filter(({ entry }) => !isSuccessfulAgentGenerationEntry(entry, agentSessionId)),
        [agentSessionId, feedEntries],
    );
    ...
    const loadContext = useCallback(async (overwriteDraftDefaults = false) => {
        try {
            ...
            const nextSettingsVideoModel = String(normalizedSettings.video_model || '').trim();
            const previousSettingsVideoModel = lastSettingsVideoModelRef.current;
            setVideoModel((prev) => {
                const current = prev.trim();
                return overwriteDraftDefaults || !current || current === previousSettingsVideoModel
                    ? nextSettingsVideoModel
                    : prev;
            });
            lastSettingsVideoModelRef.current = nextSettingsVideoModel;
    ...
    useEffect(() => {
        ...
        if (pendingIntent.assetReferences?.length && (pendingIntent.mode === 'video' || pendingIntent.mode === 'image' || pendingIntent.mode === 'cover')) {
            void generationIntentReferenceItems(pendingIntent)
                .then((items) => {
                    if (cancelled) return;
                    if (pendingIntent.mode === 'video') {
                        const requestedMode = pendingIntent.preset?.videoGenerationMode || 'reference-guided';
                        if (requestedMode === 'first-last-frame') {
                            setVideoFirstFrame(items[0] || null);
                            setVideoLastFrame(items[1] || null);
                        } else {
                            setVideoReferences(items);
                        }
                        setVideoMode(requestedMode);
                        setVideoError('');
                    } else if (pendingIntent.mode === 'cover') {
                        setCoverReferences(items.slice(0, 4));
                        setCoverError('');
                    } else {
                        setImageReferences(items.slice(0, 4));
                        setImageMode('reference-guided');
                        setImageError('');
                    }
                })
                .catch((error) => {
                    if (cancelled) return;
                    const message = error instanceof Error ? error.message : '资产素材读取失败';
                    if (pendingIntent.mode === 'video') setVideoError(message);
                    else if (pendingIntent.mode === 'cover') setCoverError(message);
                    else setImageError(message);
                })
                .finally(() => {
                    if (!cancelled) onIntentConsumed?.();
                });
        } else {
            onIntentConsumed?.();
        }
    ...
    useEffect(() => {
        onExecutionStateChange?.(isPreparingAgentMessage || agentExecutionActive || feedEntries.some((entry) => isGenerationFeedEntry(entry) && entry.status === 'running'));
    }, [agentExecutionActive, feedEntries, isPreparingAgentMessage, onExecutionStateChange]);
    ...
    const videoModelRoutes = useMemo(() => buildVideoModelRoutes(settings as Record<string, unknown>), [settings]);
    const selectedVideoModelRoute = useMemo(
        () => videoModelRoutes.find((route) => route.model === videoModel.trim()) || videoModelRoutes[0] || null,
        [videoModel, videoModelRoutes],
    );
    ...
    const videoModelOptions = useMemo<PickerOption[]>(() => videoModelRoutes.map((route) => ({
        value: route.model,
        label: route.model,
        description: `${route.provider.name} · ${route.capabilities.summary}`,
    })), [videoModelRoutes]);
    const videoModeOptions = useMemo<PickerOption[]>(() => (activeVideoCapabilities?.supportedModes || [])
        .map((mode) => ({
            value: mode,
            label: mode === 'reference-guided' && activeVideoCapabilities?.supportsReferenceAudio
                ? '多模态参考'
                : VIDEO_MODE_LABELS[mode],
        })), [activeVideoCapabilities]);
    const videoAspectRatioOptions = useMemo<PickerOption[]>(() => (activeVideoCapabilities?.aspectRatios || [])
        .map((value) => ({ value, label: value })), [activeVideoCapabilities]);
    const videoResolutionOptions = useMemo<PickerOption[]>(() => (activeVideoCapabilities?.resolutions || [])
        .map((value) => ({ value, label: value })), [activeVideoCapabilities]);
    const videoDurationOptions = useMemo<PickerOption[]>(() => (activeVideoCapabilities?.durationSeconds || [])
        .map((value) => ({ value: String(value), label: `${value} 秒` })), [activeVideoCapabilities]);
    ...
    const audioEmotionOptions = useMemo(() => {
        if (!isMinimaxTtsModel(effectiveAudioModel)) return AUDIO_EMOTION_OPTIONS;
        return AUDIO_EMOTION_OPTIONS.filter((option) => (
            option.value !== 'fluent'
            && !(option.value === 'whisper' && /speech-2\.8-/i.test(effectiveAudioModel))
        ));
    }, [effectiveAudioModel]);
    ...
    useEffect(() => {
        setVideoModel((prev) => {
            const current = prev.trim();
            if (current && videoModelRoutes.some((route) => route.model === current)) return prev;
            return videoModelRoutes[0]?.model || '';
        });
    }, [videoModelRoutes]);

    useEffect(() => {
        if (!activeVideoCapabilities) return;
        setVideoMode((current) => (
            activeVideoCapabilities.supportedModes.includes(current)
                ? current
                : activeVideoCapabilities.supportedModes[0] || 'text-to-video'
        ));
        setVideoAspectRatio((current) => (
            activeVideoCapabilities.aspectRatios.includes(current)
                ? current
                : activeVideoCapabilities.aspectRatios[0] || '16:9'
        ));
        setVideoResolution((current) => (
            activeVideoCapabilities.resolutions.includes(current)
                ? current
                : activeVideoCapabilities.resolutions[0] || '720p'
        ));
        setVideoDurationSeconds((current) => {
            if (activeVideoCapabilities.durationSeconds.includes(current)) return current;
            return activeVideoCapabilities.durationSeconds.includes(8)
                ? 8
                : activeVideoCapabilities.durationSeconds[0] || 8;
        });
        if (!activeVideoCapabilities.supportsGeneratedAudio) setVideoGenerateAudio(false);
        setVideoReferences((current) => current.slice(0, Math.max(0, activeVideoCapabilities.maxReferenceImages)));
        setVideoReferenceAudios((current) => activeVideoCapabilities.supportsReferenceAudio
            ? current.slice(0, activeVideoCapabilities.maxReferenceAudios)
            : []);
    }, [activeVideoCapabilities]);

    useEffect(() => {
        const needsReferenceMode = videoReferences.some(Boolean) && videoMode === 'reference-guided';
        const needsFirstLastMode = Boolean(videoFirstFrame || videoLastFrame) && videoMode === 'first-last-frame';
        if (!needsReferenceMode && !needsFirstLastMode) return;
        if (activeVideoCapabilities?.supportedModes.includes(videoMode)) return;
        const compatibleRoute = videoModelRoutes.find((route) => route.capabilities.supportedModes.includes(videoMode));
        if (compatibleRoute) setVideoModel(compatibleRoute.model);
    }, [activeVideoCapabilities, videoFirstFrame, videoLastFrame, videoMode, videoModelRoutes, videoReferences]);
    ...
    useEffect(() => {
        if (!audioEmotionOptions.some((option) => option.value === audioEmotion)) {
            setAudioEmotion('');
            setAudioEmotionTouched(false);
        }
    }, [audioEmotion, audioEmotionOptions]);
    ...
    const runVideoRequest = useCallback((request: VideoGenerationRequest): boolean => {
        const route = videoModelRoutes.find((item) => item.model === request.model) || selectedVideoModelRoute;
        const validationError = validateVideoGenerationRequest(request, {
            hasVideoConfig,
            videoModelCapabilities: route?.capabilities || null,
        });
        if (validationError) {
            ...
            try {
                const result = await submitVideoGeneration(window.ipcRenderer.generation, request, {
                    ...
                    routeOverride: route ? {
                        ...
                        provider: route.capabilities.providerKind,
                    } : undefined,
                });
        ...
        return true;
    }, [contextIntent?.source, createFeedEntry, hasVideoConfig, selectedVideoModelRoute, updateFeedEntries, videoModelRoutes]);
    ...
    const handleGenerateVideo = useCallback(() => {
        const accepted = runVideoRequest(buildVideoGenerationRequest({
            ...
            referenceAudios: videoReferenceAudios,
    ...
    const handleEditEntry = useCallback((entry: GenerationFeedEntry) => {
        ...
        setVideoModel(entry.request.model);
        ...
        setVideoReferenceAudios(entry.request.referenceAudios || []);
    ...
    const setVideoReferenceFile = useCallback(async (
        ...
        try {
            ...
            if (typeof target === 'number') {
                if (!activeVideoCapabilities?.supportedModes.includes('reference-guided')) {
                    throw new Error(`模型 ${effectiveVideoModel || '当前模型'} 不支持参考图视频`);
                }
                setVideoReferences((prev) => {
                    const next = [...prev];
                    next[target] = item;
                    return next.slice(0, activeVideoCapabilities.maxReferenceImages);
                });
    ...
    }, [activeVideoCapabilities, effectiveVideoModel, videoMode]);
    ...
    const appendVideoReferenceFiles = useCallback(async (files: File[]) => {
        ...
        try {
            ...
            const nextItems = await filesToReferenceItems(files, activeVideoCapabilities.maxReferenceImages);
            setVideoReferences((prev) => [...prev.filter(Boolean), ...nextItems].slice(0, activeVideoCapabilities.maxReferenceImages));
    ...
    const appendVideoReferenceAudioFiles = useCallback(async (files: File[]) => {
        if (!files.length) return;
        if (!activeVideoCapabilities?.supportsReferenceAudio || activeVideoCapabilities.maxReferenceAudios < 1) {
            setVideoError(`模型 ${effectiveVideoModel || '当前模型'} 不支持参考音频输入`);
            return;
        }
        setIsReadingVideoRefs(true);
        try {
            const nextItems = await filesToReferenceItems(files, activeVideoCapabilities.maxReferenceAudios);
            setVideoReferenceAudios((current) => (
                [...current, ...nextItems].slice(0, activeVideoCapabilities.maxReferenceAudios)
            ));
            setVideoMode('reference-guided');
            setVideoError('');
        } catch (error) {
            console.error('Failed to read video reference audio:', error);
            setVideoError('参考音频读取失败，请重试');
        } finally {
            setIsReadingVideoRefs(false);
        }
    }, [activeVideoCapabilities, effectiveVideoModel]);

    const handleVideoReferenceAudioFiles = useCallback(async (event: React.ChangeEvent<HTMLInputElement>) => {
        try {
            await appendVideoReferenceAudioFiles(Array.from(event.target.files || []));
        } finally {
            event.target.value = '';
        }
    }, [appendVideoReferenceAudioFiles]);

    const openGenerationAssetPicker = useCallback(() => {
        if (!onOpenAssets || (studioMode !== 'image' && studioMode !== 'cover' && studioMode !== 'video')) return;
        const maxCount = studioMode === 'video'
            ? (videoMode === 'first-last-frame' ? 2 : Math.max(1, activeVideoCapabilities?.maxReferenceImages || 5))
            : 4;
        onOpenAssets({
            mediaKind: 'image',
            maxCount,
            onSelect: (references) => {
                void generationIntentReferenceItems({
                    mode: studioMode,
                    source: 'standalone',
                    ...
                    if (studioMode === 'image') {
                        ...
                        setVideoReferences(items.slice(0, maxCount));
                        setVideoMode('reference-guided');
                        setVideoError('');
                    }
                }).catch((error) => {
                    const message = error instanceof Error ? error.message : '资产素材读取失败';
                    if (studioMode === 'image') setImageError(message);
                    else if (studioMode === 'cover') setCoverError(message);
                    else setVideoError(message);
                });
            },
        });
    }, [activeVideoCapabilities?.maxReferenceImages, onOpenAssets, studioMode, videoMode]);

    const openGenerationAudioAssetPicker = useCallback(() => {
        if (!onOpenAssets || studioMode !== 'video' || !activeVideoCapabilities?.supportsReferenceAudio) return;
        onOpenAssets({
            mediaKind: 'audio',
            maxCount: activeVideoCapabilities.maxReferenceAudios,
            allowedExtensions: activeVideoCapabilities.referenceAudioFormats,
            onSelect: (references) => {
                void generationIntentReferenceItems({
                    mode: 'video',
                    source: 'standalone',
                    ...
                    setVideoReferenceAudios(items.slice(0, activeVideoCapabilities.maxReferenceAudios));
                    setVideoMode('reference-guided');
                    setVideoError('');
                }).catch((error) => {
                    setVideoError(error instanceof Error ? error.message : '资产库参考音频读取失败');
                });
            },
        });
    }, [activeVideoCapabilities, onOpenAssets, studioMode]);
    ...
    const composerGridClass = studioMode === 'audio' || studioMode === 'digital-human'
        ...
        : (studioMode === 'video' && (
            ...
            || (videoMode === 'reference-guided' && activeVideoCapabilities?.supportsReferenceAudio)
        ))
    ...
    const currentVideoParameterError = currentAgentRequest.type === 'video' && currentAgentRequest.prompt.trim()
        ? validateVideoGenerationRequest(currentAgentRequest, {
            hasVideoConfig,
            videoModelCapabilities: activeVideoCapabilities,
        })
        : null;
    const canSendAgentMessage = isAgentMode
        ...
        && !isPreparingAgentMessage
        && !agentExecutionActive
        && !currentVideoParameterError
    ...
    const handleClearGenerationRecords = useCallback(async () => {
        ...
        try {
            ...
            setAgentPendingMessage(null);
            setIsPreparingAgentMessage(false);
            setAgentExecutionActive(false);
    ...
    const handleSendAgentMessage = useCallback(async () => {
        ...
        const hasCoverContext = currentAgentRequest.type === 'cover'
            && (currentAgentRequest.referenceItems.length > 0 || Boolean(currentAgentRequest.manuscriptSource));
        if ((!content && !agentAttachment && !hasCoverContext)
            || !agentSessionId
            || isAgentSessionLoading
            || isPreparingAgentMessage
            || agentExecutionActive) return;
        setIsPreparingAgentMessage(true);
        const attachments: UploadedFileAttachment[] = [];
        const attachmentContextNotes: string[] = [];
        const videoReferenceImagePaths: string[] = [];
        const videoReferenceAudioPaths: string[] = [];
        let videoFirstClipPath = '';
        let videoDrivingAudioPath = '';
        ...
        if (currentAgentRequest.type === 'video') {
            try {
                if (activeVideoCapabilities) {
                    attachmentContextNotes.push([
                        `已选视频模型：${currentAgentRequest.model}`,
                        `允许模式：${activeVideoCapabilities.supportedModes.join(', ')}`,
                        `允许时长：${activeVideoCapabilities.durationSeconds.join(', ')} 秒`,
                        `允许比例：${activeVideoCapabilities.aspectRatios.join(', ')}`,
                        `允许清晰度：${activeVideoCapabilities.resolutions.join(', ')}`,
                        `参考图上限：${activeVideoCapabilities.maxReferenceImages}`,
                        `参考音频：${activeVideoCapabilities.supportsReferenceAudio ? `支持，最多 ${activeVideoCapabilities.maxReferenceAudios} 段` : '不支持'}`,
                        `同步生成音频：${activeVideoCapabilities.supportsGeneratedAudio ? '支持' : '不支持'}`,
                        '调用 video_generate 时必须保持上述模型与参数约束，不要替换成不兼容模式。',
                    ].join('\n'));
                }
                ...
                for (const [index, imageReference] of imageReferences.entries()) {
                    const stagedImage = await createInlineAttachment(
                        imageReference,
                        imageReference.name || `video-reference-${index + 1}.png`,
                    );
                    const stagedImagePath = String(stagedImage.absolutePath || '').trim();
                    if (!stagedImagePath) {
                        throw new Error(`参考图 ${index + 1} 暂存成功，但没有返回可供生成工具读取的路径`);
                    }
                    attachments.push(stagedImage);
                    videoReferenceImagePaths.push(stagedImagePath);
                    attachmentContextNotes.push(`视频参考图 ${index + 1}：${stagedImagePath}`);
                }
                if (currentAgentRequest.firstClip?.dataUrl) {
                    const stagedFirstClip = await createInlineAttachment(currentAgentRequest.firstClip, currentAgentRequest.firstClip.name || 'first-clip.mp4');
                    videoFirstClipPath = String(stagedFirstClip.absolutePath || '').trim();
                    if (!videoFirstClipPath) {
                        throw new Error('视频续写素材暂存成功，但没有返回可供生成工具读取的路径');
                    }
                    attachments.push(stagedFirstClip);
                    attachmentContextNotes.push(`视频续写起始素材：${currentAgentRequest.firstClip.name || 'first-clip'}`);
                }
                for (const [index, referenceAudio] of currentAgentRequest.referenceAudios.entries()) {
                    const stagedAudio = await createInlineAttachment(
                        referenceAudio,
                        referenceAudio.name || `reference-audio-${index + 1}.mp3`,
                        false,
                    );
                    const stagedAudioPath = String(stagedAudio.absolutePath || '').trim();
                    if (!stagedAudioPath) {
                        throw new Error(`参考音频 ${index + 1} 暂存成功，但没有返回可供生成工具读取的路径`);
                    }
                    attachments.push({
                        ...stagedAudio,
                        displayOnly: true,
                    });
                    videoReferenceAudioPaths.push(stagedAudioPath);
                    attachmentContextNotes.push([
                        `参考音频 ${index + 1}：${referenceAudio.name || `reference-audio-${index + 1}`}`,
                        `生成工具输入路径：${stagedAudioPath}`,
                        '该音频仅供 video_generate.referenceAudios 使用，不要作为聊天模型音频输入。',
                    ].join('\n'));
                }
                if (currentAgentRequest.drivingAudio?.dataUrl) {
                    const stagedDrivingAudio = await createInlineAttachment(currentAgentRequest.drivingAudio, currentAgentRequest.drivingAudio.name || 'driving-audio.mp3');
                    videoDrivingAudioPath = String(stagedDrivingAudio.absolutePath || '').trim();
                    if (!videoDrivingAudioPath) {
                        throw new Error('驱动音频暂存成功，但没有返回可供生成工具读取的路径');
                    }
                    attachments.push(stagedDrivingAudio);
        ...
        if (currentAgentRequest.type === 'audio') {
            attachmentContextNotes.push([
                `已选 TTS 模型：${currentAgentRequest.model}`,
                `已选音色：${currentAgentRequest.voiceId}`,
                `语言增强：${currentAgentRequest.languageBoost || '自动'}`,
                `语速：${currentAgentRequest.speed}`,
                `情绪：${currentAgentRequest.emotion || '自动'}`,
                `输出格式：${currentAgentRequest.responseFormat}`,
                '调用 audio_generate 时必须保持上述已选模型和音色；不要尝试未配置的模型或服务商。',
            ].join('\n'));
        }
        ...
        const videoToolInputs = currentAgentRequest.type === 'video'
            ? {
                operation: 'generate' as const,
                model: currentAgentRequest.model,
                generationMode: currentAgentRequest.generationMode,
                referenceImages: videoReferenceImagePaths,
                referenceAudios: videoReferenceAudioPaths,
                ...(videoFirstClipPath ? { firstClip: videoFirstClipPath } : {}),
                ...(videoDrivingAudioPath ? { drivingAudio: videoDrivingAudioPath } : {}),
                count: 1,
                durationSeconds: currentAgentRequest.durationSeconds,
                aspectRatio: currentAgentRequest.aspectRatio,
                resolution: currentAgentRequest.resolution,
                generateAudio: currentAgentRequest.generateAudio,
                ...(currentAgentRequest.projectId.trim() ? { projectId: currentAgentRequest.projectId.trim() } : {}),
                ...(currentAgentRequest.title.trim() ? { title: currentAgentRequest.title.trim() } : {}),
            }
            : undefined;
        const generationToolConstraints = videoToolInputs
            ? { videoGenerate: videoToolInputs }
            : currentAgentRequest.type === 'image' || currentAgentRequest.type === 'cover'
                ? { imageGenerate: { model: currentAgentRequest.model } }
                : currentAgentRequest.type === 'audio'
                    ? {
                        audioGenerate: {
                            model: currentAgentRequest.model,
                            voiceId: currentAgentRequest.voiceId,
                        },
                    }
                    : undefined;
        const runtimeContext = buildGenerationAgentRuntimeContext({
            ...
            toolInputOverrides: videoToolInputs
                || generationToolConstraints?.imageGenerate
                || generationToolConstraints?.audioGenerate,
        ...
        setAgentPendingMessage({
            ...
            taskHints: generationToolConstraints
                ? {
                    intent: currentAgentRequest.type === 'video'
                        ? 'video_creation'
                        : currentAgentRequest.type === 'audio'
                            ? 'audio_creation'
                            : currentAgentRequest.type === 'cover'
                                ? 'cover_generation'
                                : 'image_creation',
                    generationToolConstraints,
                }
                : undefined,
        });
        if (studioMode === 'image') {
    ...
    const handleAgentPendingMessageError = useCallback((message: string) => {
        if (studioMode === 'video') setVideoError(message);
        else if (studioMode === 'cover') setCoverError(message);
        else if (studioMode === 'image') setImageError(message);
        else setAudioError(message);
    }, [studioMode]);
    const handleCancelAgentExecution = useCallback(() => {
        const sessionId = String(agentSessionId || '').trim();
        if (!sessionId) return;
        window.ipcRenderer.chat.cancel({ sessionId });
    }, [agentSessionId]);
    const studioToolbar = (
                                ...
                                isGenerationFeedEntry(entry) ? (
                                    ...
                                    <article key={entry.id} className="space-y-3">
                                        ...
                                        <Chat
                                            ...
                                            onPendingMessageError={handleAgentPendingMessageError}
                                            showClearButton={false}
                                            showWelcomeShortcuts={false}
                                            showComposerShortcuts={false}
                                            showComposer={false}
                                            modelSelectionPolicy="follow-default"
                                ...
                                <div className={composerGridClass}>
                                        ...
                                        <div className="space-y-3">
                                            {studioMode === 'image' ? (
                                                ...
                                                <div className="flex gap-3">
                                                    <UploadPreviewCard
                                                        label={isReadingVideoRefs ? '读取中' : '参考图'}
                                                        accept="image/*"
                                                        multiple
                                                    ...
                                                    {videoMode === 'reference-guided' && activeVideoCapabilities?.supportsReferenceAudio && (
                                                        <UploadPreviewCard
                                                            label={isReadingVideoRefs ? '读取中' : '参考音频'}
                                                            accept="audio/mpeg,audio/wav,.mp3,.wav"
                                                            multiple
                                                            items={videoReferenceAudios}
                                                            onChange={handleVideoReferenceAudioFiles}
                                                            onFiles={appendVideoReferenceAudioFiles}
                                                            onClear={() => setVideoReferenceAudios([])}
                                                        />
                                                    )}
                                                </div>
                                            )}
                                    ...
                                    <div className="space-y-3">
                                            ...
                                            {studioMode === 'image' ? (
                                                <>
                                                    ...
                                                    {onOpenAssets && (
                                                        <button
                                                            type="button"
                                                            onClick={openGenerationAssetPicker}
                                                    ...
                                                    <GenerationSubmitButton
                                                        ...
                                                        onCancel={isAgentMode && agentExecutionActive ? handleCancelAgentExecution : undefined}
                                                ...
                                                <>
                                                    ...
                                                    {onOpenAssets
                                                        && videoMode === 'reference-guided'
                                                        && activeVideoCapabilities?.supportsReferenceAudio && (
                                                        <button
                                                            type="button"
                                                            onClick={openGenerationAudioAssetPicker}
                                                            className="inline-flex h-9 items-center gap-2 rounded-full border border-border bg-surface-primary px-3 text-[12px] font-medium text-text-secondary shadow-[var(--ui-shadow-1)] transition-colors hover:border-border/70 hover:bg-surface-tertiary"
                                                        >
                                                            <Music2 className="h-3.5 w-3.5" />
                                                            音频资产
                                                    ...
                                                    <PopoverSelect
                                                        ...
                                                        options={activeVideoCapabilities?.supportsGeneratedAudio
                                                            ? VIDEO_AUDIO_OPTIONS
                                                            : [{ value: 'off', label: '不支持生成音轨' }]}
                                                        className="min-w-[92px]"
                                                        title="生成音轨"
                                                        panelClassName="w-[220px]"
                                                        disabled={!activeVideoCapabilities?.supportsGeneratedAudio}
                                                    />
                                                    <GenerationSubmitButton
                                        ...
                                        {studioMode === 'video' && (
                                            <div className="flex flex-wrap items-center gap-3 text-[12px] text-text-tertiary">
                                                {videoMode === 'reference-guided' && activeVideoCapabilities?.supportsReferenceAudio && (
                                                    <span>参考音频：MP3/WAV，单段 2–15 秒，合计 ≤ 15 秒，最多 {activeVideoCapabilities.maxReferenceAudios} 段</span>
                                                )}
