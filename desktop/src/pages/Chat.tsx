...
import { finalizeCancelledChatMessages } from '../features/chat/chatCancellation';
import {
...
} from '../runtime/chatSessionStore';
import {
  mergeChatMessageSnapshots,
  reconcileOptimisticChatMessageIds,
} from '../utils/chatMessageSnapshot';
import { canPersistChatComposerDraft } from '../utils/chatComposerState';
import {
  chatAttachmentIdentity,
  chatMessageMetadataRecord,
  primaryAttachmentFromPersistedMessage,
  uploadedAttachmentsFromPersistedMessage,
} from '../utils/chatMessageAttachments';
...
function xhsVideoGenerationStatusFromMessageMetadata(
  metadata: unknown,
  ...
  if (metadata && typeof metadata === 'object') {
    ...
    try {
      ...
      object = parsed && typeof parsed === 'object'
  ...
  if (!object || String(object.messageKind || '') !== 'xhs-video-generation') return null;
...
function mergeShortcutTaskHints(taskHints: unknown, shortcutTaskHints?: Record<string, unknown>): unknown {
  if (!shortcutTaskHints || Object.keys(shortcutTaskHints).length === 0) return taskHints;
  const base = taskHints && typeof taskHints === 'object' && !Array.isArray(taskHints)
    ? taskHints as Record<string, unknown>
    : {};
  return {
...
export interface ChatComposerModeParameter {
  id: string;
  label: string;
  taskHintKey: string;
...
export interface ChatComposerModeOption {
  ...
  placeholder?: string;
  icon?: 'image' | 'video';
  parameters?: ChatComposerModeParameter[];
  taskHints: Record<string, unknown>;
}

export interface ChatComposerModeGroup {
  label: string;
  options: ChatComposerModeOption[];
}
...
interface ChatProps {
  ...
  onPendingMessageError?: (message: string) => void;
  ...
  composerModeGroup?: ChatComposerModeGroup;
  ...
  clearSignal?: number;
  focusSignal?: number;
  modelSelectionPolicy?: 'preserve-selection' | 'follow-default';
}
...
function applyAttachmentDeliveryMode(
  ...
  if (!attachment) return undefined;
  if (!attachmentParticipatesInChatRuntime(attachment)) {
    return attachment;
  }
...
function attachmentSendBlockReason(attachment: UploadedFileAttachment | undefined): string {
  ...
  if (!attachmentParticipatesInChatRuntime(attachment)) return '';
  if (attachment.intakeStatus && attachment.intakeStatus !== 'ready') {
...
function uniqueUploadedAttachments(attachments: UploadedFileAttachment[]): UploadedFileAttachment[] {
  const seen = new Set<string>();
  return attachments.filter((attachment, index) => {
    const identity = chatAttachmentIdentity(attachment) || `attachment:${index}`;
...
function latestReusableMediaAttachments(messages: Message[]): UploadedFileAttachment[] {
  for (let index = messages.length - 1; index >= 0; index -= 1) {
    const mediaAttachments = uploadedAttachmentsFromMessage(messages[index])
      .filter((attachment) => {
        const kind = attachmentShortcutKind(attachment);
...
function readFixedSessionWarmSnapshot(sessionId: string | null | undefined): FixedSessionWarmSnapshot | null {
  ...
  if (!key) return null;
  const shared = readChatSessionCache<Message, ChatContextUsage>(key);
  if (shared?.messages.length) {
    const run = shared.run;
    const exactRunMessageIndex = run
      ? shared.messages.findIndex((message) => message.id === run.assistantMessageId)
      : -1;
    const fallbackRunMessageIndex = run && exactRunMessageIndex < 0
      ? shared.messages.findLastIndex((message) => message.role === 'ai' && message.isStreaming)
      : -1;
    const messages = run
      ? shared.messages.map((message, index) => (
          index === exactRunMessageIndex || index === fallbackRunMessageIndex
            ? {
                ...message,
                ...
                processingFinishedAt: run.status === 'queued' || run.status === 'running'
                  ? undefined
                  : run.updatedAt,
              }
            : message
...
function writeFixedSessionWarmSnapshot(
  ...
  const cachePatch: { messages?: Message[]; contextUsage?: ChatContextUsage | null } = {};
  if (next.messages !== undefined) cachePatch.messages = next.messages;
  if (next.contextUsage !== undefined) cachePatch.contextUsage = next.contextUsage;
  writeChatSessionCache<Message, ChatContextUsage>(key, cachePatch);
  const shared = readChatSessionCache<Message, ChatContextUsage>(key);
  fixedSessionWarmSnapshots.set(key, {
    messages: next.messages
      ?? (shared?.messages.length ? shared.messages : previous?.messages ?? shared?.messages ?? []),
    contextUsage: next.contextUsage ?? shared?.contextUsage ?? previous?.contextUsage ?? null,
    capturedAt: Date.now(),
  });
}
...
function upsertPersistedToolItem(
  ...
  const uiHint = output.uiHint && typeof output.uiHint === 'object' && !Array.isArray(output.uiHint)
    ? output.uiHint as Record<string, unknown>
    : undefined;
...
function applyPersistedRuntimeEventsToMessages(
  ...
  events.forEach((event, index) => {
    ...
    const assistantMessageId = runtimeText(event.assistantMessageId || payload.assistantMessageId);
    const runId = runtimeText(event.runId || payload.runId);
    const sequence = Number(event.sequence ?? payload.sequence);
    const exactTargetIndex = assistantMessageId
      ? next.findIndex((message) => message.id === assistantMessageId)
      : -1;
    const targetIndex = exactTargetIndex >= 0
      ? exactTargetIndex
      : persistedRuntimeTargetAssistantIndex(next, messageTimes, timestamp);
    ...
    if (eventType === 'runtime:text-delta' || eventType === 'text_delta') {
      ...
      if (messagePhase === 'commentary') {
        ...
        && sequence > (next[targetIndex].runSequence || 0)
      ) {
        next[targetIndex].content += content;
        next[targetIndex].runId = runId || next[targetIndex].runId;
        next[targetIndex].runSequence = sequence;
      }
      return;
    }
    ...
    if (eventType === 'runtime:done') {
      ...
      if (exactTargetIndex >= 0 && runtimeText(payload.content)) {
        next[targetIndex].content = String(payload.content);
      }
      next[targetIndex].isStreaming = false;
      next[targetIndex].runId = runId || next[targetIndex].runId;
      next[targetIndex].runSequence = Number.isFinite(sequence) ? sequence : next[targetIndex].runSequence;
      next[targetIndex].processingFinishedAt = timestamp;
    }
  });
...
export function Chat({
  ...
  clearSignal = 0,
  focusSignal = 0,
  modelSelectionPolicy = 'preserve-selection',
}: ChatProps) {
  ensureChatSessionStoreStarted();
  ...
  const messagesSessionIdRef = useRef<string | null>(fixedSessionId ?? null);
  const [hasNewMessages, setHasNewMessages] = useState(false);
  const [input, setInput] = useState(() => (
    readChatSessionCache<Message, ChatContextUsage>(fixedSessionId)?.draft || ''
  ));
  const [composerDraftHydratedSessionId, setComposerDraftHydratedSessionId] = useState<string | null>(null);
  const [selectedComposerModeId, setSelectedComposerModeId] = useState<string | null>(null);
  ...
  useEffect(() => {
    if (!currentSessionId || messagesSessionIdRef.current !== currentSessionId) return;
    writeChatSessionCache<Message, ChatContextUsage>(currentSessionId, {
      messages,
      contextUsage,
    });
  }, [currentSessionId, messages, contextUsage]);
  ...
  const composerModeDetailsRef = useRef<HTMLDetailsElement>(null);

  const activeComposerMode = useMemo(() => (
    composerModeGroup?.options.find((option) => option.id === selectedComposerModeId) || null
  ), [composerModeGroup, selectedComposerModeId]);
  const activeComposerModeTaskHints = useMemo(() => {
    if (!activeComposerMode) return undefined;
    const parameterHints = Object.fromEntries(
      (activeComposerMode.parameters || []).map((parameter) => {
        const rawValue = composerModeParameterValues[parameter.id] ?? parameter.defaultValue;
        return [
          parameter.taskHintKey,
          parameter.valueType === 'number' ? Number(rawValue) : rawValue,
        ];
      }),
    );
    return {
      ...activeComposerMode.taskHints,
      ...parameterHints,
    };
  }, [activeComposerMode, composerModeParameterValues]);

  useEffect(() => {
    if (!selectedComposerModeId) return;
    if (composerModeGroup?.options.some((option) => option.id === selectedComposerModeId)) return;
    setSelectedComposerModeId(null);
    setComposerModeParameterValues({});
  }, [composerModeGroup, selectedComposerModeId]);

  useEffect(() => {
    if (!focusSignal || !isActive) return undefined;
    const frame = window.requestAnimationFrame(() => {
      composerRef.current?.focus();
      composerRef.current?.syncHeight();
    });
    return () => window.cancelAnimationFrame(frame);
  }, [focusSignal, isActive]);
  ...
  useEffect(() => {
    if (!currentSessionId) {
      setComposerDraftHydratedSessionId(null);
      return;
    }
    const cached = readChatSessionCache<Message, ChatContextUsage>(currentSessionId);
    setInput(cached?.draft || '');
    const cachedAttachments = (cached?.pendingAttachments || []).filter((item): item is UploadedFileAttachment => (
      Boolean(item && typeof item === 'object' && (item as UploadedFileAttachment).type === 'uploaded-file')
    ));
    setPendingAttachments(cachedAttachments);
    setComposerDraftHydratedSessionId(currentSessionId);
  }, [currentSessionId, setPendingAttachments]);

  useEffect(() => {
    if (!canPersistChatComposerDraft({
      sessionId: currentSessionId,
      hydratedSessionId: composerDraftHydratedSessionId,
    })) return;
    writeChatSessionCache<Message, ChatContextUsage>(currentSessionId, {
      draft: input,
      pendingAttachments,
    });
  }, [composerDraftHydratedSessionId, currentSessionId, input, pendingAttachments]);
  ...
  const lastStreamSequenceRef = useRef<Map<string, number>>(new Map());
  ...
  useLayoutEffect(() => {
    ...
    if (lastFixedSessionIdRef.current !== fixedSessionId) {
      ...
      messagesSessionIdRef.current = fixedSessionId;
      localMessageMutationRef.current += 1;
  ...
  const handleMessagesScroll = useCallback(() => {
    ...
    const sessionId = currentSessionIdRef.current;
    if (sessionId) {
      writeChatSessionCache<Message, ChatContextUsage>(sessionId, {
  ...
  const loadChatModelOptions = useCallback(async () => {
    ...
    try {
      ...
      setSelectedChatModelKey((current) => {
        if (modelSelectionPolicy === 'follow-default') {
  ...
  const ensureChatModelConfig = useCallback(async () => {
    if (selectedChatModel && modelSelectionPolicy !== 'follow-default') {
      return {
    ...
    if (resolvedKey) {
      setSelectedChatModelKey((current) => {
        if (modelSelectionPolicy === 'follow-default') return resolvedKey;
        if (current && options.some((item) => item.key === current)) return current;
        return resolvedKey;
      });
    }
  ...
  }, [modelSelectionPolicy, selectedChatModel]);
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      ...
      if (container && shouldAutoScrollRef.current) {
        ...
        setHasNewMessages(false);
        if (currentSessionId) markChatSessionRead(currentSessionId);
  ...
  useEffect(() => {
    if (!isActive) return;
    const cached = readChatSessionCache<Message, ChatContextUsage>(currentSessionId);
    shouldAutoScrollRef.current = cached?.wasNearBottom ?? true;
    setHasNewMessages(Boolean(cached?.unreadCount));
    requestAnimationFrame(() => {
      const container = messagesContainerRef.current;
      if (!container || !cached) return;
      container.scrollTop = cached.wasNearBottom ? container.scrollHeight : cached.scrollTop;
    });
  ...
  const dispatchChatSend = useCallback((payload: {
  ...
  }, callbacks?: { onRejected?: (message: string) => void }) => {
    ...
    schedule(() => {
      ...
      void window.ipcRenderer.chat.send(payload).then((rawReceipt: unknown) => {
        const receipt = rawReceipt as ChatSendReceipt;
        const receiptSessionId = String(receipt?.sessionId || payload.sessionId || '').trim();
        if (!receipt?.accepted) {
          const rejectionMessage = receipt?.error || '对话请求未能启动';
          const removeOptimisticMessages = (current: Message[]) => current.filter((message) => (
            message.id !== payload.optimisticUserMessageId
            && message.id !== payload.optimisticAssistantMessageId
          ));
          const cached = readChatSessionCache<Message, ChatContextUsage>(receiptSessionId);
          if (cached) {
            writeChatSessionCache<Message, ChatContextUsage>(receiptSessionId, {
              messages: removeOptimisticMessages(cached.messages),
            });
          }
          if (!receiptSessionId || currentSessionIdRef.current === receiptSessionId) {
            setErrorNotice(rejectionMessage);
            setMessages(removeOptimisticMessages);
            setIsProcessing(false);
          }
          callbacks?.onRejected?.(rejectionMessage);
          return;
        }
        const reconcile = (current: Message[]) => reconcileOptimisticChatMessageIds(current, {
          optimisticUserMessageId: payload.optimisticUserMessageId,
          optimisticAssistantMessageId: payload.optimisticAssistantMessageId,
        ...
        if (cached) {
          writeChatSessionCache<Message, ChatContextUsage>(receiptSessionId, {
            messages: reconcile(cached.messages),
          });
        }
        if (!receiptSessionId || currentSessionIdRef.current === receiptSessionId) {
  ...
  useEffect(() => {
    ...
    const pendingMessageAttachments = uniqueUploadedAttachments([
    ...
    if (pendingMessage.deliveryMode === 'draft') {
      ...
      setSelectedAssetMentions((pendingMessage.assetReferences || [])
    ...
    const sendPendingMessage = async () => {
      ...
      if (fixedSessionId) {
        ...
        try {
          ...
          onPendingMessageError?.(error instanceof Error ? error.message : '创建创作会话失败');
          onMessageConsumed?.();
          return;
        }
      }
      ...
      if (pendingAttachmentBlockReason) {
        ...
        pendingMessageHandledRef.current = false;
        onPendingMessageError?.(pendingAttachmentBlockReason);
        onMessageConsumed?.();
        return;
      }
      ...
      const pendingAssetReferences = (pendingMessage.assetReferences || [])
        .filter((item) => item.id && item.name)
        .map((item) => ({
          id: item.id,
          name: item.name,
      ...
      dispatchChatSend({
        ...
        optimisticAssistantMessageId: aiPlaceholder.id,
      }, {
        onRejected: (message) => {
          pendingMessageHandledRef.current = false;
          onPendingMessageError?.(message);
          onMessageConsumed?.();
        },
      });
    ...
    sendPendingMessage();
  }, [isActive, pendingMessage, isProcessing, onMessageConsumed, onPendingMessageError, fixedSessionId, currentSessionId, buildPendingAssistantTimeline, dispatchChatSend, ensureChatModelConfig, notifySessionActivity, setPendingAttachment]);
  ...
  const selectSession = async (sessionId: string) => {
    ...
    const warmSnapshot = readFixedSessionWarmSnapshot(sessionId);
    const cachedSession = readChatSessionCache<Message, ChatContextUsage>(sessionId);
    const cachedRun = cachedSession?.run;
    const cachedRuntimeEvents = cachedSession?.runtimeEvents || [];
    if (warmSnapshot) {
      messagesSessionIdRef.current = sessionId;
      setMessages(warmSnapshot.messages);
      if (warmSnapshot.contextUsage) {
        setContextUsage(warmSnapshot.contextUsage);
      }
      debugUi('session:warm_restore', {
        sessionId,
        messageCount: warmSnapshot.messages.length,
      });
    }
    ...
    try {
      ...
      const runtimeEventsRaw = mergeChatRuntimeEvents(
        cachedRuntimeEvents,
        Array.isArray(incrementalRuntimeEventsRaw) ? incrementalRuntimeEventsRaw : [],
      );
      ...
      let uiMessages: Message[] = history.map((msg: any) => {
        ...
        const videoGenerationStatus = role === 'ai'
          ? xhsVideoGenerationStatusFromMessageMetadata(msg.metadata)
          : null;
        const chatRunState = role === 'ai'
          ? parseChatRunMessageMetadata(msg.metadata)
          : null;
        const isPersistedVideoGenerationRunning = videoGenerationStatus === 'running';
        const isPersistedChatRunRunning = chatRunState?.status === 'running' || chatRunState?.status === 'queued';
        const isPersistedRunRunning = isPersistedVideoGenerationRunning || isPersistedChatRunRunning;
        const resolvedProcessingStartedAt = chatRunState?.startedAt ?? processingStartedAt;
        const processingFinishedAt = role === 'ai' && !isPersistedRunRunning
          ? (chatRunState?.finishedAt ?? createdAt)
          : undefined;
        ...
        return {
          id: msg.id,
          clientMessageId: String(metadata.clientMessageId || '').trim() || undefined,
          createdAt,
          runId: chatRunState?.runId,
          runSequence: chatRunState?.sequence,
          ...
          isStreaming: isPersistedRunRunning,
          processingStartedAt: resolvedProcessingStartedAt,
      ...
      let shouldSetProcessing = uiMessages.some((message) => (
      ...
      if (shouldRecoverRuntime && runtimeProcessing) {
        ...
        if (!lastMsg || lastMsg.role !== 'ai') {
          uiMessages.push({
            id: runtimeState.run?.assistantMessageId || `streaming_${Date.now()}`,
            runId: runtimeState.run?.runId,
            runSequence: runtimeState.run?.sequence,
            role: 'ai',
            messageType: 'reply',
  ...
  const clearSession = async () => {
    ...
    try {
      if (isProcessing) {
        const runId = [...messages].reverse().find((message) => message.role === 'ai' && message.isStreaming)?.runId
          || readChatSessionCache<Message, ChatContextUsage>(currentSessionId)?.run?.runId;
        window.ipcRenderer.chat.cancel({ sessionId: currentSessionId, runId });
      }
      await window.ipcRenderer.chat.clearMessages(currentSessionId);
      clearFixedSessionWarmSnapshot(currentSessionId);
      missedChunksRef.current = '';
      flushPendingStreamingUpdates();
      setIsProcessing(false);
      setConfirmRequest(null);
  ...
  const finalizeCancelledResponse = useCallback((source: 'local' | 'runtime') => {
    ...
    flushSync(() => {
      ...
      setCliEscalationRequest(null);
      setErrorNotice(null);
      setMessages((prev) => finalizeCancelledChatMessages(prev));
    });
  }, [blurComposer, debugUi, flushPendingStreamingUpdates, suppressComposerFocus]);
  ...
  const handleCancel = useCallback(() => {
    ...
    finalizeCancelledResponse('local');
  }, [currentSessionId, finalizeCancelledResponse, messages]);
  ...
  useEffect(() => {
    ...
    const handleResponseChunk = (_: unknown, {
      content,
      messagePhase,
      runId,
      sequence,
    }: { content: string; messagePhase?: string; runId?: string; sequence?: number }) => {
      ...
      const now = performance.now();
      if (runId && Number.isFinite(sequence)) {
        const lastSequence = lastStreamSequenceRef.current.get(runId) || 0;
        if (Number(sequence) <= lastSequence) return;
        lastStreamSequenceRef.current.set(runId, Number(sequence));
      }
    ...
    const handleToolEnd = (_: unknown, toolData: {
      callId: string;
      name: string;
      output: { success: boolean; content: string; uiHint?: Record<string, unknown> };
    }) => {
      if (!isActiveRef.current) return;
      setMessages(prev => {
        const lastReplyIndex = findLastAssistantReplyIndex(prev);
        ...
        if (matchedIndex !== -1) {
          ...
          newTimeline[matchedIndex] = {
              ...
              toolData: {
                  ...
                  ...(toolData.output.uiHint ? { uiHint: toolData.output.uiHint } : {}),
              }
          };
        }
    ...
    const handleResponseEnd = (
      ...
      if (responseCancelledRef.current) {
        debugUi('response_end:ignored_after_cancel', {
          sessionId: currentSessionIdRef.current,
          source,
        });
        return;
    ...
    const handleCancelled = () => {
      if (!isActiveRef.current) return;
      finalizeCancelledResponse('runtime');
    };
    ...
    const disposeRuntimeEvents = subscribeRuntimeEventStream({
      ...
      onResponseDelta: ({ content, messagePhase, runId, sequence }) => {
        handleResponseChunk(null, { content, messagePhase, runId, sequence });
      },
  ...
  const sendMessage = async (
    content: string,
    requestedAttachments: UploadedFileAttachment[] = [],
    ...
    shortcutTaskHints?: Record<string, unknown>,
  ) => {
    const attachments = requestedAttachments.length > 0
    ...
    let targetSessionId = resolveChatSendSessionId({
      forceNewSession: fixedSessionDraft,
      fixedSessionId,
    ...
    if (!targetSessionId && onEnsureSessionForSend) {
      ...
      if (!targetSessionId) {
        setInput((current) => current || draftBeforeSessionCreation);
    ...
    if (!seededOptimisticMessages) {
      if (targetSessionId) messagesSessionIdRef.current = targetSessionId;
      localMessageMutationRef.current += 1;
    ...
    dispatchChatSend({
      ...
      taskHints: mergeSkillMentionsIntoTaskHints(
        mergeShortcutTaskHints(
          mergeShortcutTaskHints(fixedSessionTaskHints, activeComposerModeTaskHints),
          shortcutTaskHints,
        ),
        skillMentions,
      ),
  ...
  const reusableVideoAttachments = useMemo(
    () => pendingAttachments.length > 0 ? [] : latestReusableMediaAttachments(messages),
  ...
  const composerModeTriggerClass = activeComposerMode
    ? darkEmbedded
      ? 'border-rose-300/35 bg-rose-300/10 text-rose-100 hover:bg-rose-300/15'
      : 'border-accent-primary/35 bg-accent-primary/10 text-accent-primary hover:bg-accent-primary/15'
    : darkEmbedded
      ? 'border-white/10 bg-white/[0.03] text-white/62 hover:border-white/20 hover:text-white'
      : 'border-border bg-surface-primary text-text-secondary hover:border-accent-primary/30 hover:text-accent-primary';
  ...
  const renderComposerModeControl = () => {
    if (!composerModeGroup || composerModeGroup.options.length === 0) return null;
    return (
      <details ref={composerModeDetailsRef} className="group relative shrink-0 text-left">
        <summary
          className={clsx(
            'inline-flex h-8 cursor-pointer list-none select-none items-center gap-1.5 rounded-full border px-3 text-xs font-medium transition-colors [&::-webkit-details-marker]:hidden',
            composerModeTriggerClass,
          )}
          title={activeComposerMode ? `已开启${activeComposerMode.label}笔记模式` : `选择${composerModeGroup.label}类型`}
        >
          <Sparkles className="h-3.5 w-3.5" strokeWidth={1.8} />
          <span>{composerModeGroup.label}</span>
          {activeComposerMode ? <span className="opacity-75">· {activeComposerMode.label}</span> : null}
          <ChevronDown className="h-3.5 w-3.5 transition-transform group-open:rotate-180" strokeWidth={1.8} />
        </summary>
        <div className={clsx(
          'absolute bottom-full left-0 z-50 mb-2 w-[320px] max-w-[calc(100vw-2rem)] rounded-2xl border p-2 shadow-[0_18px_55px_-18px_rgba(0,0,0,0.35)] backdrop-blur-xl',
          darkEmbedded
            ? 'border-white/10 bg-[#171b20]/95'
            : 'border-border bg-surface-primary/95',
        )}>
          <div className="px-2.5 pb-2 pt-1">
            <div className={clsx('text-xs font-semibold', darkEmbedded ? 'text-white/88' : 'text-text-primary')}>
              选择笔记类型
            </div>
            <div className={clsx('mt-1 text-[11px] leading-4', darkEmbedded ? 'text-white/42' : 'text-text-tertiary')}>
              只需输入创作想法，标题、正文和内容方案会自动补齐。
            </div>
          </div>
          <div className="space-y-1">
            {composerModeGroup.options.map((option) => {
              const selected = option.id === activeComposerMode?.id;
              return (
                <button
                  key={option.id}
                  type="button"
                  onClick={() => {
                    setSelectedComposerModeId(option.id);
                    setComposerModeParameterValues(Object.fromEntries(
                      (option.parameters || []).map((parameter) => [parameter.id, parameter.defaultValue]),
                    ));
                    composerModeDetailsRef.current?.removeAttribute('open');
                ...
                >
                  <span className={clsx(
                    ...
                    selected
                      ? darkEmbedded ? 'bg-rose-300/10' : 'bg-accent-primary/10'
                      : darkEmbedded ? 'bg-white/[0.05]' : 'bg-surface-secondary',
                  )}>
                    {option.icon === 'video'
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    {option.description ? (
                      <span className={clsx('mt-0.5 block text-[11px] leading-4', selected ? 'opacity-70' : 'text-text-tertiary')}>
                        {option.description}
                      </span>
                    ) : null}
                  </span>
                  {selected ? <Check className="h-4 w-4 shrink-0" strokeWidth={2} /> : null}
                </button>
              );
            })}
          </div>
          {activeComposerMode ? (
            <button
              ...
              className={clsx(
                'mt-1 w-full rounded-lg px-3 py-2 text-left text-[11px] transition-colors',
                darkEmbedded ? 'text-white/42 hover:bg-white/[0.05] hover:text-white/72' : 'text-text-tertiary hover:bg-surface-secondary hover:text-text-secondary',
              )}
            >
              退出小红书创作模式
            </button>
  ...
  const renderComposerModeParameters = () => {
    if (!activeComposerMode?.parameters?.length) return null;
    return activeComposerMode.parameters.map((parameter) => (
      <label
        key={parameter.id}
        className={clsx(
      ...
      >
        ...
        <select
          value={composerModeParameterValues[parameter.id] ?? parameter.defaultValue}
          onChange={(event) => setComposerModeParameterValues((current) => ({
            ...current,
            [parameter.id]: event.target.value,
          }))}
          disabled={isProcessing}
          aria-label={`${activeComposerMode.label}${parameter.label}`}
          className={clsx(
            'cursor-pointer appearance-none bg-transparent pr-0.5 text-xs font-semibold outline-none disabled:cursor-not-allowed disabled:opacity-50',
            darkEmbedded ? 'text-white/88' : 'text-text-primary',
          )}
        >
        ...
        <ChevronDown className="h-3 w-3 opacity-55" strokeWidth={1.8} />
      </label>
    ));
  };

  const renderComposerAccessoryRow = (includeShortcuts: boolean) => {
    const showMode = Boolean(composerModeGroup?.options.length);
    const showShortcuts = includeShortcuts && showInlineShortcutChips;
    if (!showMode && !showShortcuts) return null;
    return (
      <div className="flex min-w-0 items-center gap-2 py-1">
        {renderComposerModeControl()}
        {renderComposerModeParameters()}
        {showShortcuts ? (
  ...
  const emptyComposerForm = (
    <div className="space-y-3.5">
      {renderComposerAccessoryRow(false)}
      {renderComposer(
        'empty',
        'empty',
              ...
              <>
                <div className={clsx('flex min-h-0 flex-1 overflow-hidden', hasInlineSidePanel && splitOuterPaddingClass)}>
                  <div className={clsx(
                    ...
                    {/* Messages */}
                    <div ref={messagesContainerRef} onScroll={handleMessagesScroll} className={clsx(hasInlineSidePanel ? 'min-w-0' : 'flex-1', 'relative overflow-y-auto py-4 md:py-5', paneOuterPaddingClass)}>
                      ...
                      {hasNewMessages ? (
                        <button
                          ...
                          className="sticky bottom-3 left-1/2 z-20 flex -translate-x-1/2 items-center gap-1.5 rounded-full border border-border bg-surface-primary/95 px-3 py-1.5 text-xs font-medium text-text-secondary shadow-lg backdrop-blur transition-colors hover:text-text-primary"
                        >
                          <ChevronDown size={14} />
                          有新消息，回到最新
                        </button>
                ...
                <div className={clsx('shrink-0', inputAreaShellClass, splitOuterPaddingClass)}>
                  <div className={clsx('mx-auto space-y-3.5', composerMaxWidthClass, contentWidthClass)}>
                    ...
                    {renderComposerAccessoryRow(true)}
