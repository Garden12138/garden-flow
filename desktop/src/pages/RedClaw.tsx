...
import {
    ...
    REDCLAW_XHS_COMPOSER_MODE_GROUP,
...
export function RedClaw({
    ...
    const [archivedSessionList, setArchivedSessionList] = useState<RedClawHistoryListItem[]>([]);
    ...
    const loadArchivedHistorySessions = useCallback(async () => {
        ...
        try {
            ...
            const contextId = buildRedClawContextId(activeSpaceId);
            const items = (Array.isArray(result) ? result : [])
                .flatMap((session): RedClawHistoryListItem[] => {
                    const metadata = session.metadata && typeof session.metadata === 'object' && !Array.isArray(session.metadata)
                        ? session.metadata as Record<string, unknown>
                        : {};
                    const isExternal = String(metadata.source || '').trim() === 'acp';
                    const isCurrentRedClawContext = metadata.contextId === contextId
                        && metadata.contextType === REDCLAW_CONTEXT_TYPE;
                    if (!isExternal && !isCurrentRedClawContext) return [];
                    return [{
                        ...createContextSessionListItem(session),
                        archived: true,
                        surface: isExternal ? 'external' : 'redclaw',
                        speakerLabel: isExternal ? 'External Agent' : REDCLAW_DISPLAY_NAME,
                    }];
                })
            ...
            if (requestId !== archivedSessionRequestIdRef.current) return;
            console.error('Failed to load archived RedClaw sessions:', error);
    ...
    const archiveUnifiedHistorySession = useCallback(async (session: RedClawHistoryListItem) => {
        ...
        try {
            ...
            activeSessionIdRef.current = nextSessionId;
            setActiveSessionId(nextSessionId);
            writeRedClawLastSessionId(activeSpaceId || 'default', nextSessionId);
            if (!nextSessionId) {
                manualDraftActiveRef.current = true;
                setChatRefreshKey((value) => value + 1);
            }
    ...
    const restoreArchivedHistorySession = useCallback(async (session: RedClawHistoryListItem) => {
        const targetSessionId = String(session?.id || '').trim();
    ...
    const deleteArchivedHistorySession = useCallback(async (session: RedClawHistoryListItem) => {
        const targetSessionId = String(session?.id || '').trim();
        if (!targetSessionId) return;
                            ...
                            <div className="h-full min-h-0 w-full overflow-hidden">
                                {activeAiSurface === 'room' ? (
                                    ...
                                    <Chat
                                        ...
                                        composerModeGroup={activeAiSurface === 'redclaw' && !activeXhsTaskHints
                                            ? REDCLAW_XHS_COMPOSER_MODE_GROUP
                                            : undefined}
