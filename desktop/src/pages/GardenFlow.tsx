...
import { Bot, Image as ImageIcon, Loader2, PanelRight, Plus, Sparkles, SlidersHorizontal, X } from 'lucide-react';
...
import {
    ...
    GARDENFLOW_CONTEXT_TYPE,
    GARDENFLOW_DISPLAY_NAME,
    GARDENFLOW_WELCOME_ICON_SRC,
    GARDENFLOW_XHS_COMPOSER_MODE_GROUP,
    RUNNER_INTERVAL_OPTIONS,
    RUNNER_MAX_AUTOMATION_OPTIONS,
    SCHEDULE_TEMPLATES,
    createGardenFlowComposerShortcuts,
    createGardenFlowComposerShortcutsForContext,
    pickScheduleTemplate,
    scheduleDraftFromTemplate,
    type GardenFlowComposerShortcutInput,
} from './gardenflow/config';
import {
    buildGardenFlowContextId,
    buildGardenFlowInitialContext,
    buildGardenFlowRuntimeMetadata,
    buildGardenFlowSessionTitle,
    createContextSessionListItem,
    normalizeClawHubSlug,
    sortContextSessionItems,
} from './gardenflow/helpers';
import { GardenFlowHistorySidebarSection, type GardenFlowHistoryListItem, type GardenFlowHistorySessionActivity } from './gardenflow/GardenFlowHistoryDrawer';
import { GardenFlowFilePreviewPane } from './gardenflow/GardenFlowFilePreviewPane';
import {
    isGardenFlowOnboardingCompleted,
    type GardenFlowOnboardingState,
} from './gardenflow/onboardingState';
import { GardenFlowSidebar } from './gardenflow/GardenFlowSidebar';
import type {
    RunnerScheduledTask,
    RunnerStatus,
    ScheduleDraft,
    SidebarTab,
} from './gardenflow/types';
...
interface FilePreviewResolveResult {
    ...
    uri?: string | null;
    version?: number | null;
    xhsNote?: ChatMessageLinkTarget['xhsNote'];
}

type GardenFlowAiSurface = 'gardenflow' | 'advisor' | 'room';
...
const GARDENFLOW_AI_SURFACE_STORAGE_KEY = 'gardenflow:gardenflow-ai-surface:v1';
const GARDENFLOW_HIDDEN_EXTERNAL_SESSION_IDS_STORAGE_KEY = 'gardenflow:gardenflow:hidden-external-session-ids:v1';
const GARDENFLOW_DRAFT_SESSION_SENTINEL = '__draft__';
function readInitialGardenFlowAiSurface(): GardenFlowAiSurface {
    if (typeof window === 'undefined') return 'gardenflow';
    const saved = String(window.localStorage.getItem(GARDENFLOW_AI_SURFACE_STORAGE_KEY) || '').trim();
    return saved === 'advisor' || saved === 'room' ? saved : 'gardenflow';
}

function readHiddenExternalSessionIds(): string[] {
    if (typeof window === 'undefined') return [];
    try {
        const raw = window.localStorage.getItem(GARDENFLOW_HIDDEN_EXTERNAL_SESSION_IDS_STORAGE_KEY);
        const parsed = raw ? JSON.parse(raw) : [];
        return Array.isArray(parsed)
            ? Array.from(new Set(parsed.map((item) => String(item || '').trim()).filter(Boolean)))
            : [];
    } catch (error) {
        console.warn('Failed to read hidden external GardenFlow sessions:', error);
        return [];
    }
}
...
function writeHiddenExternalSessionIds(sessionIds: string[]): void {
    ...
    try {
        ...
        window.localStorage.setItem(GARDENFLOW_HIDDEN_EXTERNAL_SESSION_IDS_STORAGE_KEY, JSON.stringify(normalized));
    } catch (error) {
        console.warn('Failed to write hidden external GardenFlow sessions:', error);
    }
}
...
function teamRoomFromSession(session: TeamWorkbenchSession): GardenFlowTeamRoom {
...
function advisorGardenFlowOrder(advisor: AdvisorProfile, index: number): number {
...
interface GardenFlowProps {
    ...
    composerShortcutInputs?: GardenFlowComposerShortcutInput[];
    welcomeShortcutInputs?: GardenFlowComposerShortcutInput[];
...
interface GardenFlowSpaceListPayload {
    activeSpaceId: string;
    spaces: Array<{ id: string; name: string }>;
}

const normalizeGardenFlowSpaceListPayload = (value: unknown): GardenFlowSpaceListPayload => {
...
function gardenFlowLastSessionStorageKey(spaceId: string): string {
    const normalized = String(spaceId || 'default').trim() || 'default';
    return `gardenflow:lastSession:${normalized}`;
}

function readGardenFlowLastSessionId(spaceId: string): string | null {
    if (typeof window === 'undefined') return null;
    const raw = localStorage.getItem(gardenFlowLastSessionStorageKey(spaceId));
    if (raw === GARDENFLOW_DRAFT_SESSION_SENTINEL) return null;
    const sessionId = String(raw || '').trim();
    return sessionId || null;
}

function writeGardenFlowLastSessionId(spaceId: string, sessionId: string | null): void {
    if (typeof window === 'undefined') return;
    const normalized = String(sessionId || '').trim();
    localStorage.setItem(gardenFlowLastSessionStorageKey(spaceId), normalized || GARDENFLOW_DRAFT_SESSION_SENTINEL);
}
...
function getGardenFlowImageJobExpectedCount(job: MediaJobProjection): number {
...
function getGardenFlowImageJobCompletedCount(job: MediaJobProjection): number {
    const resultProgress = job.result && typeof job.result === 'object'
        ? job.result.progress as Record<string, unknown> | undefined
        : undefined;
...
function getGardenFlowImageJobTitle(job: MediaJobProjection): string {
    const title = String(job.request?.title || '').trim();
    if (title) return title;
    const expected = getGardenFlowImageJobExpectedCount(job);
    return expected > 1 ? `批量生图 · ${expected} 张` : '图片生成';
}

function getGardenFlowImageJobOverallProgress(job: MediaJobProjection): number {
    const expected = getGardenFlowImageJobExpectedCount(job);
    const completed = Math.min(getGardenFlowImageJobCompletedCount(job), expected);
...
function GardenFlowImageGenerationPlaceholder({
    index,
    job,
}: {
    index: number;
    ...
    const expected = getGardenFlowImageJobExpectedCount(job);
    const completed = Math.min(getGardenFlowImageJobCompletedCount(job), expected);
    const overallProgress = getGardenFlowImageJobOverallProgress(job);
...
function GardenFlowImageGenerationProgressPanel({
    jobs,
}: {
    ...
    return (
        <div className="space-y-3">
            {jobs.map((job) => {
                const expected = getGardenFlowImageJobExpectedCount(job);
                const completed = Math.min(getGardenFlowImageJobCompletedCount(job), expected);
                const progress = getGardenFlowImageJobOverallProgress(job);
                    ...
                    <div key={job.jobId} className="py-1">
                        <div className="mb-3 flex items-center justify-between gap-3">
                            <div className="min-w-0">
                                <div className="truncate text-[12px] font-semibold text-text-primary">
                                    {getGardenFlowImageJobTitle(job)}
                                </div>
                        ...
                        <div className="grid max-w-[620px] grid-cols-5 gap-2.5">
                            {Array.from({ length: expected }).map((_, index) => (
                                <GardenFlowImageGenerationPlaceholder
                                    key={`${job.jobId}-${index}`}
                                    index={index}
                                    job={job}
                                />
...
function GardenFlowAiSwitchBar({
    activeSurface,
    advisors,
    selectedAdvisorId,
    onSelectGardenFlow,
    ...
    activeSurface: GardenFlowAiSurface;
    advisors: AdvisorProfile[];
    selectedAdvisorId: string | null;
    onSelectGardenFlow: () => void;
    onSelectAdvisor: (advisorId: string) => void;
    onCreateAdvisor: () => void;
}) {
    const visibleAdvisors = advisors
        .map((advisor, index) => ({ advisor, index }))
        .filter(({ advisor }) => advisor.gardenflowVisible !== false)
        .sort((left, right) => {
            const orderDelta = advisorGardenFlowOrder(left.advisor, left.index) - advisorGardenFlowOrder(right.advisor, right.index);
...
export function GardenFlow({
    ...
    const debugUi = useCallback((event: string, extra?: Record<string, unknown>) => {
        ...
        console.debug(`[ui][gardenflow] ${event}`, extra || {});
    }, []);
    ...
    const [archivedSessionList, setArchivedSessionList] = useState<GardenFlowHistoryListItem[]>([]);
    ...
    const [sessionActivityById, setSessionActivityById] = useState<Record<string, GardenFlowHistorySessionActivity>>({});
    ...
    const [activeAiSurface, setActiveAiSurface] = useState<GardenFlowAiSurface>(readInitialGardenFlowAiSurface);
    const [teamRooms, setTeamRooms] = useState<GardenFlowTeamRoom[]>([]);
    ...
    const [isGardenFlowChatExecuting, setIsGardenFlowChatExecuting] = useState(false);
    const chatSessionActivities = useChatSessionActivities();

    const activeXhsTaskHints = useMemo(() => previewTarget?.artifactType === 'xiaohongshu-note'
        ? {
            platform: 'xiaohongshu',
            taskType: 'direct_write',
            artifactType: 'xiaohongshu-note',
            activeXhsNotePath: previewTarget.projectPath || previewTarget.relativePath,
            activeXhsNoteUri: previewTarget.uri || previewTarget.href,
            xhsNoteType: previewTarget.noteType,
        }
        : undefined, [previewTarget]);
    ...
    const [onboardingState, setOnboardingState] = useState<GardenFlowOnboardingState | undefined>(undefined);
    ...
    const composerShortcuts = useMemo(
        () => composerShortcutInputs
            ? createGardenFlowComposerShortcuts(composerShortcutInputs)
            : createGardenFlowComposerShortcutsForContext,
        [composerShortcutInputs],
    );
    const welcomeShortcuts = useMemo(
        () => welcomeShortcutInputs
            ? createGardenFlowComposerShortcuts(welcomeShortcutInputs)
            : createGardenFlowComposerShortcutsForContext,
        [welcomeShortcutInputs],
    );
    ...
    const archivedSessionRequestIdRef = useRef(0);
    ...
    const setHistorySessionUnread = useCallback((sessionId: string | null | undefined, unread: boolean) => {
        ...
        void window.ipcRenderer.chat.setSessionUnread({ sessionId: safeSessionId, unread }).catch((error) => {
            console.error('Failed to update GardenFlow session unread state:', error);
        });
    ...
    useEffect(() => {
        setSessionActivityById((previous) => {
            const next = { ...previous };
            for (const activity of chatSessionActivities) {
                const isCurrentlyOpen = isActiveRef.current && activeSessionIdRef.current === activity.sessionId;
                if (activity.status === 'queued' || activity.status === 'running') {
                    next[activity.sessionId] = 'running';
                } else if (activity.status === 'failed') {
                    next[activity.sessionId] = 'failed';
                } else if (activity.status === 'completed' && activity.unreadCount > 0 && !isCurrentlyOpen) {
                    next[activity.sessionId] = 'unread-complete';
                } else if (isCurrentlyOpen || activity.unreadCount === 0) {
                    delete next[activity.sessionId];
                }
            }
            return next;
        });
    ...
    useEffect(() => {
        ...
        writeGardenFlowLastSessionId(activeSpaceId, activeSessionId);
    }, [activeSessionId, activeSpaceId]);

    useEffect(() => {
        if (typeof window !== 'undefined') {
            localStorage.setItem(GARDENFLOW_AI_SURFACE_STORAGE_KEY, activeAiSurface);
        }
    }, [activeAiSurface]);
    ...
    useEffect(() => {
        ...
        const loadTeamData = async () => {
            try {
                ...
                if (cancelled) return;
                console.error('Failed to load GardenFlow team surfaces:', error);
            }
        };
        ...
        window.addEventListener('gardenflow:team-settings-changed', handleTeamSettingsChanged);
        window.ipcRenderer.teamRuntime.onEvent(handleTeamRuntimeEvent);
        return () => {
            cancelled = true;
            window.removeEventListener('gardenflow:team-settings-changed', handleTeamSettingsChanged);
            window.ipcRenderer.teamRuntime.offEvent(handleTeamRuntimeEvent);
        };
    }, [shouldLoadHistory]);

    useEffect(() => {
        onExecutionStateChange?.(isGardenFlowChatExecuting);
    }, [isGardenFlowChatExecuting, onExecutionStateChange]);
    ...
    const loadArchivedHistorySessions = useCallback(async () => {
        const requestId = ++archivedSessionRequestIdRef.current;
        setArchivedSessionsLoading(true);
        try {
            const result = await window.ipcRenderer.chat.listArchivedSessions() as ChatSession[];
            const contextId = buildGardenFlowContextId(activeSpaceId);
            const items = (Array.isArray(result) ? result : [])
                .flatMap((session): GardenFlowHistoryListItem[] => {
                    const metadata = session.metadata && typeof session.metadata === 'object' && !Array.isArray(session.metadata)
                        ? session.metadata as Record<string, unknown>
                        : {};
                    const isExternal = String(metadata.source || '').trim() === 'acp';
                    const isCurrentGardenFlowContext = metadata.contextId === contextId
                        && metadata.contextType === GARDENFLOW_CONTEXT_TYPE;
                    if (!isExternal && !isCurrentGardenFlowContext) return [];
                    return [{
                        ...createContextSessionListItem(session),
                        archived: true,
                        surface: isExternal ? 'external' : 'gardenflow',
                        speakerLabel: isExternal ? 'External Agent' : GARDENFLOW_DISPLAY_NAME,
                    }];
                })
                .sort((left, right) => sessionUpdatedAtMs(right) - sessionUpdatedAtMs(left));
            if (requestId !== archivedSessionRequestIdRef.current) return;
            setArchivedSessionList(items);
        } catch (error) {
            if (requestId !== archivedSessionRequestIdRef.current) return;
            console.error('Failed to load archived GardenFlow sessions:', error);
            setChatActionMessage(error instanceof Error ? error.message : '加载已归档对话失败');
        } finally {
            if (requestId === archivedSessionRequestIdRef.current) {
    ...
    const unifiedHistorySessions = useMemo<GardenFlowHistoryListItem[]>(() => (
        [
            ...sessionList.map((session): GardenFlowHistoryListItem => ({
                ...session,
                surface: 'gardenflow',
                speakerLabel: GARDENFLOW_DISPLAY_NAME,
            })),
            ...externalAgentSessions.map((session): GardenFlowHistoryListItem => ({
                ...session,
                surface: 'external',
    ...
    useEffect(() => {
        ...
        const prepareFreshSession = async () => {
            ...
            const contextId = buildGardenFlowContextId(nextActiveSpaceId);
            const sessionTitle = defaultSessionTitleFromPendingMessage(pendingMessage) || buildGardenFlowSessionTitle(nextSpaceName);
            try {
                const session = await uiMeasure('gardenflow', 'sessions:create_for_pending_message', async () => (
                    window.ipcRenderer.chat.createContextSessionGuarded<ChatSession>({
                        contextId,
                        contextType: GARDENFLOW_CONTEXT_TYPE,
                        title: sessionTitle,
                        initialContext: buildGardenFlowInitialContext(nextSpaceName, nextActiveSpaceId),
                        metadata: buildGardenFlowRuntimeMetadata(nextActiveSpaceId, nextSpaceName),
                    })
                ...
                console.error('Failed to create GardenFlow context session for pending message:', error);
                if (!cancelled) {
    ...
    const loadContextSessions = useCallback(async (
        ...
        try {
            const contextId = buildGardenFlowContextId(nextActiveSpaceId);
            const listResult = await uiMeasure('gardenflow', 'sessions:list_context', async () => (
            ...
            let items = sortContextSessionItems(listResult.filter((item) => {
                const metadata = item.metadata && typeof item.metadata === 'object' && !Array.isArray(item.metadata)
                    ? item.metadata as Record<string, unknown>
                    : {};
            ...
            const rememberedSessionId = readGardenFlowLastSessionId(nextActiveSpaceId);
            ...
            if (items.length === 0 && shouldCreateIfEmpty) {
                const created = await uiMeasure('gardenflow', 'sessions:create_context', async () => (
                    window.ipcRenderer.chat.createContextSessionGuarded<ChatSession>({
                        contextId,
                        contextType: GARDENFLOW_CONTEXT_TYPE,
                        title: buildGardenFlowSessionTitle(nextSpaceName),
                        initialContext: buildGardenFlowInitialContext(nextSpaceName, nextActiveSpaceId),
                        metadata: buildGardenFlowRuntimeMetadata(nextActiveSpaceId, nextSpaceName),
                    })
                ), { activeSpaceId: nextActiveSpaceId, spaceName: nextSpaceName });
            ...
            console.error('Failed to load GardenFlow context sessions:', error);
            if (!hasSessionSnapshotRef.current) {
                setSessionList([]);
                sessionListRef.current = [];
                activeSessionIdRef.current = null;
    ...
    const initSession = useCallback(async () => {
        ...
        try {
            const spaceInfo = await uiMeasure('gardenflow', 'init_session:spaces', async () => (
                window.ipcRenderer.spaces.list()
            )) as GardenFlowSpaceListPayload;
            const normalizedSpaceInfo = normalizeGardenFlowSpaceListPayload(spaceInfo);
            ...
            console.error('Failed to initialize GardenFlow session list:', error);
            if (!hasSessionSnapshotRef.current) {
                setSessionList([]);
                setActiveSessionId(null);
                setIsSessionLoading(false);
    ...
    const loadRunnerStatus = useCallback(async (syncForm = false) => {
        ...
        try {
            const status = await uiMeasure('gardenflow', 'load_runner_status', async () => (
                window.ipcRenderer.gardenflowRunner.getStatus()
    ...
    const loadSkills = useCallback(async () => {
        ...
        try {
            const list = await uiMeasure('gardenflow', 'load_skills', async () => (
    ...
    const loadOnboardingBundle = useCallback(async () => {
        ...
        try {
            const bundle = await uiMeasure('gardenflow', 'load_onboarding_bundle', async () => (
                window.ipcRenderer.gardenflowProfile.getBundle()
            )) as {
            ...
            console.error('Failed to load GardenFlow onboarding bundle:', error);
    ...
    }, [loadOnboardingBundle, loadSkills, gardenflowOnboardingVersion]);
    ...
    useEffect(() => {
        ...
        window.ipcRenderer.gardenflowRunner.onStatus(onRunnerStatus);
        return () => {
            window.ipcRenderer.gardenflowRunner.offStatus(onRunnerStatus);
        };
    }, [isActive]);
    ...
    const createNewSession = useCallback(async (
        ...
        const nextSpaceName = activeSpaceName || nextActiveSpaceId;
        const contextId = buildGardenFlowContextId(nextActiveSpaceId);
        const sessionTitle = String(defaultTitle || '').trim() || buildGardenFlowSessionTitle(nextSpaceName);
        setHistoryLoading(true);
        try {
            const session = await uiMeasure('gardenflow', 'sessions:create_manual', async () => (
                window.ipcRenderer.chat.createContextSessionGuarded<ChatSession>({
                    contextId,
                    contextType: GARDENFLOW_CONTEXT_TYPE,
                    title: sessionTitle,
            ...
            console.error('Failed to create GardenFlow context session:', error);
    ...
    const startNewDraftSession = useCallback(() => {
        ...
        setHistoryLoading(false);
        setPreviewTarget(null);
        writeGardenFlowLastSessionId(activeSpaceId || 'default', null);
    ...
    const renameUnifiedHistorySession = useCallback(async (session: GardenFlowHistoryListItem, title: string) => {
    ...
    const switchHistorySession = useCallback((session: GardenFlowHistoryListItem) => {
    ...
    const ensureActiveChatSessionForSend = useCallback(async (
        defaultTitle?: string,
        options?: { onCreated?: (sessionId: string) => void; forceCreate?: boolean },
    ): Promise<string | null> => {
        if (!options?.forceCreate && !manualDraftActiveRef.current && activeSessionIdRef.current) {
    ...
    const createAdvisorFromGardenFlow = useCallback(() => {
        setAdvisorCreateModalOpen(true);
    }, []);

    const saveAdvisorFromGardenFlow = useCallback(async (
        ...
        try {
            ...
            console.error('Failed to create advisor from GardenFlow:', error);
            setChatActionMessage(error instanceof Error ? error.message : '创建成员失败');
        }
    }, []);

    const createRoomFromGardenFlow = useCallback(() => {
    ...
    const submitRoomCreate = useCallback(async () => {
        ...
        try {
            ...
            console.error('Failed to create GardenFlow room:', error);
    ...
    const archiveUnifiedHistorySession = useCallback(async (session: GardenFlowHistoryListItem) => {
        const targetSessionId = String(session?.id || '').trim();
        if (!targetSessionId || (session.surface !== 'gardenflow' && session.surface !== 'external')) return;
        const isExternalSession = session.surface === 'external';
        try {
            const result = await window.ipcRenderer.chat.archiveSession(targetSessionId) as { success?: boolean; error?: string };
            if (!result?.success) {
                throw new Error(result?.error || '归档对话失败');
            }
            sessionRequestIdRef.current += 1;
            ...
            void loadArchivedHistorySessions();
            if (activeSessionIdRef.current !== targetSessionId) return;
            const nextSessionId = remaining[0]?.id || null;
            activeSessionIdRef.current = nextSessionId;
            setActiveSessionId(nextSessionId);
            writeGardenFlowLastSessionId(activeSpaceId || 'default', nextSessionId);
            if (!nextSessionId) {
                manualDraftActiveRef.current = true;
                setChatRefreshKey((value) => value + 1);
            }
        } catch (error) {
            console.error('Failed to archive GardenFlow session:', error);
            setChatActionMessage(error instanceof Error ? error.message : '归档对话失败');
        }
    }, [activeSpaceId, loadArchivedHistorySessions]);

    const restoreArchivedHistorySession = useCallback(async (session: GardenFlowHistoryListItem) => {
        const targetSessionId = String(session?.id || '').trim();
        if (!targetSessionId) return;
        const result = await window.ipcRenderer.chat.unarchiveSession(targetSessionId) as { success?: boolean; error?: string };
        if (!result?.success) {
            throw new Error(result?.error || '恢复对话失败');
        }
        setArchivedSessionList((prev) => prev.filter((item) => item.id !== targetSessionId));
        if (session.surface === 'external') {
            setHiddenExternalSessionIds((prev) => {
                const next = prev.filter((id) => id !== targetSessionId);
                writeHiddenExternalSessionIds(next);
                return next;
            });
            const metadata = session.metadata && typeof session.metadata === 'object' && !Array.isArray(session.metadata)
                ? { ...(session.metadata as Record<string, unknown>), archived: false, archivedAt: null }
                : { archived: false, archivedAt: null };
            const restoredSession: ContextChatSessionListItem = {
                ...session,
                archived: false,
                metadata,
            };
            setExternalAgentSessions((prev) => sortContextSessionItems([
                restoredSession,
                ...prev.filter((item) => item.id !== targetSessionId),
            ]));
            return;
        }
        await loadContextSessions(activeSpaceId, activeSpaceName, { silent: true });
    }, [activeSpaceId, activeSpaceName, loadContextSessions]);

    const deleteArchivedHistorySession = useCallback(async (session: GardenFlowHistoryListItem) => {
        const targetSessionId = String(session?.id || '').trim();
        if (!targetSessionId) return;
        const result = await window.ipcRenderer.chat.deleteSession(targetSessionId) as { success?: boolean; error?: string };
        if (!result?.success) {
    ...
    const deleteRoomFromGardenFlow = useCallback(async (room: GardenFlowTeamRoom) => {
        if (!room?.id) return;
        try {
            ...
            console.error('Failed to delete GardenFlow room:', error);
            setChatActionMessage(error instanceof Error ? error.message : '删除团队失败');
        }
    }, [activeAiSurface, selectedRoomId]);

    const compactGardenFlowContext = useCallback(async () => {
        if (!activeSessionId || chatActionLoading) return;
        uiTraceInteraction('gardenflow', 'compact_context', { sessionId: activeSessionId });
        setChatActionLoading('compact');
        try {
            const result = await uiMeasure('gardenflow', 'compact_context:invoke', async () => (
            ...
            console.error('Failed to compact GardenFlow context:', error);
    ...
    const toggleRunner = useCallback(async () => {
        ...
        try {
            if (runnerStatus.enabled) {
                await window.ipcRenderer.gardenflowRunner.stop();
                setAutomationMessage('后台任务已暂停');
            } else {
                await window.ipcRenderer.gardenflowRunner.start({
    ...
    const runRunnerNow = useCallback(async () => {
        setAutomationLoading(true);
        try {
            await window.ipcRenderer.gardenflowRunner.runNow({});
    ...
    const saveRunnerConfig = useCallback(async () => {
        setAutomationLoading(true);
        try {
            await window.ipcRenderer.gardenflowRunner.setConfig({
                intervalMinutes: runnerIntervalMinutes,
                maxAutomationPerTick: runnerMaxAutomationPerTick,
            });
    ...
    const addScheduleTask = useCallback(async () => {
        ...
        try {
            const result = await window.ipcRenderer.gardenflowRunner.addScheduled({
    ...
    const toggleScheduleTask = useCallback(async (task: RunnerScheduledTask) => {
        setAutomationLoading(true);
        try {
            const result = await window.ipcRenderer.gardenflowRunner.setScheduledEnabled({
                taskId: task.id,
    ...
    const runScheduleTaskNow = useCallback(async (taskId: string) => {
        setAutomationLoading(true);
        try {
            const result = await window.ipcRenderer.gardenflowRunner.runScheduledNow({ taskId });
            if (!result?.success) {
    ...
    const removeScheduleTask = useCallback(async (taskId: string) => {
        setAutomationLoading(true);
        try {
            const result = await window.ipcRenderer.gardenflowRunner.removeScheduled({ taskId });
            if (!result?.success) {
    ...
    const onboardingCompleted = useMemo(
        () => onboardingState !== undefined && isGardenFlowOnboardingCompleted(onboardingState),
        [onboardingState],
    );
            ...
            try {
                ...
                setPreviewTarget((current) => current?.href === target.href
                    ? {
                        ...
                        artifactType: result.artifactType || undefined,
                        noteType: result.noteType || undefined,
                        projectPath: String(result.projectPath || '').trim() || undefined,
                        relativePath: String(result.relativePath || '').trim() || undefined,
                        uri: String(result.uri || '').trim() || undefined,
                        version: typeof result.version === 'number' ? result.version : undefined,
                        xhsNote: result.xhsNote,
                        error: result.exists === false ? '文件不存在或已被移动' : undefined,
                    }
                    : current);
            } catch (error) {
                console.error('Failed to resolve GardenFlow preview target:', error);
                setPreviewTarget((current) => current?.href === target.href
    ...
    const handleOpenManuscript = useCallback((filePath: string) => {
        ...
        handlePreviewLink({
            ...
            sourceMessageId: 'gardenflow-manuscript-list',
        });
    ...
    const handleOpenPreviewExternal = useCallback(async (target: ChatMessageLinkTarget) => {
        ...
        try {
            ...
            console.error('Failed to open GardenFlow preview target:', error);
    ...
    const handleRevealPreviewInFolder = useCallback(async (target: ChatMessageLinkTarget) => {
        ...
        try {
            ...
            console.error('Failed to reveal GardenFlow preview target:', error);
    ...
    useEffect(() => {
        ...
        onGlobalSidebarContentChange(
            <GardenFlowHistorySidebarSection
                ...
                onCreateRoom={createRoomFromGardenFlow}
                onSwitchRoom={switchRoom}
                onDeleteRoom={(room) => void deleteRoomFromGardenFlow(room)}
                ...
                onLoadArchivedSessions={loadArchivedHistorySessions}
                onRestoreArchivedSession={restoreArchivedHistorySession}
                onDeleteArchivedSession={deleteArchivedHistorySession}
        ...
        createRoomFromGardenFlow,
        deleteArchivedHistorySession,
        deleteRoomFromGardenFlow,
        historyLoading,
        handleOpenManuscript,
        activeManuscriptPath,
        loadArchivedHistorySessions,
        onGlobalSidebarContentChange,
        renameUnifiedHistorySession,
        restoreArchivedHistorySession,
        selectedRoomId,
        setHistorySessionUnread,
        sessionActivityById,
    ...
    return (
        <div className="workbench-compose h-full min-h-0 flex overflow-hidden">
                ...
                {isSessionLoading ? (
                    <div className="h-full flex items-center justify-center">
                        <div className="flex flex-col items-center gap-3 text-text-tertiary">
                            ...
                            <span className="text-xs">正在初始化 {GARDENFLOW_DISPLAY_NAME}...</span>
                        </div>
                    </div>
                ) : (
                        ...
                        <div className="relative min-h-0 flex-1 overflow-hidden">
                            <div className="h-full min-h-0 w-full overflow-hidden">
                                {activeAiSurface === 'room' ? (
                                    ...
                                    <Chat
                                        isActive={isActive}
                                        onExecutionStateChange={setIsGardenFlowChatExecuting}
                                        key={`gardenflow:${activeAiSurface}:${activeAiSurface === 'advisor' ? selectedAdvisorId || 'advisor' : 'gardenflow'}:${activeChatSessionId || 'draft'}:${chatRefreshKey}`}
                                        fixedSessionId={activeChatSessionId}
                                        fixedSessionDraft={!activeChatSessionId}
                                        fixedSessionTaskHints={activeXhsTaskHints}
                                        onEnsureSessionForSend={ensureActiveChatSessionForSend}
                                        pendingMessage={activeAiSurface === 'gardenflow' ? resolvedPendingMessage : null}
                                        onMessageConsumed={onPendingMessageConsumed}
                                        showClearButton={false}
                                        fixedSessionBannerText=""
                                        showWelcomeShortcuts={true}
                                        showComposerShortcuts={true}
                                        composerModeGroup={activeAiSurface === 'gardenflow' && !activeXhsTaskHints
                                            ? GARDENFLOW_XHS_COMPOSER_MODE_GROUP
                                            : undefined}
                                        ...
                                        welcomeIconAccessory={(
                                            <GardenFlowAiSwitchBar
                                                activeSurface={activeAiSurface}
                                                advisors={advisors}
                                                selectedAdvisorId={selectedAdvisorId}
                                                onSelectGardenFlow={() => setActiveAiSurface('gardenflow')}
                                                onSelectAdvisor={switchAdvisor}
                                                onCreateAdvisor={createAdvisorFromGardenFlow}
                                            />
                                        )}
                            ...
                            {advisorCreateModalOpen && (
                                <AdvisorModal
                                    advisor={null}
                                    defaultMode="manual"
                                    onSave={saveAdvisorFromGardenFlow}
                                    ...
                                    >
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <div className="max-h-56 overflow-y-auto rounded-xl border border-border/80 p-1 custom-scrollbar">
                                                {advisors.filter((advisor) => advisor.gardenflowVisible !== false).length === 0 ? (
                                                    <div className="px-3 py-4 text-center text-xs text-text-tertiary">暂无可选成员</div>
                                                ) : advisors.filter((advisor) => advisor.gardenflowVisible !== false).map((advisor) => {
            ...
            {previewPaneVisible && previewTarget ? (
                <aside
                    className={clsx(
                        'gardenflow-preview-sidebar min-h-0 min-w-[420px] flex-1 overflow-hidden bg-surface-primary',
                        isPreviewSidebarClosing ? 'gardenflow-preview-sidebar--closing' : 'gardenflow-preview-sidebar--open'
                    )}
                >
                    <GardenFlowFilePreviewPane
