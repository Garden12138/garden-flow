...
function mergeAttachmentLists(
  primary: UploadedFileAttachment[],
  secondary: UploadedFileAttachment[],
): UploadedFileAttachment[] {
  const merged: UploadedFileAttachment[] = [];
...
export function useChatAttachments({
  ...
  const [hydratedAttachmentDraftScopeId, setHydratedAttachmentDraftScopeId] = useState('');
  const [isAttachmentUploading, setIsAttachmentUploading] = useState(false);
  const [isFileDragActive, setIsFileDragActive] = useState(false);
  const pendingAttachmentsRef = useRef<UploadedFileAttachment[]>([]);
  ...
  const clearPendingAttachment = useCallback(() => {
    ...
    if (attachments.length > 0) {
      void window.ipcRenderer.chat.discardAttachments({
        sessionId: currentSessionId || undefined,
        attachments,
      });
    }
    focusComposer();
  }, [currentSessionId, focusComposer]);
  ...
  const removePendingAttachment = useCallback((attachment: UploadedFileAttachment) => {
    const key = attachmentIdentity(attachment);
    setPendingAttachments((current) => {
      const next = current.filter((item) => attachmentIdentity(item) !== key);
      pendingAttachmentsRef.current = next;
      return next;
    });
    void window.ipcRenderer.chat.discardAttachments({
  ...
  useEffect(() => {
    setHydratedAttachmentDraftScopeId('');
    if (!isPersistentAttachmentDraftScope(attachmentDraftScopeId)) {
      clearAttachmentDraft('chat', attachmentDraftScopeId);
      setIsAttachmentUploading(false);
      setPendingAttachments([]);
      setHydratedAttachmentDraftScopeId(attachmentDraftScopeId);
      return;
    }
    const drafts = loadAttachmentDrafts('chat', attachmentDraftScopeId);
    pendingAttachmentsRef.current = drafts;
    setPendingAttachments(drafts);
    setHydratedAttachmentDraftScopeId(attachmentDraftScopeId);
    if (!currentSessionId || !window.ipcRenderer?.chat?.getStagedAttachments) return;
    let disposed = false;
    void window.ipcRenderer.chat.getStagedAttachments({ sessionId: currentSessionId }).then((result) => {
      if (disposed || !result?.success || !Array.isArray(result.attachments)) return;
      const staged = result.attachments.filter((item): item is UploadedFileAttachment => (
        Boolean(item)
        && typeof item === 'object'
        && !Array.isArray(item)
        && String((item as UploadedFileAttachment).type || '') === 'uploaded-file'
      ));
      const next = mergeAttachmentLists(pendingAttachmentsRef.current, staged);
      pendingAttachmentsRef.current = next;
  ...
  }, [attachmentDraftScopeId, currentSessionId]);

  useEffect(() => {
    if (!isPersistentAttachmentDraftScope(attachmentDraftScopeId)) return;
    if (hydratedAttachmentDraftScopeId !== attachmentDraftScopeId) return;
    pendingAttachmentsRef.current = pendingAttachments;
    saveAttachmentDrafts('chat', attachmentDraftScopeId, pendingAttachments);
  }, [attachmentDraftScopeId, hydratedAttachmentDraftScopeId, pendingAttachments]);
  ...
  const replacePendingAttachment = useCallback((attachment: UploadedFileAttachment | null) => {
    const next = attachment ? [attachment] : [];
    pendingAttachmentsRef.current = next;
    setPendingAttachments(next);
  }, []);
  const replacePendingAttachments = useCallback((attachments: UploadedFileAttachment[]) => {
  ...
  return {
    ...
    setPendingAttachment: replacePendingAttachment,
    setPendingAttachments: replacePendingAttachments,
  };
}
