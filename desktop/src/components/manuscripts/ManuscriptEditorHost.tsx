...
const XhsNoteWorkbench = lazy(async () => ({
    default: (await import('./XhsNoteWorkbench')).XhsNoteWorkbench,
}));
...
export function ManuscriptEditorHost({ filePath, onNavigateToGenerationStudio, isActive = false, onClose, onImmersiveModeChange }: ManuscriptEditorHostProps) {
    ...
    const [xhsEditorTarget, setXhsEditorTarget] = useState<ChatMessageLinkTarget | null>(null);
    ...
    const [xhsChatFocusSignal, setXhsChatFocusSignal] = useState(0);
    ...
    const tryOpenStructuredXhsNote = useCallback(async (targetPath: string): Promise<boolean> => {
        if (!/\.(redpost|redvideo)$/i.test(String(targetPath || '').trim())) {
            setXhsEditorTarget(null);
            return false;
        }
        const result = await window.ipcRenderer.manuscripts.getXhsNote<{
            success?: boolean;
            error?: string;
        ...
        if (!result?.success || !result.xhsNote) {
            setXhsEditorTarget(null);
            return false;
        }
        const uri = result.uri || `manuscripts://${result.relativePath || targetPath}`;
        setXhsEditorTarget({
            href: uri,
            label: result.xhsNote.finalTitle || pathBasenameSafe(targetPath),
            kind: 'manuscript',
            resolvedUrl: '',
            isLocal: true,
            localPathCandidate: result.projectPath || targetPath,
            extension: result.noteType === 'video' ? 'redvideo' : 'redpost',
            artifactType: 'xiaohongshu-note',
            ...
            sourceMessageId: 'manuscripts-history',
        });
        setEditorDescriptor({
            title: result.xhsNote.finalTitle || DEFAULT_UNTITLED_DRAFT_TITLE,
            draftType: 'unknown',
        });
        return true;
    }, []);
    ...
    useEffect(() => {
        ...
        void (async () => {
            ...
            setMode('editor');
            try {
                if (await tryOpenStructuredXhsNote(filePath)) return;
    ...
    }, [filePath, tryOpenStructuredXhsNote]);
    ...
    const visibleDrafts = useMemo(() => {
        ...
        return currentNestedDraftFiles.filter((item) => {
            ...
            if (isRemovedMediaDraftType(meta?.draftType) && meta?.contentModel !== 'xhs-note-v1') return false;
    ...
    const openDraftEditor = useCallback(async (targetPath: string) => {
        ...
        setMode('editor');
        if (await tryOpenStructuredXhsNote(targetPath)) return;
    ...
    }, [fileMetaMap, tryOpenStructuredXhsNote]);
    ...
    const editorChatBinding = useMemo(() => buildEditorSessionBinding({
        ...
        packageState,
        editorBodyDirty,
        xhsNote: xhsEditorTarget ? {
            noteType: xhsEditorTarget.noteType,
            projectPath: xhsEditorTarget.projectPath || xhsEditorTarget.localPathCandidate || editorFile,
            uri: xhsEditorTarget.uri || xhsEditorTarget.href,
            revision: xhsEditorTarget.xhsNote?.revision || xhsEditorTarget.version,
        } : null,
    }), [
        editorAiWorkspaceMode,
        editorBodyDirty,
    ...
    if (mode === 'editor' && editorFile && xhsEditorTarget) {
        return (
            <div className="flex h-full min-h-0 flex-col bg-background">
                <div className="flex min-h-[52px] items-center justify-between gap-3 border-b border-border bg-surface-primary px-4">
                    <div className="min-w-0">
                        <div className="truncate text-sm font-semibold text-text-primary">{xhsEditorTarget.xhsNote?.finalTitle || xhsEditorTarget.label}</div>
                        <div className="text-[11px] text-text-tertiary">小红书{xhsEditorTarget.noteType === 'video' ? '视频' : '图文'}笔记 · v{xhsEditorTarget.xhsNote?.revision || xhsEditorTarget.version || 0}</div>
                    </div>
                    <div className="flex items-center gap-2">
                        <button type="button" onClick={() => setXhsChatFocusSignal((value) => value + 1)} className="rounded-xl border border-border px-3 py-1.5 text-xs font-medium text-text-secondary transition hover:bg-surface-secondary">在对话中继续</button>
                        {onClose && <button type="button" onClick={onClose} className="inline-flex h-8 w-8 items-center justify-center rounded-lg text-text-tertiary hover:bg-surface-secondary" title="关闭"><X className="h-4 w-4" /></button>}
                    </div>
                </div>
                <Suspense fallback={<div className="flex h-full items-center justify-center text-text-tertiary">小红书稿件工作台加载中...</div>}>
                    <XhsNoteWorkbench
                        target={xhsEditorTarget}
                        ...
                        chatFocusSignal={xhsChatFocusSignal}
                        isActive={isActive}
                    />
                </Suspense>
            </div>
