...
import { isAutomationTaskUiHint, type AutomationTaskUiHint } from '../../shared/automationTask';
...
const isAudioAssetUrl = (value: string): boolean => {
  const normalized = String(value || '').trim().toLowerCase();
  return ['.mp3', '.wav', '.m4a', '.flac', '.aac', '.ogg', '.oga', '.opus'].some((ext) => normalized.includes(ext));
};
...
function LiveProcessingElapsed({
  startedAt,
  finishedAt,
  isRunning,
}: {
  ...
  const endAt = resolveProcessingEndAt({
    startedAt,
    finishedAt,
    isRunning,
    now: liveNow,
  });
  return <>{formatProcessingElapsed(endAt - startedAt)}</>;
}
...
function ProcessingTimerBadge({
  ...
  const isRunning = Boolean(isStreaming && !Number.isFinite(finishedAt));

  return (
    <div className="chat-processing-timer" aria-live="off">
      <span className="chat-processing-timer__label">已处理</span>
      <span className="chat-processing-timer__value">
        <LiveProcessingElapsed
          startedAt={startedAt}
...
const isPreviewRelativePath = (value: string): boolean => {
  ...
  return /\.(png|jpe?g|webp|gif|bmp|svg|avif|ico|tiff?|mp4|webm|mov|m4v|mkv|avi|ogv|mp3|wav|m4a|flac|aac|ogg|oga|opus|pdf|docx?|odt|pptx?|odp|xlsx?|ods|html?|md|markdown|redpost|redvideo|thrive|txt|srt|vtt|diff|patch|json|csv|tsv|ya?ml|toml|ini|conf|config|env|xml|log|sql|sh|bash|zsh|fish|ts|tsx|js|jsx|mjs|cjs|rs|py|go|java|c|cpp|cc|cxx|h|hpp|hh|hxx|css|scss|sass|less|vue|svelte|astro|rb|php|swift|kt|kts|scala|r|lua|pl|pm|dart|dockerfile|lock|zip|rar|7z|tar|gz|tgz)(?:[?#].*)?$/i.test(raw);
};
...
const inferMessageLinkKind = (href: string, localPathCandidate?: string): ChatMessageLinkKind => {
  ...
  if (extension === 'thrive' || extension === 'redpost' || extension === 'redvideo') return 'manuscript';
...
const MARKDOWN_COMPONENTS: Components = {
  ...
  h1({ children }: any) {
    return <h1 className="mb-3 mt-5 text-xl font-semibold leading-tight first:mt-0">{children}</h1>;
  },
  h2({ children }: any) {
    return <h2 className="mb-2 mt-4 text-lg font-semibold leading-snug first:mt-0">{children}</h2>;
  },
  h3({ children }: any) {
    return <h3 className="mb-2 mt-3 text-base font-semibold leading-snug first:mt-0">{children}</h3>;
  },
};
...
export const MessageItem = memo(({
  ...
  // 结构化渲染提示由工具契约产出，这里只按 kind 分发，不解析回复文本。
  const automationTaskHints = useMemo<AutomationTaskUiHint[]>(() => {
    if (isUser) return [];
    const seen = new Set<string>();
    const hints: AutomationTaskUiHint[] = [];
    for (const item of msg.timeline || []) {
      const hint = item.toolData?.uiHint;
      if (!isAutomationTaskUiHint(hint)) continue;
      if (seen.has(hint.taskId)) continue;
      seen.add(hint.taskId);
      hints.push(hint);
    }
    return hints;
  }, [isUser, msg.timeline]);
  ...
  const shouldAutoHideWorkflow = false;
  ...
  const markdownComponents = useMemo<Components>(() => ({
    ...
    img({ src, alt }: any) {
      ...
      if (isAudioAssetUrl(mediaUrl)) {
        return (
          <audio
            src={mediaUrl}
            controls
            preload="metadata"
            className="my-3 w-full max-w-[36rem]"
            onContextMenu={(event) => handleMediaContextMenu(event, mediaUrl, rawSource)}
            title="右键复制或在文件夹中打开"
          />
        );
  ...
  const renderedAssistantContent = useMemo(() => (
    linkRenderMode === 'preview-card' && !isUser
      ? linkifyPreviewFilePaths(normalizeGeneratedMediaMarkup(visibleAssistantContent))
      : normalizeGeneratedMediaMarkup(visibleAssistantContent)
  ), [isUser, linkRenderMode, visibleAssistantContent]);

  const renderPreviewAwareMarkdownContent = useCallback((content: string) => (
    linkRenderMode === 'preview-card' && !isUser
      ? linkifyPreviewFilePaths(normalizeGeneratedMediaMarkup(content))
      : normalizeGeneratedMediaMarkup(content)
  ...
  const isUploadedAudioAttachment = useCallback((attachment: Extract<NonNullable<Message['attachment']>, { type: 'uploaded-file' }>) => {
    const kind = String(attachment.kind || '').trim().toLowerCase();
    const mimeType = String(attachment.mimeType || '').trim().toLowerCase();
    const ext = String(attachment.ext || '').trim().replace(/^\./, '').toLowerCase();
    return kind === 'audio' || mimeType.startsWith('audio/') || ['mp3', 'wav', 'm4a', 'flac', 'aac', 'ogg', 'oga', 'opus'].includes(ext);
  }, []);
  ...
  const renderUploadedFileCard = (attachment: Extract<NonNullable<Message['attachment']>, { type: 'uploaded-file' }>) => {
    ...
    if (isUploadedAudioAttachment(attachment)) {
      const actionSource = resolveUploadedAttachmentActionSource(attachment);
      const audioSource = actionSource ? resolveAssetUrl(actionSource) : '';
      if (audioSource) {
        return (
          <div className="mt-2 w-full max-w-[520px] rounded-xl border border-border bg-surface-primary/90 p-3 shadow-sm">
            <div className="mb-2 flex min-w-0 items-center gap-2">
              ...
              <div className="min-w-0">
                <div className="text-[11px] text-text-tertiary">参考音频</div>
                <div className="truncate text-sm font-medium text-text-primary" title={attachment.name}>{attachment.name}</div>
              </div>
            </div>
            <audio
              src={audioSource}
              ...
              preload="metadata"
              className="w-full"
              onContextMenu={(event) => handleMediaContextMenu(event, audioSource, actionSource)}
            />
          </div>
        );
      }
  ...
  const renderTimelineWorkflow = (timeline: ProcessItem[]) => {
    ...
    const toolCount = timeline.filter((item) => (
      item.type === 'tool-call'
    ...
    const runningItem = [...timeline].reverse().find((item) => item.status === 'running');
    const statusText = msg.isStreaming
      ? String(runningItem?.title || runningItem?.content || '正在处理').replace(/\s+/g, ' ').trim().slice(0, 64)
      : failedCount > 0
        ? `${failedCount} 个步骤失败`
        : '执行完成';

    return (
      <details className={clsx('group/workflow w-full max-w-3xl', showWorkflowOnTop ? 'mb-3' : 'mt-3')}>
        <summary className="flex cursor-pointer list-none items-center gap-2 rounded-lg py-1.5 text-xs text-text-tertiary transition-colors hover:text-text-secondary [&::-webkit-details-marker]:hidden">
          <ChevronDown className="h-3.5 w-3.5 -rotate-90 transition-transform group-open/workflow:rotate-0" />
          <span className="min-w-0 truncate font-medium">{statusText}</span>
          {toolCount > 0 ? <span>{toolCount} 个工具</span> : null}
          {typeof msg.processingStartedAt === 'number' ? (
            <span aria-label="执行耗时" aria-live="off">
              <LiveProcessingElapsed
                startedAt={msg.processingStartedAt}
                finishedAt={msg.processingFinishedAt}
                isRunning={Boolean(msg.isStreaming && !Number.isFinite(msg.processingFinishedAt))}
              />
            </span>
          ) : null}
        </summary>
        <div className="mt-2 space-y-3 border-l border-border/70 pl-4">
          {nodes}
        </div>
      </details>
    );
  ...
  return (
    <div className={clsx('chat-message-row', isUser ? 'chat-message-row-user' : 'chat-message-row-ai')}>
      ...
      {hasRenderableMessageContent && (
        isUser ? (
          ...
          <div className={clsx('chat-ai-shell group', msg.isStreaming && 'chat-ai-shell-streaming')}>
            ...
            {showProcessingTimer && !showTimeline && (
            ...
            {/* 复制按钮 */}
            {!msg.isStreaming && visibleAssistantContent && (
              <div className="chat-ai-actions opacity-0 transition-opacity group-hover:opacity-100">
                <button
                  onClick={() => onCopyMessage(msg.id, visibleAssistantContent)}
      ...
      {automationTaskHints.length > 0 && (
        <div className="w-full max-w-3xl">
          {automationTaskHints.map((hint) => (
  ...
  const msgChanged = 
    ...
    prevProps.msg.displayContent !== nextProps.msg.displayContent ||
    prevProps.msg.attachment !== nextProps.msg.attachment ||
