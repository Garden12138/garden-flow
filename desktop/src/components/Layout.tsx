...
export function Layout({ children, currentView, onNavigate, immersiveMode = false, hideGlobalSidebar = false, globalNotice = null, globalSidebarContent, activeModalView, renderTitleBarContent, renderTitleBarActions }: LayoutProps) {
  ...
  const {
    ...
    canCreateSpace: !RUNTIME_FEATURES.officialAccountAuth || canUseMembershipEntitlement(ENTITLEMENTS.spacesCreate),
    openMembershipModal: () => {
      if (RUNTIME_FEATURES.officialAccountAuth) {
  ...
  return (
    ...
    >
        ...
        >
          ...
          <div className={clsx('border-t border-border', sidebarVisualCollapsed ? 'px-2 py-2 flex flex-col items-center gap-2' : 'px-4 py-2 space-y-2')}>
            {RUNTIME_FEATURES.officialAccountAuth && (
              ...
              >
                <Crown className="h-[16px] w-[16px] shrink-0" strokeWidth={founderSponsorState.active ? 1.9 : 1.85} />
                {!sidebarVisualCollapsed && (
      ...
      {RUNTIME_FEATURES.officialAccountAuth && founderSponsorOpen && (
        <FounderSponsorModal
