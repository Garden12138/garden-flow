...
export type GenerationAgentRuntimeContextParams = {
    ...
    toolInputOverrides?: Record<string, unknown>;
...
export function sanitizeGenerationRequestForAgent(request: GenerationRequest): Record<string, unknown> {
    ...
    if (request.type === 'video') {
        return {
            ...request,
            referenceItems: summarizeGenerationReferenceItems(request.referenceItems),
            referenceAudios: summarizeGenerationReferenceItems(request.referenceAudios),
...
export function buildGenerationAgentRuntimeContext(params: GenerationAgentRuntimeContextParams): string {
    ...
    return [
        ...
        JSON.stringify({
            ...
            toolInputOverrides: params.toolInputOverrides,
            ...
            executionExpectation: 'Use the dedicated image_generate, video_generate, or audio_generate tool for the requested media. Apply toolInputOverrides directly to the matching tool arguments when present. For cross-media work, chain successful artifacts by absolutePath (for example image_generate assets into video_generate.referenceImages). Complete the requested generation without asking for a second confirmation unless a required field is genuinely missing.',
        }, null, 2),
        '[/GenerationAgentContext]',
    ].join('\n');
}
