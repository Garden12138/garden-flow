...
export type CaptureJobsBarVideo = {
  id: string;
  title?: string;
  status?: 'processing' | 'completed' | 'failed';
};

type CaptureJobsBarProps = {
  videos?: CaptureJobsBarVideo[];
};

export function CaptureJobsBar({ videos = [] }: CaptureJobsBarProps) {
  ...
  const activeCount = runningLocalTasks.length + remoteActive.length + processingVideos.length;
    ...
    <>
      ...
      >
        <span className="inline-flex min-w-0 items-center gap-2">
          ...
          {!snapshot.active?.progressMessage && processingVideos[0]?.title && (
            <span className="truncate text-text-tertiary">{processingVideos[0].title}</span>
          )}
        </span>
          ...
          >
            ...
            <div className="max-h-[320px] overflow-y-auto">
              ...
              {processingVideos.map((video) => (
                <div key={`youtube-${video.id}`} className="flex items-center justify-between gap-3 rounded-md px-2 py-2 text-[12px] hover:bg-surface-secondary">
                  <div className="min-w-0">
                    <div className="truncate font-semibold text-text-primary">{video.title || 'YouTube 视频'}</div>
                    <div className="mt-0.5 truncate text-[11px] text-text-tertiary">
                      处理中 · 字幕与摘要生成中
                    </div>
                  </div>
                  <div className="shrink-0 text-[11px] text-text-tertiary">YouTube</div>
                </div>
              ))}
              {waitingCount === 0 && remoteActive.length === 0 && processingVideos.length === 0 && (
