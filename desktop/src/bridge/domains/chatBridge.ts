...
import type { ChatSendReceipt } from '../../../shared/chatRunState';
...
export function createChatBridge(core: BridgeCore) {
  return {
    chat: {
      ...
      renameSession: (payload: { sessionId: string; title: string }) => core.invokeChannel('chat:rename-session', payload),
    ...
    chatrooms: {
      list: <T = unknown>() => core.invokeChannel('chatrooms:list') as Promise<T>,
      messages: <T = unknown>(roomId: string) => core.invokeChannel('chatrooms:messages', roomId) as Promise<T>,
      send: (payload: Record<string, unknown>) => core.sendChannel('chatrooms:send', payload),
      cancel: (payload: { roomId: string }) => core.invokeChannel('chatrooms:cancel', payload),
      create: <T = unknown>(payload: { name: string; advisorIds: string[] }) =>
        core.invokeChannel('chatrooms:create', payload) as Promise<T>,
      delete: (roomId: string) => core.invokeChannel('chatrooms:delete', roomId),
      clear: <T = unknown>(roomId: string) => core.invokeChannel('chatrooms:clear', roomId) as Promise<T>,
      update: <T = unknown>(payload: { roomId: string; name: string; advisorIds: string[] }) =>
        core.invokeChannel('chatrooms:update', payload) as Promise<T>,
    },
  };
}
