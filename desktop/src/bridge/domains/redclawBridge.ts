...
export function createRedClawBridge(core: BridgeCore) {
  return {
    redclawRunner: {
      ...
      listBuiltin: () => core.invokeChannel('redclaw:runner-list-builtin'),
      builtinReadiness: (payload: { taskId: string }) => core.invokeChannel('redclaw:runner-builtin-readiness', payload),
      setBuiltinEnabled: (payload: { taskId: string; enabled: boolean }) =>
        core.invokeChannel('redclaw:runner-set-builtin-enabled', payload),
      setBuiltinSettings: (payload: { taskId: string; settings?: Record<string, unknown>; scheduleTime?: string }) =>
        core.invokeChannel('redclaw:runner-set-builtin-settings', payload),
      runBuiltinNow: (payload: { taskId: string }) => core.invokeChannel('redclaw:runner-run-builtin-now', payload),
      installBuiltinMcpPreset: (payload?: { presetId?: string }) =>
        core.invokeChannel('redclaw:runner-install-builtin-mcp', payload || {}),
