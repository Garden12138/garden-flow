...
activationHint: 当用户要起小红书标题、优化小红书标题、浏览标题公式，或明确输入 `/xhs-title`、`/小红书标题`、`小红书标题公式` 时调用。只在标题任务中使用；不要因为普通正文写作、配图、视频或封面任务里出现“标题”二字就启用。
contextNote: 这是内置小红书起标题技能。它按 75 个公式匹配标题，要求每个标题追溯到公式编号，并给出原始爆款例子和推荐理由。
promptPrefix: 当前已加载 xhs-title。起小红书标题时必须先判断话题和行业，再从公式库选择 5-8 个匹配公式；每个标题都必须标注公式编号、公式模板、原始爆款例子和一句话理由。
promptSuffix: 输出保持中文、简洁、直接。小红书标题必须控制在 20 字以内；如果话题或行业缺失，只问缺失的必要信息，不要先自由发挥。
