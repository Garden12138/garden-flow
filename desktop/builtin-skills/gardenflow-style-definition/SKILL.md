---
description: GardenFlow 空间首次创作前的自媒体定位与写作风格定义。用“账号首页截图 + 文章/对标内容截图”两步证据流完成定位和文风学习。
allowedRuntimeModes: [gardenflow]
hookMode: inline
autoActivate: false
activationScope: session
hidden: true
activationHint: 当前空间尚未完成风格定义且这是用户第一次和 GardenFlow 对话时自动激活；用户点击“重新定义风格”或明确要求重做空间定位时也可激活。除此之外不要主动调用。
contextNote: 你正在主导当前 GardenFlow 空间的风格定义流程。主路径只有两个大步：第一步让用户上传账号主页截图，可以是 1 到 3 张同一账号的首页、简介区、置顶/近期内容截图，用来判断账号定位；第二步让用户上传一篇自己的文章截图或对标账号文章截图，用来学习创作风格。每一步都先细致分析再让用户确认或修正。流程要少问，但不能牺牲关键信息；如果截图无法判断目标群体、创作目标、账号定位、信任来源或商业边界，必须继续追问关键缺口，不能为了压缩轮次硬落档。
promptPrefix: 你已进入 GardenFlow 风格定义流程。不要让用户填写固定问卷 UI。第一轮必须优先请用户上传账号主页截图，推荐文案是“请把你的账号主页截图发给我，可以发 1 到 3 张，比如首页、简介区、置顶内容或近期作品列表；我可以一起分析多张图片，先判断账号定位、受众和内容方向。如果你还没有账号，直接回复‘还没有’。”收到账号截图后必须先做细致定位分析，至少覆盖账号定位、目标群体、创作目标、内容方向、信任来源、商业/转化线索、当前优势和关键缺口，再问用户是否准确、是否修正。定位确认后，第二大步请用户上传“一篇自己的文章/笔记截图”，或者“一个对标账号的文章截图/链接截图”，用于学习标题、开头、正文结构、语气、节奏和 CTA。收到文章截图后先总结创作风格，并问用户是否准确、是否修正。主流程建议 3 到 6 轮，最多 8 轮；如果关键定位信息不足，可以继续追问到最多 10 轮。信息足够后，给用户短确认摘要；只有用户确认后，才调用 `Operate(resource="gardenflow.profile", operation="completeStyleDefinition", input={...})` 完成落盘。
...
maxPromptChars: 6200
---
# GardenFlow Style Definition

这个技能负责 GardenFlow 当前空间的首次风格定义。它不是写作执行技能；完成后要生成或更新空间级 `writing-style` 技能，让后续写作任务按稳定风格执行。
...
- 当前空间第一次进入 GardenFlow 对话且风格定义未完成时，由宿主自动激活。
...
`Operate(resource="gardenflow.profile", operation="completeStyleDefinition", input={...})`
