...
const initDb = () => {
  db.exec(`
    CREATE TABLE IF NOT EXISTS settings (
      ...
      video_api_key TEXT,
      video_model TEXT,
      video_models_json TEXT,
      video_providers_json TEXT,
      active_video_provider_id TEXT,
  ...
  try {
    db.exec(`ALTER TABLE settings ADD COLUMN video_models_json TEXT;`);
  } catch { /* Column already exists */ }
  try {
    db.exec(`ALTER TABLE settings ADD COLUMN video_providers_json TEXT;`);
  } catch { /* Column already exists */ }
  try {
    db.exec(`ALTER TABLE settings ADD COLUMN active_video_provider_id TEXT;`);
  } catch { /* Column already exists */ }
  try {
...
const SETTINGS_EXTRA_KEYS = [
  ...
  'video_analysis_protocol',
  'video_analysis_max_direct_video_bytes',
  'docling_endpoint',
  'tika_endpoint',
...
type SettingsExtraKey = typeof SETTINGS_EXTRA_KEYS[number];

const parseSettingsExtras = (raw: unknown): Partial<Record<SettingsExtraKey, unknown>> => {
  ...
  try {
    ...
    ) as Partial<Record<SettingsExtraKey, unknown>>;
  } catch {
    return {};
  }
};

const serializeSettingsExtras = (
  settings: Record<string, unknown>,
  current: Record<string, unknown> | undefined,
): string => {
  const extras: Partial<Record<SettingsExtraKey, unknown>> = {};
  for (const key of SETTINGS_EXTRA_KEYS) {
    ...
    if (current && Object.prototype.hasOwnProperty.call(current, key) && current[key] !== undefined) {
...
export const saveSettings = (settings: {
  ...
  video_providers_json?: string;
  active_video_provider_id?: string;
  ...
  const stmt = db.prepare(`
    ...
    ON CONFLICT(id) DO UPDATE SET
      ...
      video_models_json = @video_models_json,
      video_providers_json = @video_providers_json,
      active_video_provider_id = @active_video_provider_id,
      ...
      settings_extra_json = @settings_extra_json
  `);
  ...
  return stmt.run({
    ...
    video_models_json: settings.video_models_json ?? current?.video_models_json ?? '[]',
    video_providers_json: settings.video_providers_json ?? current?.video_providers_json ?? '[]',
    active_video_provider_id: settings.active_video_provider_id ?? current?.active_video_provider_id ?? '',
    ...
    settings_extra_json: serializeSettingsExtras(settings, current),
  });
};
...
export const getSettings = () => {
  ...
  const result = stmt.get() as ({
    ...
    chat_max_tokens_default?: number;
    chat_max_tokens_deepseek?: number;
    settings_extra_json?: string;
  } & Record<string, unknown>) | undefined;
  let shouldRepairChatTokenSettings = false;
  if (result) {
    const extras = parseSettingsExtras(result.settings_extra_json);
    for (const key of SETTINGS_EXTRA_KEYS) {
      if (Object.prototype.hasOwnProperty.call(extras, key)) {
        if (key === 'video_models_json') {
          if (!String(result.video_models_json || '').trim()) {
            result.video_models_json = String(extras.video_models_json || '').trim();
          }
          continue;
        }
        if (key === 'ai_model_routes_json') {
          const routes = extras.ai_model_routes_json;
          result.ai_model_routes_json = typeof routes === 'string'
            ? routes
            : JSON.stringify(routes && typeof routes === 'object' ? routes : {});
          continue;
        }
        if (key === 'image_hosting_json') {
          const hosting = extras.image_hosting_json;
          result.image_hosting_json = typeof hosting === 'string'
            ? hosting
            : JSON.stringify(hosting && typeof hosting === 'object' ? hosting : {});
          continue;
        }
        result[key] = extras[key];
      }
    }
    delete result.settings_extra_json;
  }
...
function sessionMatchesContext(session: ChatSession, contextId: string, contextType: string): boolean {
  if (!session.metadata) return false;
  try {
    const meta = JSON.parse(session.metadata);
...
/**
 * 列出绑定到同一 context 的会话（按更新时间倒序）
 */
export const listChatSessionsByContext = (contextId: string, contextType: string): ChatSession[] => {
  return getChatSessions().filter((session) => sessionMatchesContext(session, contextId, contextType));
};
...
export const getChatSessionByContext = (contextId: string, contextType: string): ChatSession | null => {
  return listChatSessionsByContext(contextId, contextType)[0] || null;
};

/**
 * 更新会话元数据
 */
export const updateChatSessionMetadata = (
  id: string,
  metadata: Record<string, any>,
  options?: { touchUpdatedAt?: boolean },
): void => {
  if (options?.touchUpdatedAt === false) {
    db.prepare('UPDATE chat_sessions SET metadata = ? WHERE id = ?')
      .run(JSON.stringify(metadata), id);
    return;
  }
  db.prepare('UPDATE chat_sessions SET metadata = ?, updated_at = ? WHERE id = ?')
 ...
 * 更新已存在的聊天消息内容。
 *
 * 长任务会先写入一条可恢复的进度消息，完成或失败后再原位更新，避免稿件页
 * 重载会话历史时丢失仅存在于 renderer 内存中的流式占位消息。
 */
export const updateChatMessage = (
  id: string,
  updates: { content?: string; display_content?: string | null; metadata?: string | null },
): boolean => {
  const existing = db.prepare('SELECT * FROM chat_messages WHERE id = ?').get(id) as ChatMessage | undefined;
  if (!existing) return false;

  db.prepare(`
    UPDATE chat_messages
    SET content = @content, display_content = @display_content, metadata = @metadata
    WHERE id = @id
  `).run({
    id,
    content: updates.content ?? existing.content,
    display_content: updates.display_content === undefined
      ? existing.display_content ?? null
      : updates.display_content,
...
export const listRuntimeEvents = (params: {
  ...
  if (params.runId) {
    where.push("json_extract(payload_json, '$.runId') = ?");
    values.push(params.runId);
  }
  if (Number.isFinite(params.afterSequence)) {
    where.push("CAST(json_extract(payload_json, '$.sequence') AS INTEGER) > ?");
    values.push(Math.max(0, Math.floor(Number(params.afterSequence))));
  }
  ...
  const records = db.prepare(`
    ...
    ORDER BY created_at ${Number.isFinite(params.afterSequence) ? 'ASC' : 'DESC'}, id ${Number.isFinite(params.afterSequence) ? 'ASC' : 'DESC'}
    LIMIT ?
  `).all(...values) as RuntimeEventRecord[];
  return Number.isFinite(params.afterSequence) ? records : records.reverse();
};
...
try {
  db.exec(`ALTER TABLE wander_history ADD COLUMN source_mode TEXT;`);
} catch { /* Column already exists */ }
try {
  db.exec(`ALTER TABLE wander_history ADD COLUMN subject_refs TEXT;`);
} catch { /* Column already exists */ }
...
export const saveWanderHistory = (
  id: string,
  items: any[],
  result: any,
  metadata?: { sourceMode?: string; subjectRefs?: unknown[] },
): WanderHistory => {
  const now = Date.now();
  const scopedSpaceId = resolveSpaceId();
  const stmt = db.prepare(`
    INSERT INTO wander_history (id, space_id, items, result, created_at, status, abandoned_at, source_mode, subject_refs)
    VALUES (@id, @space_id, @items, @result, @created_at, @status, @abandoned_at, @source_mode, @subject_refs)
  `);
  const sourceMode = String(metadata?.sourceMode || '').trim() || null;
  const subjectRefs = Array.isArray(metadata?.subjectRefs) && metadata.subjectRefs.length > 0
    ? JSON.stringify(metadata.subjectRefs)
    : null;
  stmt.run({
    id,
    space_id: scopedSpaceId,
