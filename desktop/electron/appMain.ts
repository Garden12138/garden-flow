...
import { attachmentParticipatesInChatRuntime, attachmentRequiresDirectModelInput } from '../shared/chatAttachmentDelivery';
import {
...
import {
  ...
  saveXhsNoteProject,
} from './core/xhsNoteProjectStore';
import { writeXhsMaterialZip } from './core/xhsMaterialZip';
import {
  createBrowserCaptureBridgeService,
  type BrowserCaptureBridgeRequestContext,
} from './core/browserCaptureBridgeService';
import {
  bridgePayloadError,
  ...
  type BrowserKnowledgeEntryPayload,
} from './core/browserCapturePayloads';
import {
  browserNativeHostNeedsInstall,
  inspectBrowserNativeHost,
...
import { AppCliTool } from './core/tools/appCliTool';
...
import {
  ...
  getMcpOAuthStatus,
  ensurePresetMcpServer,
  COMPUTER_USE_MCP_PRESET,
...
function updateChatSessionMetadataPatch(sessionId: string, patch: Record<string, unknown>) {
  ...
  updateChatSessionMetadata(sessionId, metadata, { touchUpdatedAt: false });
  return { success: true, session: serializeChatSessionForRenderer(getChatSession(sessionId)) };
}

type ActiveXhsSessionBinding = {
...
function resolveExistingXhsProjectPath(value: unknown): string | null {
  let candidate = String(value || '').trim();
  if (!candidate) return null;
  if (candidate.startsWith('manuscripts://')) {
    candidate = decodeURIComponent(candidate.slice('manuscripts://'.length));
  }
  const manuscriptsRoot = path.resolve(getManuscriptsDir());
  const absolutePath = path.isAbsolute(candidate)
    ? path.resolve(candidate)
    : path.resolve(manuscriptsRoot, candidate);
  const relativePath = path.relative(manuscriptsRoot, absolutePath);
  if (relativePath.startsWith('..') || path.isAbsolute(relativePath)) return null;
  if (!absolutePath.endsWith('.redpost') && !absolutePath.endsWith('.redvideo')) return null;
  try {
    return fsSync.statSync(absolutePath).isDirectory() ? absolutePath : null;
  } catch {
    return null;
  }
}

function toActiveXhsSessionBinding(absolutePath: string): ActiveXhsSessionBinding {
  const relativePath = path.relative(getManuscriptsDir(), absolutePath).split(path.sep).join('/');
  return {
    artifactType: 'xiaohongshu-note',
    activeXhsNotePath: absolutePath,
    activeXhsNoteUri: `manuscripts://${relativePath}`,
    xhsNoteType: absolutePath.endsWith('.redvideo') ? 'video' : 'image',
  };
}

function recoverActiveXhsSessionBinding(
  sessionId: string,
  explicitPath?: unknown,
): Record<string, unknown> {
  const session = getChatSession(sessionId);
  const metadata = parseChatSessionMetadata(session?.metadata);
  const directCandidates = [explicitPath, metadata.activeXhsNotePath, metadata.activeXhsNoteUri];
  let activePath = directCandidates
    .map(resolveExistingXhsProjectPath)
    .find((candidate): candidate is string => Boolean(candidate));

  if (!activePath) {
    const artifactCandidates = listAgentTasks({ ownerSessionId: sessionId, limit: 100 })
      .flatMap((task) => parseAgentTaskRecord(task)?.artifacts || [])
      .filter((artifact) => artifact.type === 'xiaohongshu-note-project')
      .sort((left, right) => Number(right.createdAt || 0) - Number(left.createdAt || 0));
    activePath = artifactCandidates
      .flatMap((artifact) => [artifact.absolutePath, artifact.relativePath])
      .map(resolveExistingXhsProjectPath)
      .find((candidate): candidate is string => Boolean(candidate));
  }
  ...
  if (activePath) {
    Object.assign(nextMetadata, toActiveXhsSessionBinding(activePath));
  } else {
    delete nextMetadata.artifactType;
    delete nextMetadata.activeXhsNotePath;
    delete nextMetadata.activeXhsNoteUri;
    delete nextMetadata.xhsNoteType;
  }
  if (session && JSON.stringify(nextMetadata) !== JSON.stringify(metadata)) {
    updateChatSessionMetadata(sessionId, nextMetadata);
  }
  return nextMetadata;
}

function clearDeletedXhsSessionBindings(deletedPath: string): void {
  const normalizedDeletedPath = path.resolve(deletedPath);
  for (const session of getChatSessions()) {
    const metadata = parseChatSessionMetadata(session.metadata);
    const boundPath = String(metadata.activeXhsNotePath || '').trim();
    if (!boundPath) continue;
    const relativePath = path.relative(normalizedDeletedPath, path.resolve(boundPath));
    const bindingWasDeleted = relativePath === ''
      || (!relativePath.startsWith('..') && !path.isAbsolute(relativePath));
    if (!bindingWasDeleted) continue;
    delete metadata.artifactType;
    delete metadata.activeXhsNotePath;
    delete metadata.activeXhsNoteUri;
    delete metadata.xhsNoteType;
    updateChatSessionMetadata(session.id, metadata);
  }
}
...
function emitKnowledgeCatalogChanged(payload: Record<string, unknown> = {}) {
  for (const browserWindow of BrowserWindow.getAllWindows()) {
    if (browserWindow.isDestroyed()) continue;
    browserWindow.webContents.send('knowledge:changed', payload);
    browserWindow.webContents.send('knowledge:catalog-updated', payload);
    browserWindow.webContents.send('knowledge-updated', payload);
  }
  emitRendererDataChanged('knowledge', payload);
}
...
type RedClawAuthoringHints = {
  ...
  xhsNoteType?: 'image' | 'video';
  xhsVideoDurationSeconds?: number;
  xhsVideoAspectRatio?: '9:16' | '16:9';
  activeXhsNotePath?: string;
  activeXhsNoteUri?: string;
...
function normalizeRedClawAuthoringHints(raw: unknown): RedClawAuthoringHints | null {
  ...
  const requestedVideoDuration = Number(record.xhsVideoDurationSeconds);
  const requestedVideoAspectRatio = String(record.xhsVideoAspectRatio || '').trim();
  return {
    platform,
    taskType,
    ...
    xhsNoteType: String(record.xhsNoteType || '').trim() === 'video'
      ? 'video'
      : String(record.xhsNoteType || '').trim() === 'image'
        ? 'image'
        : undefined,
    xhsVideoDurationSeconds: Number.isFinite(requestedVideoDuration)
      && requestedVideoDuration >= 1
      && requestedVideoDuration <= 300
      ? Math.round(requestedVideoDuration)
      : undefined,
    xhsVideoAspectRatio: requestedVideoAspectRatio === '9:16' || requestedVideoAspectRatio === '16:9'
      ? requestedVideoAspectRatio
      : undefined,
    activeXhsNotePath: String(record.activeXhsNotePath || '').trim() || undefined,
    activeXhsNoteUri: String(record.activeXhsNoteUri || '').trim() || undefined,
    editorAction: String(record.editorAction || '').trim() === 'generate-video'
      ? 'generate-video'
      : undefined,
  };
}
...
function buildRedClawAuthoringPrompt(userBrief: string, hints: RedClawAuthoringHints): string {
  ...
  return renderPrompt(template, {
    ...
    xhs_note_type: hints.xhsNoteType || '',
    xhs_video_duration_seconds: hints.xhsVideoDurationSeconds || '',
    xhs_video_aspect_ratio: hints.xhsVideoAspectRatio || '',
    active_xhs_note_path: hints.activeXhsNotePath || '',
    active_xhs_note_uri: hints.activeXhsNoteUri || '',
...
function resolveForcedSkillNames(input: unknown): string[] {
  ...
  const xhsNoteType = String(record.xhsNoteType || '').trim();
  if (platform === 'xiaohongshu') {
    if (!forcedSkillNames.includes('writing-style')) forcedSkillNames.push('writing-style');
    if (!forcedSkillNames.includes('xhs-title')) forcedSkillNames.push('xhs-title');
...
function compactChatAttachments(input: unknown): Array<Record<string, unknown>> {
  ...
  return input
    ...
    .map((item) => ({
      ...
      displayOnly: item.displayOnly === true ? true : undefined,
    }))
    .slice(0, 12);
}
...
const getBundledBrowserPluginCandidateDirs = (): string[] => {
  ...
  const candidates = [
    ...
    path.join(process.cwd(), 'Plugin', 'dist', 'extension'),
    path.join(path.resolve(appPath, '..'), 'Plugin', 'dist', 'extension'),
  ];
...
const isUsableBrowserPluginDir = async (candidate: string): Promise<boolean> => {
  try {
    ...
    const sidePanel = String(manifest?.side_panel?.default_path || '').trim();
    if (manifest?.manifest_version !== 3 || !serviceWorker || !sidePanel) return false;
    await Promise.all([
      fs.access(path.join(candidate, serviceWorker)),
      fs.access(path.join(candidate, sidePanel)),
    ]);
    return true;
  } catch {
    return false;
...
const warmupBrowserPluginPrepared = async (): Promise<void> => {
  try {
    ...
    if (browserNativeHostNeedsInstall(nativeHost)) {
      await installBrowserNativeHost({
        appExecutable: process.execPath,
        appPath: app.getAppPath(),
        isPackaged: app.isPackaged,
        resourcesPath: process.resourcesPath,
...
const sanitizeRemoteReleaseCopy = (value: string): string => String(value || '')
  .replace(/!\[[^\]]*\]\([^\s)]+(?:\s+"[^"]*")?\)/g, '')
  .replace(/\[([^\]]+)\]\([^\s)]+(?:\s+"[^"]*")?\)/g, '$1')
  .replace(/https?:\/\/\S+/gi, '')
...
async function fetchLatestGithubRelease(): Promise<{
  ...
  try {
    ...
    return {
      version,
      htmlUrl: '',
      name: sanitizeRemoteReleaseCopy(String(data.name || '')),
      publishedAt: String(data.published_at || ''),
      body: sanitizeRemoteReleaseCopy(String(data.body || '')),
      assets: (Array.isArray(data.assets) ? data.assets : [])
        ...
        .filter((asset) => asset.name && isHttpUrl(asset.downloadUrl)),
    };
  } finally {
...
const emitAppUpdateInstallProgress = (payload: {
  ...
  win?.webContents.send('app:update-install-progress', payload);
};

const downloadAppUpdateAsset = async (input: {
  version: string;
  downloadUrl: string;
  ...
  const contentLength = input.contentLength || headerLength || 0;
  const expectedDigest = /^sha256:([a-f0-9]{64})$/i.exec(input.digest)?.[1]?.toLowerCase() || '';
  const hash = createHash('sha256');
  const fileHandle = await fs.open(input.destination, 'w');
  let downloaded = 0;
  try {
  ...
  if (contentLength > 0 && downloaded !== contentLength) {
    throw new Error('Update download was incomplete');
  }
...
const registerLocalAssetProtocols = () => {
  ...
  const getAssetContentType = (filePath: string): string => (
  ...
  const handleAssetRequest = (schemeName: string) => async (request: Request) => {
    ...
    const byteRange = resolveLocalAssetByteRange(rangeHeader, fileStat.size);
    const commonHeaders = {
      'Accept-Ranges': 'bytes',
    ...
    const end = byteRange?.end ?? Math.max(0, fileStat.size - 1);
    ...
    if (byteRange) {
      responseHeaders['Content-Range'] = `bytes ${start}-${end}/${fileStat.size}`;
    }
    ...
    const body = Readable.toWeb(fsSync.createReadStream(resolvedPath, { start, end })) as ReadableStream<Uint8Array>;
...
async function buildStagedChatAttachment(targetPath: string, originalPath: string, fileName?: string) {
  ...
  return {
    type: 'uploaded-file',
    attachmentId: `chat-attachment-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`,
...
const stagedChatAttachmentsBySession = new Map<string, Record<string, unknown>[]>();

function stagedChatAttachmentIdentity(attachment: Record<string, unknown>): string {
  return String(
...
function rememberStagedChatAttachment(sessionId: unknown, attachment: Record<string, unknown>): void {
  const normalizedSessionId = String(sessionId || '').trim();
  if (!normalizedSessionId) return;
  const identity = stagedChatAttachmentIdentity(attachment);
  const current = stagedChatAttachmentsBySession.get(normalizedSessionId) || [];
  const next = identity
    ? [...current.filter((item) => stagedChatAttachmentIdentity(item) !== identity), attachment]
    : [...current, attachment];
  stagedChatAttachmentsBySession.set(normalizedSessionId, next.slice(-9));
}

function mergeStagedChatAttachments(
  sessionId: unknown,
  supplied: Record<string, unknown>[],
): Record<string, unknown>[] {
  const normalizedSessionId = String(sessionId || '').trim();
  const staged = normalizedSessionId ? stagedChatAttachmentsBySession.get(normalizedSessionId) || [] : [];
  const merged: Record<string, unknown>[] = [];
  const seen = new Set<string>();
  for (const attachment of [...supplied, ...staged]) {
    const identity = stagedChatAttachmentIdentity(attachment);
...
function listStagedChatAttachments(sessionId: unknown): Record<string, unknown>[] {
  const normalizedSessionId = String(sessionId || '').trim();
  return normalizedSessionId
    ? [...(stagedChatAttachmentsBySession.get(normalizedSessionId) || [])]
    : [];
}

function forgetStagedChatAttachments(sessionId: unknown, attachments: Record<string, unknown>[]): void {
  const identities = new Set(attachments.map(stagedChatAttachmentIdentity).filter(Boolean));
  if (identities.size === 0) return;
  const normalizedSessionId = String(sessionId || '').trim();
  const entries = normalizedSessionId
    ? [[normalizedSessionId, stagedChatAttachmentsBySession.get(normalizedSessionId) || []] as const]
    : [...stagedChatAttachmentsBySession.entries()];
  for (const [key, current] of entries) {
    const next = current.filter((item) => !identities.has(stagedChatAttachmentIdentity(item)));
    if (next.length > 0) {
      stagedChatAttachmentsBySession.set(key, next);
    } else {
      stagedChatAttachmentsBySession.delete(key);
    }
  }
}
...
app.whenReady().then(async () => {
  ...
  try {
    await browserCaptureBridgeService.start();
    markStartupPhase('browser-capture-bridge-started');
  } catch (error) {
    console.error('[browser-capture] Desktop Bridge failed to start:', error);
  }
  createWindow();
...
ipcMain.handle('runtime:get-events', (_event, payload?: {
  ...
  return getSessionRuntimeStore().listRuntimeEvents({
    ...
    afterSequence: Number.isFinite(Number(payload?.afterSequence))
      ? Math.max(0, Math.floor(Number(payload?.afterSequence)))
      : undefined,
...
async function buildBrowserPluginStatus() {
  ...
  const problems: Array<{ code: string; message: string; recovery: string }> = [];
  if (!bundledDir) {
    problems.push({
  ...
  if (!exported) {
    problems.push({
      code: 'PLUGIN_NOT_EXPORTED',
      message: '浏览器插件尚未准备',
      recovery: '点击“准备浏览器插件”完成导出和 Native Host 安装',
    });
  }
  if (!bridge.listening) {
    problems.push({
  ...
  if (nativeHost.staleTargets.length > 0) {
    problems.push({
      code: 'NATIVE_HOST_STALE',
      message: 'Native Host 配置需要修复',
      recovery: '再次点击“准备浏览器插件”自动修复',
    });
  } else if (nativeHost.installedTargets.length === 0) {
    problems.push({
      code: 'NATIVE_HOST_NOT_INSTALLED',
      message: 'Native Host 尚未安装',
      recovery: '点击“准备浏览器插件”完成安装',
    });
  } else if (nativeHost.missingTargets.length > 0) {
    problems.push({
      code: 'NATIVE_HOST_TARGET_MISSING',
      message: `${nativeHost.missingTargets.map((item) => item.label).join('、')} 尚未安装 Native Host`,
      recovery: '再次点击“准备浏览器插件”完成所有浏览器的安装',
    });
  }
  for (const failure of nativeHost.failures) {
    problems.push({
      code: 'NATIVE_HOST_TARGET_FAILED',
      message: `${failure.label}：${failure.error || '检查失败'}`,
  ...
  for (const instance of bridge.instances) {
    ...
    problems.push({
      ...
      message: `${instance.accessProblem.platform}：${instance.accessProblem.code === 'BROWSER_LOGIN_REQUIRED'
        ? '平台要求登录'
        : instance.accessProblem.code === 'BROWSER_SECURITY_CHALLENGE'
          ? '需要完成安全验证'
          : '正文暂不可访问'}`,
  ...
  if (exported && nativeHost.installedTargets.length > 0 && bridge.instances.length === 0) {
    problems.push({
      ...
      recovery: '在浏览器扩展管理页选择导出的目录，随后重新加载插件',
    });
  }
  return {
    success: true,
    ...
    nativeHost: {
      installedTargets: nativeHost.installedTargets,
...
ipcMain.handle('plugin:browser-extension-status', async () => {
  try {
    return await buildBrowserPluginStatus();
  } catch (error) {
    return {
      success: false,
      bundled: false,
...
ipcMain.handle('plugin:prepare-browser-extension', async () => {
  try {
    ...
    const status = await buildBrowserPluginStatus();
...
ipcMain.handle('app:install-update', async () => {
  if (appUpdateInstallInFlight) {
    return { success: false, inFlight: true, error: '更新安装已在进行中' };
  }
  appUpdateInstallInFlight = true;
  let installerPath = '';
  let version = '';
  try {
    emitAppUpdateInstallProgress({ status: 'checking', version });
    ...
    return { success: true, installed: true, hasUpdate: true };
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    if (installerPath) {
      await fs.rm(installerPath, { force: true }).catch(() => undefined);
    }
    emitAppUpdateInstallProgress({ status: 'failed', version, error: message });
    return { success: false, hasUpdate: Boolean(version), error: message };
  } finally {
...
ipcMain.handle('file:preview-resolve', async (_, payload?: { source?: string }) => {
  try {
    ...
    const virtualManuscriptMatch = /^manuscripts:\/\/(.+)$/i.exec(source);
    const rawPath = virtualManuscriptMatch
      ? path.join(getWorkspacePaths().manuscripts, normalizeRelativePath(decodeURIComponent(virtualManuscriptMatch[1])))
      : resolveAssetSourceToPath(source);
    const normalizedPath = path.resolve(path.normalize(rawPath));
    let resolvedPath = normalizedPath;
    try {
      resolvedPath = fsSync.realpathSync.native(normalizedPath);
    ...
    const isXhsNoteProject = exists && isDirectory && isStructuredXhsNoteProject(resolvedPath);
    if (isXhsNoteProject) {
      const note = await getXhsNoteProject(resolvedPath);
      return {
        success: true,
        isLocal: true,
        exists: true,
        ...
        xhsNote: note.document,
      };
    }
    const kind = isDirectory && isManuscriptPackageName(path.basename(resolvedPath))
      ? 'manuscript'
      : previewKindForExtension(extension, true);
...
async function getOrCreateChatService(sessionId: string, sender?: Electron.WebContents): Promise<PiChatServiceInstance> {
  ...
  if (!service) {
    ...
    service = new PiChatService({ interactiveToolConfirmation: true });
...
// 获取或创建上下文关联的会话 (知识库聊天)
ipcMain.handle('chat:getOrCreateContextSession', async (_, {
  contextId,
  contextType,
  title,
  initialContext,
  ...
  if (existingSession) {
    ...
    try {
      const meta = JSON.parse(existingSession.metadata || '{}');
      const nextMetadata = mergeContextSessionMetadata({
        currentMetadata: meta,
        requestedMetadata: requestedMetadata && typeof requestedMetadata === 'object' ? requestedMetadata : {},
        contextId,
        contextType,
        initialContext,
      });
      if (JSON.stringify(meta) !== JSON.stringify(nextMetadata)) {
        Object.assign(meta, nextMetadata);
        updateChatSessionMetadata(existingSession.id, meta);
        existingSession.metadata = JSON.stringify(meta);
      }
    } catch (e) {
  ...
  return getChatSessions()
    ...
    .filter((session): session is NonNullable<ReturnType<typeof serializeChatSessionForRenderer>> => (
      Boolean(
        session
        && !session.archived
        && session.metadata.contextId === contextId
        && session.metadata.contextType === contextType
      )
    ))
    .map((session) => ({
      id: session.id,
...
ipcMain.handle('redclaw:runner-install-builtin-mcp', async (_, payload: { presetId?: string }) => {
  ...
  try {
    ...
    return { success: true, added: result.added, server: result.server };
  } catch (error) {
    return { success: false, error: error instanceof Error ? error.message : String(error) };
  }
});
...
ipcMain.handle('media:import-files', async (event, payload?: { kind?: 'image' | 'video'; multiple?: boolean }) => {
  try {
    ...
    const filters: Electron.FileFilter[] = requestedKind === 'video'
      ? [{ name: '视频文件', extensions: ['mp4', 'mov', 'webm', 'm4v', 'avi', 'mkv'] }]
      : requestedKind === 'image'
...
ipcMain.handle('image-hosting:test-upload', async (_, payload?: { image_hosting_json?: unknown }) => {
  try {
    const [{ uploadTestPng }, { getSettings }] = await Promise.all([
      import('./core/imageHosting/service'),
      import('./db'),
    ]);
    return await uploadTestPng({
      settings: payload?.image_hosting_json,
      getSettings: () => getSettings(),
    });
  } catch (error) {
    return { ok: false, error: error instanceof Error ? error.message : String(error || '图床测试上传失败') };
  }
});
...
ipcMain.handle('chat:get-runtime-state', async (event, sessionId: string) => {
  ...
  try {
    const chatRun = getPersistedChatRunSnapshot(sessionId);
    const service = chatServices.get(sessionId);
    if (!service && !chatRun) {
      return {
        success: true,
    ...
    if (browserWindow && service) {
      service.setWindow(browserWindow);
    }
    const runtime = service?.getRuntimeState();
    const isRunProcessing = chatRun?.status === 'queued' || chatRun?.status === 'running';
    return {
      success: true,
      sessionId,
      isProcessing: Boolean(isRunProcessing || runtime?.isProcessing),
      partialResponse: chatRun?.content || runtime?.partialResponse || '',
      updatedAt: Number(chatRun?.updatedAt || runtime?.updatedAt || 0),
      run: chatRun,
    };
  } catch (error) {
    return {
...
ipcMain.handle('chat:pick-attachment', async (event, payload?: { sessionId?: string }) => {
  try {
    const ownerWindow = BrowserWindow.fromWebContents(event.sender);
    const pickerOptions: Electron.OpenDialogOptions = {
      title: '选择要上传的文件',
      properties: ['openFile'],
      filters: [
        { name: '支持的素材', extensions: ['png', 'jpg', 'jpeg', 'webp', 'gif', 'bmp', 'svg', 'mp4', 'mov', 'webm', 'm4v', 'avi', 'mkv', 'mp3', 'wav', 'm4a', 'aac', 'flac', 'ogg', 'opus', 'txt', 'md', 'pdf', 'doc', 'docx'] },
    ...
    const stagedAttachment = await buildStagedChatAttachment(targetPath, selectedPath, path.basename(selectedPath));
...
ipcMain.handle('chat:create-inline-attachment', async (_event, payload?: {
  ...
  try {
    ...
    const stagedAttachment = await buildStagedChatAttachment(targetPath, rawFileName, rawFileName);
    if (payload?.rememberForChat !== false) {
      rememberStagedChatAttachment(payload?.sessionId, stagedAttachment);
    }
    return {
      success: true,
...
ipcMain.handle('chat:get-staged-attachments', async (_event, payload?: { sessionId?: string }) => ({
  success: true,
  attachments: listStagedChatAttachments(payload?.sessionId),
}));
...
ipcMain.handle('chat:discard-attachments', async (_event, payload?: {
  ...
  try {
    ...
    forgetStagedChatAttachments(
      payload?.sessionId,
      attachments.filter((item): item is Record<string, unknown> => Boolean(item) && typeof item === 'object' && !Array.isArray(item)),
    );
    let removed = 0;
    for (const attachment of attachments) {
...
const structuredXhsVideoGenerationRuns = new Map<string, Promise<void>>();

function reconcilePersistedChatRun(_sessionId: string, run: ChatRunLifecycle): void {
...
function reconcileInterruptedChatRun(sessionId: string): void {
  if (!sessionId) return;
  if (getActiveChatRun(sessionId)) return;
  const persistedRun = getPersistedChatRunSnapshot(sessionId);
  if (persistedRun && (persistedRun.status === 'queued' || persistedRun.status === 'running')) {
    const message = getChatMessages(sessionId).find((item) => item.id === persistedRun.assistantMessageId);
    if (message) {
      const finishedAt = Date.now();
      updateChatMessage(message.id, {
        content: message.content || '上次处理未完成：应用退出前没有收到最终回复，请重新发送。',
        metadata: buildChatRunMessageMetadata({
          messageKind: 'chat-run',
          runId: persistedRun.runId,
          userMessageId: persistedRun.userMessageId,
          assistantMessageId: persistedRun.assistantMessageId,
          status: 'failed',
          phase: 'failed',
          sequence: persistedRun.sequence,
  ...
  const service = chatServices.get(sessionId);
  if (service?.getRuntimeState().isProcessing) return;

  const startedAt = Number(lastMessage.timestamp || 0);
  if (!Number.isFinite(startedAt) || Date.now() - startedAt < 5_000) return;

  const finishedAt = Date.now();
  const runId = `recovered_interrupted_run_${lastMessage.id}`;
  addChatMessage({
    id: `chat_run_recovered_${finishedAt}_${Math.random().toString(36).slice(2, 8)}`,
    session_id: sessionId,
    role: 'assistant',
    content: '上次处理未完成：任务在回复保存前中断，请重新发送。',
    metadata: buildChatRunMessageMetadata({
      messageKind: 'chat-run',
      runId,
      status: 'failed',
...
async function runStructuredXhsVideoGeneration(params: {
  ...
  const notePath = String(params.hints.activeXhsNotePath || '').trim();
  if (!notePath || params.hints.xhsNoteType !== 'video') {
    throw new Error('当前会话没有绑定可生成的视频笔记工程');
  }

  const referenceImages = params.attachments
    .filter((item) => String(item.kind || '').trim().toLowerCase() === 'image')
    .map((item) => String(item.absolutePath || '').trim())
    .filter(Boolean)
    .slice(0, 9);
  const callId = `xhs_video_${Date.now()}`;
  const command = 'video generate-note';
  const input = {
    command,
    payload: {
      notePath,
  ...
  const emitChatEvent = params.emitChatEvent || ((channel: string, data: unknown) => params.sender.send(channel, data));
  emitChatEvent('chat:phase-start', { name: '生成并绑定视频' });
  emitChatEvent('chat:thought-start', {});
  emitChatEvent('chat:thought-delta', {
    content: referenceImages.length > 0
      ? `已读取 ${referenceImages.length} 张参考图，正在按分镜生成视频...`
      : '正在按已确认的分镜生成视频...',
  });
  emitChatEvent('chat:tool-start', {
    callId,
    name: 'app_cli',
    input,
    description: '按视频笔记分镜生成片段并绑定最终成片',
  });

  try {
    const result = await new AppCliTool().execute(input);
    emitChatEvent('chat:tool-end', {
      callId,
      name: 'app_cli',
      output: {
        success: result.success,
        content: result.display || result.llmContent || result.error?.message || '',
      },
    });
    if (!result.success) {
      throw new Error(result.error?.message || result.llmContent || '视频生成失败');
    }

    const response = [
      '视频已生成，并绑定到当前稿件的“最终成片”槽位。',
      result.llmContent,
    ].filter(Boolean).join('\n\n');
    emitChatEvent('chat:response-chunk', { content: response });
    emitChatEvent('chat:response-end', { content: response });
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    throw Object.assign(error instanceof Error ? error : new Error(message), {
      chatErrorMessage: '视频生成失败',
      chatErrorHint: message,
      chatErrorCategory: 'video-generation',
    });
  }
}
...
async function executeChatMessage(
  ...
  payload: any,
  onReceipt?: (receipt: ChatSendReceipt) => void,
): Promise<void> {
  let {
    sessionId,
    message,
  ...
  const suppliedAttachments = normalizeUploadedChatAttachments(attachment, attachments);
  const rawAttachments = mergeStagedChatAttachments(sessionId, suppliedAttachments);
  const runtimeAttachments = rawAttachments.filter(attachmentParticipatesInChatRuntime);
  const rawAttachment = rawAttachments[0] || ((attachment && typeof attachment === 'object') ? (attachment as Record<string, unknown>) : null);
  const runtimeAttachment = runtimeAttachments[0] || null;
  ...
  if (runtimeAttachments.length > 0) {
    ...
    const unsupportedMultimodalAttachment = runtimeAttachments.find((item) => {
      const kind = String(item.kind || 'file').trim().toLowerCase();
      return attachmentRequiresDirectModelInput(item) && !supportsAttachmentKindDirectInput(resolvedModelName, kind);
    });
  ...
  // 保存用户消息到数据库
  if (getActiveChatRun(sessionId)) {
    sender.send('chat:error', {
      message: '当前会话已有回复正在生成',
      category: 'request',
      hint: '可以切换到其他会话继续提问，或停止当前回复后重试。',
    });
    const active = getActiveChatRun(sessionId)?.snapshot();
    onReceipt?.({
      accepted: false,
      sessionId,
      runId: active?.runId || '',
  ...
  const userMsgId = `chat_user_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`;
  ...
  forgetStagedChatAttachments(sessionId, rawAttachments);
  const startedChatRun = startChatRun(sessionId, userMsgId);
  if (!startedChatRun.receipt.accepted) {
    sender.send('chat:error', {
      message: startedChatRun.receipt.error || '当前会话暂时无法开始回复',
      category: 'request',
    });
    return;
  }
  ...
  try {
    ...
    if (
      ...
      if (structuredXhsVideoGenerationRuns.has(generationKey)) {
        throw Object.assign(new Error('当前稿件的视频仍在生成，请等待完成后再试。'), {
          ...
          chatErrorHint: '当前稿件已有生成任务在运行，请勿重复点击。',
          chatErrorCategory: 'video-generation',
        });
      }
      ...
      try {
        ...
        if (structuredXhsVideoGenerationRuns.get(generationKey) === generationRun) {
          structuredXhsVideoGenerationRuns.delete(generationKey);
        }
      }
    }
    ...
    const routeAnalysis = getAgentRuntime().analyzeRuntimeContext({
      runtimeContext: {
        ...
        metadata: {
          ...sessionMeta,
          attachmentType: runtimeAttachment?.type || null,
          ...effectiveTaskHints,
        },
      },
    });
    ...
    if ((routeAnalysis.shouldUseCoordinator || isStructuredXhsEditor) && runtimeMode !== 'background-maintenance') {
      ...
      await getLongTaskCoordinator().maybeRun(prepared.task.id, {
        baseSystemPrompt,
        force: isStructuredXhsEditor,
        skipPreparationRoles: isStructuredXhsEditor,
        llm: {
          apiKey: resolvedChatApiKey,
          baseURL: resolvedChatBaseURL,
    ...
    try {
      ...
      service.setChatRunEventSink(null);
    }
    ...
    const modelLabel = resolvedModelName || '当前模型';
...
ipcMain.on('chat:send-message', (event, payload) => {
  void executeChatMessage(event, payload);
});

ipcMain.handle('chat:send-message', (event, payload) => new Promise<ChatSendReceipt>((resolve) => {
  let settled = false;
  const settle = (receipt: ChatSendReceipt) => {
    if (settled) return;
    settled = true;
    resolve(receipt);
  };
  void executeChatMessage(event, payload, settle).then(() => {
    if (!settled) {
      settle({
        accepted: false,
...
ipcMain.on('chat:cancel', (_, payload?: { sessionId?: string; runId?: string } | string) => {
    ...
    if (sessionId) {
      ...
      const requestedRunId = typeof payload === 'string' ? '' : String(payload?.runId || '').trim();
      if (requestedRunId && activeRun?.runId !== requestedRunId) return;
      const taskId = activeRun?.snapshot().taskId;
      if (taskId) void getLongTaskCoordinator().cancel(taskId);
      if (!taskId) {
        const staleTasks = getTaskGraphRuntime().listTasks({
          status: 'running',
          ownerSessionId: sessionId,
          limit: 20,
        });
        for (const task of staleTasks) {
          getTaskGraphRuntime().cancelTask(task.id);
        }
      }
      const service = chatServices.get(sessionId);
      if (service) {
...
ipcMain.handle('manuscripts:get-xhs-note', async (_, payload: { path: string } | string) => {
  try {
    const filePath = typeof payload === 'string' ? payload : payload?.path;
    const snapshot = await getXhsNoteProject(String(filePath || ''));
    return { success: true, ...snapshot, xhsNote: snapshot.document };
  } catch (error) {
    return { success: false, error: error instanceof Error ? error.message : String(error) };
  }
});

ipcMain.handle('manuscripts:save-xhs-note', async (_, payload: {
  path?: string;
  ...
  try {
    const snapshot = await saveXhsNoteProject(payload || { document: {} });
    invalidateManuscriptTreeCache(getManuscriptsDir());
    emitRendererDataChanged('manuscripts', { action: 'save-xhs-note', entityId: snapshot.relativePath });
    return { success: true, ...snapshot, xhsNote: snapshot.document };
  } catch (error) {
    return { success: false, error: error instanceof Error ? error.message : String(error) };
  }
...
ipcMain.handle('manuscripts:bind-xhs-note-media', async (_, payload: {
  path: string;
  slotId: string;
  assetId: string;
  expectedRevision?: number;
  ...
  try {
    const snapshot = await bindXhsNoteMedia(payload);
    emitRendererDataChanged('manuscripts', { action: 'bind-xhs-note-media', entityId: snapshot.relativePath });
    emitRendererDataChanged('media', { action: 'bind', entityId: payload.assetId });
    return { success: true, ...snapshot, xhsNote: snapshot.document };
  } catch (error) {
    return { success: false, error: error instanceof Error ? error.message : String(error) };
  }
...
ipcMain.handle('manuscripts:export-xhs-material-package', async (_, payload: {
  path: string;
  outputPath?: string;
}) => {
  try {
    let outputPath = String(payload?.outputPath || '').trim();
    let allowExternalOutput = false;
    if (!outputPath) {
      const snapshot = await getXhsNoteProject(payload.path);
      const safeTitle = String(snapshot.document.finalTitle || '小红书笔记').replace(/[<>:"/\\|?*]/g, '-');
      const picker = await dialog.showSaveDialog({
        title: '导出小红书素材包',
        defaultPath: path.join(app.getPath('downloads'), `${safeTitle}-素材包.zip`),
        filters: [{ name: 'ZIP Archive', extensions: ['zip'] }],
      });
      if (picker.canceled || !picker.filePath) return { success: false, canceled: true };
      outputPath = picker.filePath;
      allowExternalOutput = true;
    }
    const result = await exportXhsMaterialPackage({
      path: payload.path,
      outputPath,
      allowExternalOutput,
    });
  ...
  try {
    if (isPackage) {
      ...
      return {
        ...
        contentModel: String(metadata?.contentModel || '').trim() || undefined,
      };
    }
      ...
      if (isManuscriptPackageName(entry.name)) {
        ...
        try {
          result.push({
            ...
            contentModel: metadata.contentModel,
          });
        } catch {
          result.push({
            name: entry.name,
...
ipcMain.handle('manuscripts:save', async (_, { path: filePath, content, metadata }: { path: string; content: string; metadata?: any }) => {
  ...
  try {
    const stat = await fs.stat(fullPath);
    if (stat.isDirectory()) {
      if (isStructuredXhsNoteProject(fullPath)) {
        return { success: false, error: '结构化小红书笔记不能通过普通 Markdown 保存，请使用 saveXhsNote' };
      }
...
ipcMain.handle('manuscripts:delete', async (_, filePath: string) => {
  ...
  try {
    ...
    clearDeletedXhsSessionBindings(fullPath);
    invalidateManuscriptTreeCache(getManuscriptsDir());
...
ipcMain.handle('knowledge:get-item-detail', async (_, payload?: {
  ...
  if (kind === 'redbook-note') {
    ...
    const commentsSnapshot = Array.isArray(meta.commentsSnapshot)
      ? meta.commentsSnapshot.filter((comment: any) => comment && (comment.author || comment.text || comment.content))
      : [];
    const sourceUrl = String(meta.sourceUrl || item.sourceUrl || '');
    ...
    return {
      ...
      commentsSnapshot,
      xhsComments: buildXhsCommentsSnapshotView(commentsSnapshot, {
        noteId: item.id,
        sourceLink: sourceUrl,
        total: Number(meta.stats?.comments || commentsSnapshot.length),
      }),
...
ipcMain.handle('wander:get-random', async () => {
  try {
    return await getRandomWanderItems(3);
  } catch (error) {
...
ipcMain.handle('wander:list-comment-candidates', async () => {
  try {
    return await listWanderCommentCandidates();
  } catch (error) {
    console.error('Failed to list wander comment candidates:', error);
    return [];
  }
});
...
type WanderBrainstormRequestOptions = {
  ...
  deepThink?: boolean;
  requestId?: string;
  sourceMode?: 'inspiration' | 'comment_insight' | 'random' | 'comments';
  subjectIds?: string[];
};

type WanderBrainstormRequestItem = {
  id: string;
  type: 'note' | 'video';
  title: string;
  content: string;
...
type WanderBrainstormRequestPayload = WanderBrainstormRequestItem[] | {
  items?: WanderBrainstormRequestItem[];
...
async function handleWanderBrainstormRequest(
  ...
  const payload = itemsOrPayload && typeof itemsOrPayload === 'object' && !Array.isArray(itemsOrPayload)
    ? itemsOrPayload
    : null;
  const rawItems = Array.isArray(itemsOrPayload)
    ? itemsOrPayload
  ...
  try {
    const run = await runWanderBrainstorm({
      requestId,
      items,
      multiChoice: effectiveOptions.multiChoice,
      deepThink: effectiveOptions.deepThink,
      sourceMode: effectiveOptions.sourceMode,
      subjectIds: effectiveOptions.subjectIds,
      reportProgress,
    });
    return {
...
type KnowledgeEntryPayload = BrowserKnowledgeEntryPayload;
...
const browserCaptureOperationResults = new BrowserCaptureOperationCache(256);
const forbiddenBrowserCaptureKeys = new Set([
  'authorization',
  'cookie',
  'cookies',
  'password',
...
function assertBrowserCapturePayloadSafe(value: unknown, depth = 0): void {
  if (depth > 20) {
    throw bridgePayloadError('INVALID_CAPTURE_PAYLOAD', '采集内容嵌套层级过深');
  }
  if (Array.isArray(value)) {
    for (const item of value) assertBrowserCapturePayloadSafe(item, depth + 1);
    return;
  }
  if (!value || typeof value !== 'object') return;
  for (const [key, child] of Object.entries(value as Record<string, unknown>)) {
    const normalizedKey = key.replace(/[^a-z0-9]/gi, '').toLowerCase();
    if (forbiddenBrowserCaptureKeys.has(normalizedKey)) {
      throw bridgePayloadError('AUTH_DATA_NOT_ALLOWED', `采集请求不能包含认证字段：${key}`);
    }
    assertBrowserCapturePayloadSafe(child, depth + 1);
  }
}

function validateBrowserKnowledgeEntry(entry: BrowserKnowledgeEntryPayload): BrowserKnowledgeEntryPayload {
  if (!entry || typeof entry !== 'object' || Array.isArray(entry)) {
    throw bridgePayloadError('INVALID_CAPTURE_PAYLOAD', '知识库条目格式无效');
  }
  const sourceUrl = String(entry.source?.sourceLink || entry.source?.sourceUrl || '').trim();
  if (sourceUrl && !/^https?:\/\//i.test(sourceUrl)) {
    throw bridgePayloadError('INVALID_SOURCE_URL', '采集来源必须是 HTTP(S) 网页');
  }
  const assetSources = [
    entry.assets?.coverUrl,
    entry.assets?.thumbnailUrl,
    entry.assets?.videoUrl,
    ...(Array.isArray(entry.assets?.imageUrls) ? entry.assets.imageUrls : []),
  ].filter((item): item is string => typeof item === 'string' && Boolean(item.trim()));
  for (const assetSource of assetSources) validateRemoteMediaSource(assetSource);
  const hasContent = Boolean(
    String(entry.content?.title || '').trim()
    || String(entry.content?.text || entry.content?.description || entry.content?.html || '').trim()
    || assetSources.length > 0
  );
  if (!hasContent) {
    throw bridgePayloadError('CONTENT_NOT_ACCESSIBLE', '当前页面没有可保存的可见内容');
  }
  return entry;
}

async function handleBrowserCaptureBridgeRequest(
  method: string,
  params: Record<string, unknown>,
  _context: BrowserCaptureBridgeRequestContext,
): Promise<unknown> {
  if (method === 'desktop.health') {
    await Promise.all([ensureKnowledgeRedbookDir(), ensureKnowledgeYoutubeDir()]);
    ...
    return {
      ...
      counts: {
        redbook: countTruthyDirectoryEntries(redbookDirs.filter((entry) => entry.isDirectory())),
        youtube: countTruthyDirectoryEntries(youtubeDirs.filter((entry) => entry.isDirectory())),
        media: mediaAssets.length,
      },
    };
  }

  const operationId = String(params.operationId || '').trim().slice(0, 500);
  if (!operationId) {
    throw bridgePayloadError('OPERATION_ID_REQUIRED', '写入请求缺少 operationId');
  }
  const operationKey = `${method}:${operationId}`;
  return browserCaptureOperationResults.run(operationKey, async () => {
    const payload = params.payload;
    if (!payload || typeof payload !== 'object' || Array.isArray(payload)) {
      throw bridgePayloadError('INVALID_CAPTURE_PAYLOAD', '采集请求 payload 格式无效');
    }
    assertBrowserCapturePayloadSafe(payload);

    let result: unknown;
    if (method === 'knowledge.ingestEntry') {
      const persisted = await persistKnowledgeEntry(validateBrowserKnowledgeEntry(payload as BrowserKnowledgeEntryPayload));
      if (!persisted.success) throw bridgePayloadError('KNOWLEDGE_INGEST_FAILED', persisted.error || '内容入库失败');
      result = {
        entryId: persisted.entryId,
        duplicate: Boolean(persisted.duplicate),
        updated: Boolean(persisted.updated),
      };
    } else if (method === 'knowledge.ingestXhsEntryV2') {
      const normalized = normalizeXhsV2Entry(payload);
      const persisted = await persistKnowledgeEntry(validateBrowserKnowledgeEntry(normalized.entry));
      if (!persisted.success) throw bridgePayloadError('KNOWLEDGE_INGEST_FAILED', persisted.error || '小红书内容入库失败');
      result = {
        entryId: persisted.entryId,
        duplicate: Boolean(persisted.duplicate),
        updated: Boolean(persisted.updated),
        comments: { captured: normalized.capturedComments },
      };
    } else if (method === 'knowledge.ingestZhihuAnswer' || method === 'knowledge.ingestZhihuArticle') {
      const normalized = method === 'knowledge.ingestZhihuAnswer'
        ? normalizeZhihuAnswerEntry(payload)
        : normalizeZhihuArticleEntry(payload);
      const persisted = await persistKnowledgeEntry(validateBrowserKnowledgeEntry(normalized));
      if (!persisted.success) throw bridgePayloadError('KNOWLEDGE_INGEST_FAILED', persisted.error || '知乎内容入库失败');
      result = {
        entryId: persisted.entryId,
        duplicate: Boolean(persisted.duplicate),
        updated: Boolean(persisted.updated),
      };
    } else if (method === 'knowledge.ingestMediaAssets') {
      ...
      const items = (Array.isArray(payloadRecord.items) ? payloadRecord.items : [])
        ...
        .map((item) => item && typeof item === 'object' && !Array.isArray(item)
          ...
          : {})
        .map((item) => ({
          title: String(item.title || '').trim().slice(0, 2_000) || undefined,
          source: validateRemoteMediaSource(item.source),
        }));
      if (items.length === 0) throw bridgePayloadError('CONTENT_NOT_ACCESSIBLE', '没有可导入的媒体资源');
...
async function persistXhsNote(note: any): Promise<{ success: boolean; noteId?: string; error?: string }> {
  ...
  try {
    ...
    win?.webContents.send('knowledge:new-note', { noteId, title: meta.title });
    emitKnowledgeCatalogChanged({ action: 'create', entryId: noteId, kind: 'xhs-note' });
...
async function persistYoutubeNote(payload: {
  ...
  try {
    ...
    emitKnowledgeCatalogChanged({ action: 'create', entryId: noteId, kind: 'youtube-video' });
    ...
    (async () => {
      ...
      emitKnowledgeCatalogChanged({ action: 'update', entryId: noteId, kind: 'youtube-video' });
    })().catch(err => {
