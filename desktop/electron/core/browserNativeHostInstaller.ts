...
async function resolveHostExecutable(options: BrowserNativeHostInstallOptions): Promise<string> {
    ...
    if (process.platform === 'win32') {
        throw new Error('Windows 开发模式不能注册 Native Host，请使用打包后的 Bojin 验证');
    }
    const launcherPath = path.join(browserCaptureStateRoot(), `${BROWSER_CAPTURE_NATIVE_HOST_NAME}.dev-launcher.sh`);
    const script = [
        '#!/bin/sh',
        '# Generated by Bojin for local browser-extension development.',
...
function manifest(hostPath: string) {
    return {
        name: BROWSER_CAPTURE_NATIVE_HOST_NAME,
        description: 'Bojin local content capture bridge',
        path: hostPath,
        type: 'stdio',
