...
export class SkillManager {
    ...
    private async discoverBuiltinSkills(): Promise<SkillDefinition[]> {
        ...
        const cwdRootBuiltin = path.join(process.cwd(), 'builtin-skills');
        const runtimeBuiltin = path.join(__dirname, 'builtin-skills');
        const runtimeParentBuiltin = path.join(__dirname, '..', 'builtin-skills');
        const sourceElectronBuiltin = path.join(__dirname, '..', '..', 'electron', 'builtin-skills');
        const sourceRootBuiltin = path.join(__dirname, '..', '..', 'builtin-skills');
        const runtimeSystem = path.join(__dirname, '..', 'system-skills');
        const packagedSystem = path.join(__dirname, '..', '..', 'electron', 'system-skills');
        // Later dirs override earlier ones. Put live source last so `pnpm dev`
        // does not keep a stale copy from dist-electron/builtin-skills.
        const candidates = [
            cwdRootBuiltin,
            cwdBuiltin,
            cwdSystem,
            runtimeBuiltin,
            runtimeParentBuiltin,
            sourceRootBuiltin,
            runtimeSystem,
            packagedSystem,
            sourceElectronBuiltin,
            cwdElectronBuiltin,
        ];
