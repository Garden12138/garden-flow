...
import {
  ...
  DASHSCOPE_MINIMAX_VOICE_QUERY_MODEL,
...
class AudioProviderRequestError extends Error {
  ...
  constructor(message: string, options: { providerCode?: string; statusCode: number; terminal?: boolean }) {
    super(message);
    this.name = 'AudioProviderRequestError';
...
function createAudioProviderHttpError(statusCode: number, rawBody: string): AudioProviderRequestError {
  let providerCode = '';
  let providerMessage = '';
  try {
    const parsed = JSON.parse(rawBody) as Record<string, unknown>;
    providerCode = text(parsed.code) || text(parsed.error_code);
    providerMessage = text(parsed.message) || text(parsed.error_message);
  } catch {
    providerMessage = rawBody.trim();
  }
  const normalizedMessage = providerMessage.toLowerCase();
  const productNotActivated = providerCode.toLowerCase() === 'invalidparameter'
    && normalizedMessage.includes('product is not activated');
  if (productNotActivated) {
    return new AudioProviderRequestError(
      `阿里云百炼尚未开通 MiniMax 模型服务（HTTP ${statusCode} · ${providerCode}）。请在华北2（北京）的模型市场开通 MiniMax，并确认当前 API Key 也创建于北京地域。`,
      { providerCode, statusCode, terminal: true },
    );
  }
  const detail = providerMessage || rawBody.trim();
  return new AudioProviderRequestError(
    `语音生成失败 (${statusCode})${providerCode ? ` [${providerCode}]` : ''}: ${detail}`,
...
async function synthesizeSpeechAsset(request: Record<string, unknown>): Promise<{
  ...
  const route = resolveAudioModelRoute(settings, requestedModel);
  const model = route.model;
  const responseFormat = text(request.responseFormat) || text(request.response_format) || text(request.format) || 'mp3';
  const routeEndpoint = text(request.endpoint) || text(request.baseURL) || route.baseURL;
  const apiKey = text(request.apiKey)
    || text(request.voiceApiKey)
    || text(request.ttsApiKey)
    || route.apiKey;
  const isDashScopeMiniMax = isDashScopeMiniMaxSpeechRoute(model, routeEndpoint);
  const voiceId = text(request.voiceId)
    || text(request.voice_id)
    || text(request.voice)
    || (isDashScopeMiniMax ? DASHSCOPE_MINIMAX_DEFAULT_VOICE_ID : 'alloy');
  const endpoint = isDashScopeMiniMax
    ? buildDashScopeMultimodalGenerationUrl(routeEndpoint)
    : audioEndpoint(routeEndpoint);
  ...
  const body: Record<string, unknown> = isDashScopeMiniMax
    ? buildDashScopeMiniMaxSpeechRequest({
        model,
        text: input,
        voiceId,
        speed: Number.isFinite(speed) ? speed : undefined,
        languageBoost: text(request.languageBoost) || text(request.language_boost),
        emotion: text(request.emotion),
        responseFormat,
      })
    : {
        model,
        ...
        ...(text(request.languageBoost) ? { language_boost: text(request.languageBoost) } : {}),
        ...(text(request.emotion) ? { emotion: text(request.emotion) } : {}),
      };
  ...
  if (!response.ok) {
    ...
    throw createAudioProviderHttpError(response.status, errorBody);
  }
  const audio = isDashScopeMiniMax
    ? await (async () => {
        let parsed: unknown;
        try {
          parsed = await response.json();
        } catch {
          throw new Error('DashScope MiniMax 返回了无法解析的 JSON');
        }
        const result = parseDashScopeMiniMaxSpeechResponse(parsed);
        if (result.encoding === 'url') {
          const audioResponse = await fetch(result.audio);
          if (!audioResponse.ok) throw new Error(`下载语音结果失败 (${audioResponse.status})`);
          return {
            bytes: Buffer.from(await audioResponse.arrayBuffer()),
            mimeType: text(audioResponse.headers.get('content-type')) || extensionForFormat(result.format).mimeType,
          };
        }
        if (!/^(?:[0-9a-f]{2})+$/i.test(result.audio)) {
          throw new Error('DashScope MiniMax 返回了无效的 hex 音频数据');
        }
        return {
          bytes: Buffer.from(result.audio, 'hex'),
          mimeType: extensionForFormat(result.format).mimeType,
        };
      })()
    : await parseAudioResponse(response, responseFormat);
  const asset = await createGeneratedMediaAsset({
    prompt: input,
    dataBuffer: audio.bytes,
    mimeType: audio.mimeType,
    projectId: optionalText(request.projectId),
    provider: isDashScopeMiniMax ? 'dashscope' : 'voice',
    providerTemplate: isDashScopeMiniMax ? 'dashscope-minimax-tts' : 'tts',
    model,
    ...
    try {
      ...
      if (job.kind === 'image') {
        const imageResult = await generateImagesToMediaLibrary({
          ...
          endpoint: optionalText(request.endpoint) || optionalText(request.baseURL),
        ...
        const videoResult = await generateVideosToMediaLibrary({
          ...
          generationMode: ['text-to-video', 'reference-guided', 'first-last-frame', 'continuation'].includes(text(request.generationMode))
            ...
            : undefined,
          referenceImages: Array.isArray(request.referenceImages) ? request.referenceImages as string[] : [],
          referenceAudios: Array.isArray(request.referenceAudios) ? request.referenceAudios as string[] : [],
...
export async function listVoices(payload: Record<string, unknown> = {}): Promise<Record<string, unknown>> {
  const settings = readSettings();
  const requestedModel = text(payload.model) || text(payload.ttsModel);
  const route = resolveAudioModelRoute(settings, requestedModel);
  const model = route.model;
  const routeEndpoint = text(payload.endpoint) || text(payload.baseURL) || route.baseURL;
  const isDashScopeMiniMax = isDashScopeMiniMaxSpeechRoute(model, routeEndpoint);
  const endpoint = isDashScopeMiniMax
    ? buildDashScopeMultimodalGenerationUrl(routeEndpoint)
    : voicesEndpoint(routeEndpoint);
  const storedLocal: Array<Record<string, unknown>> = Array.isArray(settings.voice_list_json)
    ? settings.voice_list_json.filter((item): item is Record<string, unknown> => Boolean(item && typeof item === 'object'))
    : [];
  const local: Array<Record<string, unknown>> = isDashScopeMiniMax
    ? [createDashScopeMiniMaxFallbackVoice(model), ...storedLocal]
    : storedLocal.length > 0 ? storedLocal : DEFAULT_VOICES;
  ...
  const apiKey = text(payload.apiKey)
    || text(payload.voiceApiKey)
    || text(payload.ttsApiKey)
    || route.apiKey;
  try {
    const url = new URL(endpoint);
    if (!isDashScopeMiniMax && model) url.searchParams.set('model', model);
    const response = await fetch(url, isDashScopeMiniMax ? {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        input: { action: 'get_voice', voice_type: 'all' },
        model: DASHSCOPE_MINIMAX_VOICE_QUERY_MODEL,
      }),
    } : {
    ...
    const remote: Array<Record<string, unknown>> = isDashScopeMiniMax
      ? parseDashScopeMiniMaxVoices(parsed, model).map((item) => ({ ...item }))
      : parseVoiceItems(parsed);
...
export async function synthesizeVoiceSpeech(payload: Record<string, unknown>): Promise<Record<string, unknown>> {
  try {
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : String(error),
      providerErrorCode: error instanceof AudioProviderRequestError ? error.providerCode : undefined,
      statusCode: error instanceof AudioProviderRequestError ? error.statusCode : undefined,
      terminal: error instanceof AudioProviderRequestError ? error.terminal : false,
    };
  }
}
