...
const RESPONSE_STYLE_PROMPT = loadPrompt('response_style.txt', '');
...
export function getCoreSystemPrompt(options: SystemPromptOptions): string {
    ...
    sections.push(RESPONSE_STYLE_PROMPT);
...
function getSkillsSection(skills: SkillDefinition[]): string {
    const hasVideoDirector = skills.some((skill) => !skill.disabled && skill.name === 'video-director');
    ...
    if (hasVideoDirector) {
        lines.push(
            ...
            '- If the task is to generate a video, animated shot, motion clip, reference-image video, or first/last-frame transition, you must load `video-director` before using any video tool.',
            '- Once `video-director` is loaded, you must strictly follow its workflow.',
