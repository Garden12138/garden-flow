...
interface GardenFlowOnboardingState {
  version: number;
  startedAt?: string;
  updatedAt: string;
...
export interface GardenFlowProfilePromptBundle {
  profileRoot: string;
  onboardingState: GardenFlowOnboardingState;
  files: {
    agent: string;
    soul: string;
    identity: string;
...
export type GardenFlowProfileDocType = 'agent' | 'soul' | 'user' | 'creator_profile';

export interface GardenFlowOnboardingTurnResult {
  handled: boolean;
  responseText?: string;
  completed?: boolean;
}

const PROFILE_DIR = path.join('gardenflow', 'profile');
...
const ONBOARDING_STEPS: OnboardingStep[] = [
  {
    key: 'assistant_style',
    question: '1/5 先定一下我的协作风格。你希望 GardenFlow 在对话里更偏向哪种风格？例如：高执行、强结构、温和陪跑、直接批判。',
...
function defaultOnboardingState(): GardenFlowOnboardingState {
  return {
    version: 1,
    updatedAt: nowIso(),
...
function buildDefaultAgentTemplate(): string {
  return [
    '# Agent.md',
    '',
    '你是 GardenFlow，服务于 GardenFlow 的多平台内容创作执行 Agent。',
    '',
    '## 启动顺序（每次会话）',
    '1. 读取 Soul.md（你的行为风格）',
    ...
    '',
    '## 核心档案职责',
    '- Soul.md：维护 GardenFlow 的协作语气、反馈方式、执行风格。',
    '- user.md：维护用户的稳定画像与长期事实。',
    '- CreatorProfile.md：维护用户的长期自媒体定位、目标群体、风格、商业目标与运营边界。',
    '- Agent.md：维护 GardenFlow 的工作契约、流程和规则，不为一次性任务随意改写。',
    '',
    '## 创作流程',
...
function buildDefaultSoulTemplate(): string {
  return [
    ...
    '',
    '## 什么时候更新本文件',
    '- 用户明确要求 GardenFlow 改变沟通方式、反馈力度、协作氛围时更新。',
...
function buildDefaultCreatorProfileTemplate(): string {
  return [
    ...
    '',
    '## 维护规则',
    '- 本文档是用户长期自媒体策略档案，每次 GardenFlow 会话都应优先参考。',
...
async function loadOnboardingState(): Promise<GardenFlowOnboardingState> {
  const filePath = getFilePath(ONBOARDING_STATE_FILE);
  const parsed = await readJson<Partial<GardenFlowOnboardingState>>(filePath, {});
  return {
...
async function saveOnboardingState(state: GardenFlowOnboardingState): Promise<void> {
...
function getAnswer(state: GardenFlowOnboardingState, key: string, fallback: string): string {
  const value = normalizeAnswer(state.answers[key] || '');
  return value || fallback;
}

async function finalizeOnboarding(state: GardenFlowOnboardingState): Promise<void> {
  ...
  try {
    await addUserMemoryToFile(`用户GardenFlow协作偏好: ${style}`, 'preference', ['gardenflow', 'onboarding']);
    await addUserMemoryToFile(`用户小红书创作主目标: ${goal}`, 'fact', ['gardenflow', 'goal']);
    await addUserMemoryToFile(`用户目标受众: ${audience}`, 'fact', ['gardenflow', 'audience']);
    await addUserMemoryToFile(`用户内容赛道偏好: ${lane}`, 'preference', ['gardenflow', 'content-lane']);
    await addUserMemoryToFile(`用户语气与边界: ${constraints}`, 'preference', ['gardenflow', 'constraints']);
  } catch (error) {
    console.warn('[gardenflowProfileStore] Failed to write onboarding memories:', error);
  }
}

export async function ensureGardenFlowProfileFiles(): Promise<void> {
...
export async function loadGardenFlowProfilePromptBundle(): Promise<GardenFlowProfilePromptBundle> {
  await ensureGardenFlowProfileFiles();
...
export async function updateGardenFlowCreatorProfile(markdown: string): Promise<{ path: string; content: string }> {
  await ensureGardenFlowProfileFiles();
...
export async function updateGardenFlowProfileDocument(
  docType: GardenFlowProfileDocType,
  markdown: string,
): Promise<{ path: string; content: string; docType: GardenFlowProfileDocType }> {
  await ensureGardenFlowProfileFiles();

  const mapping: Record<GardenFlowProfileDocType, { fileName: string; title: string }> = {
...
export async function handleGardenFlowOnboardingTurn(userInput: string): Promise<GardenFlowOnboardingTurnResult> {
  await ensureGardenFlowProfileFiles();
  ...
  if (!state.askedFirstQuestion) {
    ...
    return {
      handled: true,
      responseText: [
        '在开始创作前，我们先做一次 GardenFlow 个性化设定（只需 1-2 分钟）。',
  ...
  if (isSkipCommand(normalized)) {
    ...
    return {
      handled: true,
      completed: true,
      responseText: '已按默认配置完成 GardenFlow 设定，并写入当前空间档案与长期记忆。现在可以直接给我创作目标。',
    };
  }
...
export async function ensureGardenFlowOnboardingCompletedWithDefaults(): Promise<boolean> {
  await ensureGardenFlowProfileFiles();
  const state = await loadOnboardingState();
  if (state.completedAt) {
    return false;
...
export async function startGardenFlowStyleDefinition(forceRestart = false): Promise<GardenFlowOnboardingState> {
  await ensureGardenFlowProfileFiles();
  const state = await loadOnboardingState();
...
export async function saveGardenFlowInitializationProgress(
  stepIndex: number,
  answers: Record<string, unknown>,
): Promise<GardenFlowOnboardingState> {
  await ensureGardenFlowProfileFiles();
  const state = await loadOnboardingState();
...
export async function completeGardenFlowInitialization(answers: Record<string, unknown>): Promise<GardenFlowOnboardingState> {
  const state = await saveGardenFlowInitializationProgress(ONBOARDING_STEPS.length, answers);
  state.askedFirstQuestion = true;
  state.stepIndex = ONBOARDING_STEPS.length;
  await finalizeOnboarding(state);
  return state;
...
export async function completeGardenFlowStyleDefinition(payload: Record<string, unknown>): Promise<GardenFlowProfilePromptBundle> {
  await ensureGardenFlowProfileFiles();
  ...
  if (creatorProfile) await updateGardenFlowProfileDocument('creator_profile', creatorProfile);
  if (soul) await updateGardenFlowProfileDocument('soul', soul);
  if (payload.answers && typeof payload.answers === 'object') {
    await completeGardenFlowInitialization(payload.answers as Record<string, unknown>);
  }
  return loadGardenFlowProfilePromptBundle();
}
