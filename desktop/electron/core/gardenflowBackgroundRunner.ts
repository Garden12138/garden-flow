...
import { getGardenFlowProject, listGardenFlowProjects } from './gardenflowStore';
...
import { getHeadlessTaskSupervisor, type HeadlessTaskBackoffConfig } from './headlessTaskSupervisor';
...
export interface GardenFlowScheduledTask {
  id: string;
  name: string;
  enabled: boolean;
  mode: ScheduleMode;
...
export interface GardenFlowLongCycleTask {
  id: string;
  name: string;
  enabled: boolean;
...
export interface GardenFlowHeartbeatConfig {
  enabled: boolean;
  intervalMinutes: number;
...
/**
 * 内置任务遵循“定义在代码、状态在配置”模型：
 * 这里只持久化用户态（开关、频率覆盖、配置项、执行结果），任务定义随版本升级。
 */
export interface GardenFlowBuiltinTaskState {
  enabled: boolean;
...
export interface GardenFlowBuiltinTaskProjection extends GardenFlowBuiltinTaskState {
  id: string;
  name: string;
  description: string;
  source: 'builtin';
...
interface GardenFlowBackgroundConfig {
  ...
  projectStates: Record<string, GardenFlowBackgroundProjectState>;
  heartbeat: GardenFlowHeartbeatConfig;
  scheduledTasks: Record<string, GardenFlowScheduledTask>;
  longCycleTasks: Record<string, GardenFlowLongCycleTask>;
  builtinTasks: Record<string, GardenFlowBuiltinTaskState>;
}

export interface GardenFlowBackgroundRunnerStatus {
  enabled: boolean;
  ...
  scheduledTasks: Record<string, GardenFlowScheduledTask>;
  longCycleTasks: Record<string, GardenFlowLongCycleTask>;
  builtinTasks: Record<string, GardenFlowBuiltinTaskProjection>;
}

const DEFAULT_HEARTBEAT: GardenFlowHeartbeatConfig = {
  enabled: true,
  intervalMinutes: 30,
...
const DEFAULT_CONFIG: GardenFlowBackgroundConfig = {
  enabled: false,
  intervalMinutes: 20,
...
function buildCronExpressionForScheduledTask(task: GardenFlowScheduledTask): string | null {
...
function normalizeScheduledTask(
  taskId: string,
  raw: Partial<GardenFlowScheduledTask> | undefined,
  now: string,
): GardenFlowScheduledTask | null {
  ...
  const normalized: GardenFlowScheduledTask = {
    id,
    ...
    source: normalizeAutomationTaskSource(raw?.source) || 'manual',
    createdAt,
    updatedAt,
...
function normalizeLongCycleTask(
  taskId: string,
  raw: Partial<GardenFlowLongCycleTask> | undefined,
  now: string,
): GardenFlowLongCycleTask | null {
  const id = String(taskId || raw?.id || '').trim();
  if (!id) return null;
...
function normalizeBuiltinTaskState(
  definition: BuiltinAutomationDefinition,
  raw: Partial<GardenFlowBuiltinTaskState> | undefined,
): GardenFlowBuiltinTaskState {
  const scheduleRaw = raw?.schedule as { mode?: string; time?: string } | undefined;
  const time = sanitizeTimeHHmm(scheduleRaw?.time);
  return {
    enabled: raw?.enabled === true,
    schedule: time ? { mode: 'daily', time } : undefined,
    settings: normalizeBuiltinTaskSettings(definition, raw?.settings),
    lastRunAt: typeof raw?.lastRunAt === 'string' ? raw.lastRunAt : undefined,
    lastResult: raw?.lastResult,
    lastError: typeof raw?.lastError === 'string' ? raw.lastError : undefined,
    nextRunAt: typeof raw?.nextRunAt === 'string' ? raw.nextRunAt : undefined,
...
function normalizeConfig(raw: Partial<GardenFlowBackgroundConfig> | null | undefined): GardenFlowBackgroundConfig {
  const now = nowIso();

  const projectStates = raw?.projectStates || {};
  const normalizedProjectStates: Record<string, GardenFlowBackgroundProjectState> = {};
  ...
  const normalizedScheduledTasks: Record<string, GardenFlowScheduledTask> = {};
  ...
  const normalizedLongCycleTasks: Record<string, GardenFlowLongCycleTask> = {};
  ...
  const heartbeatRaw = (raw?.heartbeat || {}) as Partial<GardenFlowHeartbeatConfig>;
  const heartbeat: GardenFlowHeartbeatConfig = {
  ...
  const builtinTasksRaw = (raw?.builtinTasks || {}) as Record<string, Partial<GardenFlowBuiltinTaskState>>;
  const normalizedBuiltinTasks: Record<string, GardenFlowBuiltinTaskState> = {};
  for (const definition of listBuiltinAutomationDefinitions()) {
    normalizedBuiltinTasks[definition.id] = normalizeBuiltinTaskState(definition, builtinTasksRaw[definition.id]);
  }
  ...
  return {
    ...
    builtinTasks: normalizedBuiltinTasks,
  };
}
  ...
  if (!params.hasCopyPack) {
    return {
      ...
      message: [
        ...
        '',
        '请推进项目到“文案包已保存”状态：',
        '1) 先读取当前项目信息（优先 app_cli: gardenflow get --project-id ...）。',
        params.platform === 'wechat_official_account'
          ? '2) 生成标题候选、摘要、导语、正文、结尾 CTA、配图建议、发布计划。'
          : '2) 生成标题候选、正文、标签、封面文案、发布计划。',
        '3) 调用 gardenflow_save_copy_pack（或 app_cli gardenflow save-copy）落盘。',
        '4) 返回一句简要执行结果。',
      ].join('\n'),
    };
  }
  ...
  if (!params.hasImagePack) {
    return {
      ...
      message: [
        ...
        '1) 读取项目和已有文案。',
        '2) 生成封面与配图提示词。',
        '3) 调用 gardenflow_save_image_pack（或 app_cli gardenflow save-image）落盘。',
        '4) 返回一句简要执行结果。',
      ].join('\n'),
    };
  }
...
function computeNextRunForScheduledTask(task: GardenFlowScheduledTask, referenceMs: number): string | null {
...
function ensureScheduledTaskNextRun(task: GardenFlowScheduledTask, nowMs: number): void {
...
function isScheduledTaskDue(task: GardenFlowScheduledTask, nowMs: number): boolean {
...
function ensureLongCycleNextRun(task: GardenFlowLongCycleTask, nowMs: number): void {
...
function builtinTaskScheduleTime(
  definition: BuiltinAutomationDefinition,
...
function ensureBuiltinTaskNextRun(
  definition: BuiltinAutomationDefinition,
  state: GardenFlowBuiltinTaskState,
  nowMs: number,
): void {
  ...
  if (state.nextRunAt && parseIsoMs(state.nextRunAt) !== null) {
    return;
  }
  const baseMs = parseIsoMs(state.lastRunAt || '') ?? nowMs;
  state.nextRunAt = computeNextRunForBuiltinTask(definition, state, baseMs) || undefined;
}

function isBuiltinTaskDue(state: GardenFlowBuiltinTaskState, nowMs: number): boolean {
  if (!state.enabled) return false;
  const nextRunMs = parseIsoMs(state.nextRunAt || '');
  if (nextRunMs === null) return false;
  return nowMs >= nextRunMs;
}

function isLongCycleTaskDue(task: GardenFlowLongCycleTask, nowMs: number): boolean {
  if (!task.enabled) return false;
...
export class GardenFlowBackgroundRunner extends EventEmitter {
  private config: GardenFlowBackgroundConfig = { ...DEFAULT_CONFIG };
  ...
  private readonly scheduledTaskScheduler = new BackgroundCronScheduler<GardenFlowScheduledTask>();
  private readonly longCycleTaskScheduler = new BackgroundCronScheduler<GardenFlowLongCycleTask & { mode: 'interval'; recurring: true }>();
  private heartbeatInFlight = false;
  private readonly lockOwnerId = `gardenflow-bg:${process.pid}:${Date.now().toString(36)}`;
  ...
  private getConfigPath(): string {
    return path.join(getWorkspacePaths().gardenflow, 'background-runner.json');
  }

  private getLockPath(): string {
    return path.join(getWorkspacePaths().gardenflow, 'background-runner.lock');
  }
  ...
  private createRuntimeTask(params: {
    ...
    const task = runtime.createInteractiveTask({
      ...
      metadata: {
        source: 'gardenflow-background-runner',
        ...params.metadata,
      },
    });
  ...
  private normalizeSchedules(nowMs: number): void {
    ...
    for (const definition of listBuiltinAutomationDefinitions()) {
      ...
      ensureBuiltinTaskNextRun(definition, state, nowMs);
    }
  ...
  private getNextAutomationFireAt(): string | null {
    if (backgroundAutomationHeld() || !this.config.enabled || this.lockState !== 'owner') {
      return null;
    }
    ...
    for (const state of Object.values(this.config.builtinTasks)) {
      if (!state.enabled) continue;
      const ms = parseIsoMs(state.nextRunAt);
  ...
  private async loadConfig(): Promise<void> {
    ...
    try {
      ...
      this.config = normalizeConfig(JSON.parse(raw) as Partial<GardenFlowBackgroundConfig>);
    } catch {
      this.config = normalizeConfig(DEFAULT_CONFIG);
    }

    if (backgroundAutomationHeld()) {
      this.isLoaded = true;
      this.emitStatus();
      return;
    }
  ...
  async init(): Promise<void> {
    ...
    if (backgroundAutomationHeld()) return;
  ...
  getStatus(): GardenFlowBackgroundRunnerStatus {
    return {
      ...
      builtinTasks: Object.fromEntries(
        listBuiltinAutomationDefinitions().map((definition) => [
          definition.id,
          this.projectBuiltinTask(definition, this.ensureBuiltinTaskState(definition)),
        ]),
      ),
    };
  }
  ...
  }): Promise<GardenFlowBackgroundRunnerStatus> {
    await this.ensureLoaded();
  ...
  private async syncScheduledTaskWorkItem(task: GardenFlowScheduledTask): Promise<void> {
  ...
  private async syncLongCycleTaskWorkItem(task: GardenFlowLongCycleTask): Promise<void> {
  ...
  private async ensureScheduledTaskWorkItem(task: GardenFlowScheduledTask): Promise<void> {
    const store = getWorkItemStore();
  ...
  private async ensureLongCycleTaskWorkItem(task: GardenFlowLongCycleTask): Promise<void> {
    const store = getWorkItemStore();
    const existingId = String(task.workItemId || '').trim();
    if (existingId) {
      const existing = await store.getWorkItem(existingId);
  ...
  listScheduledTasks(): GardenFlowScheduledTask[] {
  ...
  async addScheduledTask(input: {
    ...
    enabled?: boolean;
    source?: AutomationTaskSource;
  }): Promise<GardenFlowScheduledTask> {
    await this.ensureLoaded();
    ...
    const now = nowIso();
    const task: GardenFlowScheduledTask = {
      ...
      source: normalizeAutomationTaskSource(input.source) || 'manual',
      createdAt: now,
      updatedAt: now,
      nextRunAt: undefined,
    };
  ...
  async removeScheduledTask(taskId: string): Promise<GardenFlowBackgroundRunnerStatus> {
    await this.ensureLoaded();
    const id = String(taskId || '').trim();
    if (!id) throw new Error('taskId is required');
  ...
  async setScheduledTaskEnabled(taskId: string, enabled: boolean): Promise<GardenFlowScheduledTask> {
    await this.ensureLoaded();
    const id = String(taskId || '').trim();
    if (!id) throw new Error('taskId is required');
  ...
  private ensureBuiltinTaskState(definition: BuiltinAutomationDefinition): GardenFlowBuiltinTaskState {
    if (!this.config.builtinTasks) {
      this.config.builtinTasks = {};
    }
    const existing = this.config.builtinTasks[definition.id];
    if (existing) return existing;
    const created: GardenFlowBuiltinTaskState = {
      enabled: false,
      settings: normalizeBuiltinTaskSettings(definition, undefined),
    };
    this.config.builtinTasks[definition.id] = created;
    return created;
  }

  private projectBuiltinTask(
    definition: BuiltinAutomationDefinition,
    state: GardenFlowBuiltinTaskState,
  ): GardenFlowBuiltinTaskProjection {
    return {
      ...state,
      id: definition.id,
  ...
  async listBuiltinTasks(): Promise<GardenFlowBuiltinTaskProjection[]> {
    await this.ensureLoaded();
    const nowMs = Date.now();
    return listBuiltinAutomationDefinitions().map((definition) => {
      const state = this.ensureBuiltinTaskState(definition);
      ensureBuiltinTaskNextRun(definition, state, nowMs);
      return this.projectBuiltinTask(definition, state);
    });
  }

  async getBuiltinTaskReadiness(taskId: string): Promise<BuiltinAutomationReadinessReport> {
    await this.ensureLoaded();
    const definition = getBuiltinAutomationDefinition(taskId);
    if (!definition) throw new Error('Builtin automation task not found');
    const state = this.ensureBuiltinTaskState(definition);
    return definition.checkReadiness(state.settings);
  }

  async setBuiltinTaskEnabled(taskId: string, enabled: boolean): Promise<{
    task: GardenFlowBuiltinTaskProjection;
    readiness: BuiltinAutomationReadinessReport | null;
  }> {
    await this.ensureLoaded();
    const definition = getBuiltinAutomationDefinition(taskId);
    if (!definition) throw new Error('Builtin automation task not found');
    ...
    const readiness = await definition.checkReadiness(state.settings);
    if (!readiness.ready) {
      throw new Error(`内置任务未就绪，无法开启：${readiness.blockingReason}`);
    }
    ...
    state.nextRunAt = undefined;
    state.updatedAt = nowIso();
    ensureBuiltinTaskNextRun(definition, state, Date.now());
    await this.persistConfig();
    this.emitStatus();
    return { task: this.projectBuiltinTask(definition, state), readiness };
  }

  async setBuiltinTaskSettings(taskId: string, input: {
    settings?: Record<string, unknown>;
    scheduleTime?: string;
  }): Promise<GardenFlowBuiltinTaskProjection> {
    await this.ensureLoaded();
    const definition = getBuiltinAutomationDefinition(taskId);
    if (!definition) throw new Error('Builtin automation task not found');
    const state = this.ensureBuiltinTaskState(definition);
    ...
    if (input.settings !== undefined) {
      state.settings = normalizeBuiltinTaskSettings(definition, {
        ...state.settings,
        ...input.settings,
      });
    }
  ...
  async runBuiltinTaskNow(taskId: string): Promise<GardenFlowBuiltinTaskProjection> {
    await this.ensureLoaded();
    const definition = getBuiltinAutomationDefinition(taskId);
    if (!definition) throw new Error('Builtin automation task not found');
    if (this.currentAutomationTaskId) {
      throw new Error(
        this.currentAutomationTaskId === definition.id
          ? '该采集任务正在执行中。当前可能在操作一个过小或被挡住的 Chrome 窗口，看起来像没打开浏览器。请等待结束，或完全退出 GardenFlow 后再重新运行。'
          : `已有自动化任务正在执行（${this.currentAutomationTaskId}），请等待结束后再点立即运行。`,
      );
    }
    if (this.config.enabled && this.lockState !== 'owner') {
      await this.acquireOwnership();
    }
    await this.executeBuiltinTask(definition, 'manual');
    await this.persistConfig();
    this.emitStatus();
    return this.projectBuiltinTask(definition, this.ensureBuiltinTaskState(definition));
  }

  listLongCycleTasks(): GardenFlowLongCycleTask[] {
    const nowMs = Date.now();
  ...
  async addLongCycleTask(input: {
    ...
    enabled?: boolean;
    source?: AutomationTaskSource;
  }): Promise<GardenFlowLongCycleTask> {
    await this.ensureLoaded();
    ...
    const now = nowIso();
    const task: GardenFlowLongCycleTask = {
  ...
  async removeLongCycleTask(taskId: string): Promise<GardenFlowBackgroundRunnerStatus> {
    await this.ensureLoaded();
    const id = String(taskId || '').trim();
    if (!id) throw new Error('taskId is required');
  ...
  async setLongCycleTaskEnabled(taskId: string, enabled: boolean): Promise<GardenFlowLongCycleTask> {
    await this.ensureLoaded();
    const id = String(taskId || '').trim();
    if (!id) throw new Error('taskId is required');
  ...
  async stop(options?: { persist?: boolean }): Promise<GardenFlowBackgroundRunnerStatus> {
    await this.ensureLoaded();
    if (this.timer) {
      clearTimeout(this.timer);
      this.timer = null;
  ...
  async runNow(projectId?: string): Promise<GardenFlowBackgroundRunnerStatus> {
    await this.ensureLoaded();
    if (this.config.enabled && this.lockState !== 'owner') {
      await this.acquireOwnership();
    }
  ...
  async runScheduledTaskNow(taskId: string): Promise<GardenFlowBackgroundRunnerStatus> {
    await this.ensureLoaded();
    if (this.config.enabled && this.lockState !== 'owner') {
      await this.acquireOwnership();
    }
  ...
  async runLongCycleTaskNow(taskId: string): Promise<GardenFlowBackgroundRunnerStatus> {
    await this.ensureLoaded();
    if (this.config.enabled && this.lockState !== 'owner') {
      await this.acquireOwnership();
    }
  ...
  private async runAgentPrompt(params: {
    taskId?: string;
    taskKind?: 'gardenflow-project' | 'scheduled-task' | 'long-cycle' | 'heartbeat';
    workItemId?: string;
    metadata?: Record<string, unknown>;
    contextId: string;
    title: string;
    ...
    try {
      ...
      if (Array.isArray(params.skills) && params.skills.length > 0) {
        await service.activateSkills(params.skills);
      }
      ...
      const result = await getHeadlessTaskSupervisor().run({
        ...
        execute: (signal) => getHeadlessAgentRunner().runGardenFlowTask({
          taskId: effectiveTaskId,
          title: params.title,
          contextId: params.contextId,
      ...
      if (taskId) {
        await getBackgroundTaskRegistry().completeTask(taskId, 'headless gardenflow task completed');
      }
  ...
  private async runProject(projectId: string): Promise<void> {
    ...
    const { project, projectDir } = await getGardenFlowProject(projectId);
    const taskId = this.createRuntimeTask({
      runtimeMode: 'background-maintenance',
      intent: 'automation',
      roleId: 'ops-coordinator',
      ownerSessionId: `gardenflow-bg:${projectId}`,
    ...
    try {
      ...
      const result = await this.runAgentPrompt({
        contextId: `gardenflow-bg-${projectId}`,
        title: `GardenFlow BG ${project.goal.slice(0, 24)}`,
        contextContent: [
          `后台项目: ${projectId}`,
          `目标: ${project.goal}`,
          `平台: ${project.platform}`,
  ...
  private async runProjectTick(reason: 'scheduled' | 'manual' | 'init' | 'workspace-change', onlyProjectId?: string): Promise<void> {
    ...
    try {
      const projects = await listGardenFlowProjects(100);
  ...
  private buildScheduledTaskPrompt(task: GardenFlowScheduledTask): string {
  ...
  private async executeScheduledTask(taskId: string, reason: 'scheduled' | 'manual'): Promise<void> {
    ...
    const runtimeTaskId = this.createRuntimeTask({
      ...
      intent: 'automation',
      roleId: 'ops-coordinator',
      ownerSessionId: `gardenflow-scheduled:${task.id}`,
    ...
    try {
      ...
      const result = await this.runAgentPrompt({
        contextId: `gardenflow-schedule-${task.id}`,
        title: `GardenFlow Schedule ${task.name}`,
  ...
  private async executeBuiltinTask(
    ...
    if (this.currentAutomationTaskId) {
      const message = this.currentAutomationTaskId === definition.id
        ? '该采集任务正在执行中。当前可能在操作一个过小或被挡住的 Chrome 窗口，看起来像没打开浏览器。请等待结束，或完全退出 GardenFlow 后再重新运行。'
        : `已有自动化任务正在执行（${this.currentAutomationTaskId}），请等待结束后再点立即运行。`;
      if (reason === 'manual') {
      ...
      this.emit('log', {
        level: 'warn',
        message: `Builtin task skipped because another automation is running: ${definition.id} (active=${this.currentAutomationTaskId})`,
        reason,
        at: nowIso(),
      });
      return;
    ...
    if (!state.enabled) {
      this.emit('log', {
        level: 'info',
        message: `Builtin task skipped because it is disabled: ${definition.id}`,
        reason,
        at: nowIso(),
      });
      return;
    ...
    const runtimeTaskId = this.createRuntimeTask({
      ...
      intent: 'automation',
      roleId: 'ops-coordinator',
      ownerSessionId: `gardenflow-builtin:${definition.id}`,
      goal: `执行内置自动化任务 ${definition.name}`,
      userInput: definition.name,
      metadata: { builtinTaskId: definition.id, reason },
    });
    ...
    const settleNextRun = () => {
      state.nextRunAt = computeNextRunForBuiltinTask(definition, state, Date.now()) || undefined;
      state.updatedAt = nowIso();
    };
    ...
    try {
      ...
      if (!readiness.ready) {
        ...
        state.lastError = readiness.blockingReason || '内置任务未就绪';
        settleNextRun();
      ...
      if (definition.id === XHS_AUTO_CAPTURE_TASK_ID) {
        ...
        this.emit('log', {
          level: 'info',
          message: `Builtin task starting structured capture: ${definition.id} keyword=${launch.keyword}`,
          reason,
          at: nowIso(),
        });
        const round = await runXhsStructuredCaptureRound(
          {
            keyword: launch.keyword,
            maxNotes: launch.maxNotesPerRun,
            pacing: launch.pacing,
          },
          createXhsStructuredCaptureIo((level, message) => {
        ...
        if (round.status === 'blocked') {
          state.lastRunAt = nowIso();
          state.lastResult = 'skipped';
          state.lastError = round.reason || '采集被登录墙/安全验证阻断';
          settleNextRun();
          runtime.skipNode(runtimeTaskId, 'execute_tools', state.lastError);
          runtime.completeTask(runtimeTaskId, `builtin:${definition.id}:skipped`);
          this.emit('log', {
        ...
        if (round.status !== 'captured') {
          throw new Error(round.reason || '小红书采集未完成，不记为成功');
        }
        runtime.addCheckpoint(runtimeTaskId, 'execute_tools', '结构化采集完成', {
      ...
      this.emit('log', {
        level: 'info',
        message: `Builtin task starting a fresh session: ${definition.id}`,
        reason,
        at: nowIso(),
      });
      const result = await this.runAgentPrompt({
        contextId: `gardenflow-builtin-${definition.id}`,
        title: `GardenFlow Builtin ${definition.name}`,
        contextContent: [
          `内置任务ID: ${definition.id}`,
          `名称: ${definition.name}`,
          definition.description,
        ].join('\n'),
        prompt: definition.buildPrompt(state.settings),
        displayContent: `[内置自动化:${definition.name}]`,
        skills: definition.requiredSkills,
        freshSession: true,
        metadata: {
        ...
        backoff: {
          ...
          giveUpAfterMs: 16 * 60 * 1000,
        },
      });
      const latestAssistant = [...getChatMessages(result.sessionId)]
        .reverse()
        .find((msg) => msg.role === 'assistant' && String(msg.content || '').trim());
      if (!latestAssistant) {
        throw new Error('内置任务在未完成前结束（无助手小结），不记为成功');
      }
      this.mirrorLatestAssistantToMainSession(result.sessionId, `[内置自动化结果:${definition.name}]`);
      runtime.addCheckpoint(runtimeTaskId, 'execute_tools', '内置任务会话完成', {
        builtinTaskId: definition.id,
        sessionId: result.sessionId,
      });
      ...
      runtime.completeNode(runtimeTaskId, 'execute_tools', '内置任务执行完成');
      runtime.completeTask(runtimeTaskId, `builtin:${definition.id}:success`);
      this.emit('log', { level: 'info', message: `Builtin task completed: ${definition.id}`, reason, at: nowIso() });
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      state.lastRunAt = nowIso();
      ...
      this.lastError = message;
      runtime.failTask(runtimeTaskId, message, 'execute_tools');
      this.emit('log', { level: 'error', message: `Builtin task failed: ${definition.id}: ${message}`, reason, at: nowIso() });
    } finally {
      this.currentAutomationTaskId = null;
      this.emitStatus();
    }
  ...
  private buildLongCyclePrompt(task: GardenFlowLongCycleTask): string {
    return [
      '[GardenFlow 长周期任务]',
      `任务ID: ${task.id}`,
      `任务名: ${task.name}`,
  ...
  private async executeLongCycleTask(taskId: string, reason: 'scheduled' | 'manual'): Promise<void> {
    ...
    const runtimeTaskId = this.createRuntimeTask({
      ...
      roleId: 'ops-coordinator',
      ownerSessionId: `gardenflow-longcycle:${task.id}`,
    ...
    try {
      ...
      const result = await this.runAgentPrompt({
        contextId: `gardenflow-long-${task.id}`,
        title: `GardenFlow Long ${task.name}`,
        workItemId: task.workItemId,
        contextContent: [
          `长周期任务ID: ${task.id}`,
          `名称: ${task.name}`,
  ...
  private ensureMainGardenFlowSession() {
    ...
    const contextType = 'gardenflow';

    let session = getChatSessionByContext(contextId, contextType);
    if (!session) {
      session = createChatSession(
        `session_gardenflow_main_${activeSpaceId}`,
        `GardenFlow · ${activeSpaceId}`,
        {
          contextId,
          contextType,
          contextContent: `GardenFlow 主会话（空间：${activeSpaceId}）`,
          isContextBound: true,
        },
      );
    }
  ...
  private async runHeartbeat(reason: 'scheduled' | 'manual' | 'init' | 'workspace-change'): Promise<void> {
    ...
    const runtimeTaskId = this.createRuntimeTask({
      ...
      intent: 'automation',
      roleId: 'ops-coordinator',
      ownerSessionId: 'gardenflow-heartbeat',
      goal: '执行 GardenFlow 心跳汇报',
      userInput: 'heartbeat',
      metadata: { reason },
    });
    try {
      ...
      if (heartbeat.prompt) {
        ...
        await this.runAgentPrompt({
          contextId: `gardenflow-heartbeat:${getWorkspacePaths().activeSpaceId}`,
          title: 'GardenFlow Heartbeat',
          contextContent: '这是 GardenFlow 的后台心跳汇报会话。',
          prompt,
  ...
  private async runMaintenanceTick(reason: 'scheduled' | 'manual' | 'init' | 'workspace-change'): Promise<void> {
    ...
    try {
      ...
      const dueBuiltin = listBuiltinAutomationDefinitions()
        .map((definition) => ({ definition, state: this.ensureBuiltinTaskState(definition) }))
        .filter((entry) => isBuiltinTaskDue(entry.state, nowMs))
        .sort((a, b) => (parseIsoMs(a.state.nextRunAt || '') || 0) - (parseIsoMs(b.state.nextRunAt || '') || 0));

      if (reason === 'init' || reason === 'workspace-change') {
        for (const entry of dueBuiltin) {
          entry.state.nextRunAt = computeNextRunForBuiltinTask(entry.definition, entry.state, nowMs) || undefined;
          entry.state.updatedAt = nowIso();
          this.emit('log', {
            level: 'info',
            message: `Builtin task skipped catch-up on ${reason}: ${entry.definition.id}; next ${entry.state.nextRunAt || 'unscheduled'}`,
            reason,
            at: nowIso(),
          });
        }
        ...
        for (const entry of dueBuiltin) {
          if (automationBudget <= 0) break;
          await this.executeBuiltinTask(entry.definition, reason === 'manual' ? 'manual' : 'scheduled');
          automationBudget -= 1;
        }
      }
...
let globalRunner: GardenFlowBackgroundRunner | null = null;

export function getGardenFlowBackgroundRunner(): GardenFlowBackgroundRunner {
  if (!globalRunner) {
    globalRunner = new GardenFlowBackgroundRunner();
  }
  return globalRunner;
}
