...
import { resolveSettingsLlm } from '../aiModelRouteResolver';
...
import { validateRuntimeCompletion, type XhsMediaCompletionState } from './runtimeCompletion';
...
import { validateXhsAutoCaptureCompletion } from '../xhsAutoCaptureCompletion';
...
const PREP_ROLE_SEQUENCE_BY_INTENT: Partial<Record<NonNullable<AgentTaskSnapshot['intent']>, RoleId[]>> = {
  ...
  video_creation: ['planner', 'researcher', 'video-director'],
  audio_creation: ['planner', 'researcher', 'audio-director'],
...
const registerCoordinatorMediaArtifact = (
  taskId: string,
  toolName: string,
  result: ToolResult,
  command?: string,
  ...
  const artifactType = kind === 'generated-videos'
    ...
    : kind === 'generated-images'
      ...
      : kind === 'generated-audios'
        ? 'audio'
      : '';
  if (!artifactType || !Array.isArray(data?.assets)) return;

  const xhsBinding = data.xhsBinding && typeof data.xhsBinding === 'object' && !Array.isArray(data.xhsBinding)
  ...
  data.assets.forEach((asset, index) => {
    ...
    getTaskGraphRuntime().addArtifact(taskId, {
      type: artifactType,
      label: String(record.id || `${artifactType}-${index + 1}`),
...
const validateCoordinatorExecution = async (
  taskId: string,
  sessionId: string,
  response?: string,
) => {
  const task = getTaskGraphRuntime().getTask(taskId);
  if (!task?.route) return { complete: true };
  const metadata = task.metadata && typeof task.metadata === 'object'
    ? task.metadata as Record<string, unknown>
    : {};
  const activeNotePath = String(metadata.activeXhsNotePath || '').trim();
  const requiresImage = task.route.requiredCapabilities.includes('image-generation');
  const requiresVideo = task.route.requiredCapabilities.includes('video-generation');
  let xhsMediaState: XhsMediaCompletionState | null = null;

  if (activeNotePath && (requiresImage || requiresVideo)) {
    try {
      const snapshot = await getXhsNoteProject(activeNotePath);
      xhsMediaState = {
        notePath: snapshot.relativePath,
        noteType: requiresVideo ? 'video' : 'image',
      ...
      return {
        complete: false,
        feedback: `无法读取小红书笔记以验收媒体生成结果：${error instanceof Error ? error.message : String(error)}`,
      };
    }
  }
  ...
  const captureCompletion = validateXhsAutoCaptureCompletion({
    metadata,
    toolResults: getToolResultStore().list(sessionId).map((item) => ({
  ...
  return validateRuntimeCompletion({
    route: task.route,
    metadata,
...
const buildCoordinatorSourceContext = (sessionId: string): CoordinatorSourceContext => {
  ...
  const latestUser = [...messages].reverse().find((item) => (
    item.role === 'user'
    && !/^coord_(?:user|repair_user)_/.test(item.id)
  ...
  if (uploadedAttachments.length === 0) {
    const latestReusableAttachments = [...messages]
      .reverse()
      .filter((item) => item.role === 'user' && item.id !== latestUser.id)
      .flatMap((item) => uploadedAttachmentRecordsFromMessage(item))
      .filter((item) => {
        const kind = String(item.kind || '').trim().toLowerCase();
        return kind === 'image' || kind === 'video';
      })
      .slice(0, 9);
    uploadedAttachments.push(...latestReusableAttachments);
  }
...
export class LongTaskCoordinator {
  ...
  private readonly backgroundTaskIds = new Map<string, string>();

  async cancel(taskId: string): Promise<boolean> {
    const normalizedTaskId = String(taskId || '').trim();
    if (!normalizedTaskId) return false;
    const controller = this.abortControllers.get(normalizedTaskId);
    controller?.abort();
    getTaskGraphRuntime().cancelTask(normalizedTaskId);
    const backgroundTaskId = this.backgroundTaskIds.get(normalizedTaskId);
    const backgroundTask = backgroundTaskId
      ? await getBackgroundTaskRegistry().getTask(backgroundTaskId)
      : null;
    if (backgroundTask) {
      await getBackgroundTaskRegistry().cancelTask(backgroundTask.id);
    }
    return Boolean(controller || backgroundTask);
  }
  ...
  private async runMaybeRun(taskId: string, options: CoordinatorRunOptions = {}): Promise<AgentTaskSnapshot | null> {
    ...
    if (!task) return null;
    if (!options.force && !shouldUseCoordinator(task)) {
      return runtime.resumeTask(taskId);
    }

    const llm = options.llm?.apiKey && options.llm?.baseURL && options.llm?.model
      ? options.llm
      : this.resolveLlmConfig(task.runtimeMode);
    const backgroundRegistry = getBackgroundTaskRegistry();
    ...
    this.backgroundTaskIds.set(taskId, backgroundTask.id);
    try {
      await getHeadlessTaskSupervisor().run({
        ...
        execute: async (signal) => {
          ...
          if (signal.aborted || controller.signal.aborted) combinedController.abort();
          try {
            await this.executeCoordinatedTask(taskId, backgroundTask.id, llm, combinedController.signal, options);
            return true;
          } finally {
  ...
  private resolveLlmConfig(runtimeMode: RuntimeMode): LlmConfig {
    ...
    const baseURL = normalizeApiBaseUrl(
      String(routed?.baseURL || settings.api_endpoint || '').trim(),
      'https://api.openai.com/v1',
    );
    const model = String(
      routed?.modelName
      || resolveScopedModelName(settings, MODEL_SCOPE_BY_MODE[runtimeMode], String(settings.model_name || 'gpt-4o-mini')),
    ).trim();
    if (!apiKey) {
  ...
  private async executeCoordinatedTask(taskId: string, backgroundTaskId: string, llm: LlmConfig, signal: AbortSignal, options: CoordinatorRunOptions = {}): Promise<void> {
    ...
    const prepRoles = options.skipPreparationRoles
      ? []
      : normalizeRoleSequence(task, coordinatorContext);
    const prepRoleBatches = options.skipPreparationRoles
      ? []
      : normalizeRoleBatches(task, coordinatorContext);
    ...
    const activeRoleId = [...outputs].reverse().find((item) => (
      item.roleId === 'copywriter'
      || item.roleId === 'image-director'
      || item.roleId === 'video-director'
      || item.roleId === 'audio-director'
      || item.roleId === 'ops-coordinator'
    ))?.roleId
    ...
    getSessionRuntimeStore().appendTranscript({
      sessionId,
      recordType: 'coordinator.internal_prompt',
      role: 'system',
      content: executionPrompt,
      payload: { taskId: task.id, visibility: 'internal', phase: 'execute' },
    });
    ...
    const queryRuntime = new QueryRuntime(
      ...
      toolExecutor,
      {
        onEvent: (event) => {
          ...
          if (event.type !== 'response_end' && event.type !== 'done') {
        ...
        onToolResult: (toolName, result, command) => {
          registerCoordinatorMediaArtifact(task.id, toolName, result, command);
        },
        validateCompletion: async ({ response }) => validateCoordinatorExecution(task.id, sessionId, response),
      },
      {
        sessionId,
        apiKey: llm.apiKey,
        ...
        messages: [],
        signal,
        maxTurns: task.route.requiredCapabilities.some((capability) => (
          capability === 'image-generation' || capability === 'video-generation'
        )) ? 100 : task.route.requiresLongRunningTask ? 18 : 12,
        maxTimeMinutes: task.route.requiredCapabilities.some((capability) => (
          capability === 'image-generation' || capability === 'video-generation'
        )) ? 45 : task.route.requiresLongRunningTask ? 8 : 5,
        temperature: 0.3,
    ...
    getSessionRuntimeStore().appendTranscript({
      sessionId,
      recordType: 'coordinator.internal_result',
      role: 'system',
    ...
    if (task.intent === 'manuscript_creation' && !hasSuccessfulManuscriptWrite(sessionId)) {
      ...
      getSessionRuntimeStore().appendTranscript({
        ...
        payload: { taskId: task.id, visibility: 'internal', phase: 'repair' },
      });
      ...
      const repairRuntime = new QueryRuntime(
        ...
        toolExecutor,
        {
          onEvent: (event) => {
            options.onRuntimeEvent?.(event);
            if (event.type !== 'response_chunk' && event.type !== 'response_end' && event.type !== 'done') {
              forwardRuntimeEventToChat(event, options.emitChatEvent);
            }
            if (event.type === 'thinking') {
              void backgroundRegistry.appendTurn(backgroundTaskId, { source: 'thought', text: event.content });
