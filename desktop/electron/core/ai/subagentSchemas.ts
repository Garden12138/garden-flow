...
export const ROLE_OUTPUT_RULES: Record<RoleId, SubagentOutputRule> = {
  ...
  'video-director': { requireArtifact: true, requireHandoff: true },
  'audio-director': { requireArtifact: true, requireHandoff: true },
...
export const ROLE_OUTPUT_SCHEMA_HINT: Record<RoleId, string> = {
  ...
  'video-director': '必须输出 JSON：{ "summary": string, "artifact": string, "handoff": string, "risks"?: string[] }',
  'audio-director': '必须输出 JSON：{ "summary": string, "artifact": string, "handoff": string, "risks"?: string[] }',
