...
export interface GardenFlowProject {
  ...
  workItemId?: string;
  goal: string;
  platform: GardenFlowContentPlatform;
  taskType: GardenFlowAuthoringTaskType;
  targetAudience?: string;
  tone?: string;
  successCriteria?: string;
  sourcePlatform?: GardenFlowContentPlatform;
  sourceNoteId?: string;
  sourceMode?: GardenFlowSourceMode;
  sourceTitle?: string;
  sourceManuscriptPath?: string;
  tags: string[];
  status: GardenFlowProjectStatus;
  createdAt: string;
  updatedAt: string;
}

export interface GardenFlowImagePrompt {
  purpose?: string;
  prompt: string;
  style?: string;
  ratio?: string;
...
export interface GardenFlowRetrospectiveMetrics {
  views?: number;
  likes?: number;
  comments?: number;
  collects?: number;
...
const GARDENFLOW_DIR_NAME = 'gardenflow';
const PROJECTS_DIR_NAME = 'projects';

const DEFAULT_PLATFORM: GardenFlowContentPlatform = 'xiaohongshu';
const DEFAULT_TASK_TYPE: GardenFlowAuthoringTaskType = 'direct_write';

function nowIso(): string {
  return new Date().toISOString();
}

function normalizePlatform(value: unknown): GardenFlowContentPlatform {
  return String(value || '').trim() === 'wechat_official_account'
    ? 'wechat_official_account'
    : DEFAULT_PLATFORM;
}

function normalizeTaskType(value: unknown): GardenFlowAuthoringTaskType {
  return String(value || '').trim() === 'expand_from_xhs'
    ? 'expand_from_xhs'
    : DEFAULT_TASK_TYPE;
}

function platformLabel(platform: GardenFlowContentPlatform): string {
  return platform === 'wechat_official_account' ? '公众号' : '小红书';
}

function taskTypeLabel(taskType: GardenFlowAuthoringTaskType): string {
  return taskType === 'expand_from_xhs' ? '小红书扩写公众号' : '直接写稿';
}

function normalizeProject(raw: Partial<GardenFlowProject> & { id: string; goal: string }): GardenFlowProject {
  return {
    id: raw.id,
...
function resolveGardenFlowRoot(): string {
  return path.join(getWorkspacePaths().base, GARDENFLOW_DIR_NAME);
}

function resolveProjectsDir(): string {
  return path.join(resolveGardenFlowRoot(), PROJECTS_DIR_NAME);
}
...
export async function createGardenFlowProject(input: {
  goal: string;
  platform?: GardenFlowContentPlatform;
  taskType?: GardenFlowAuthoringTaskType;
  targetAudience?: string;
  ...
  tags?: string[];
  workItemId?: string;
}): Promise<{ project: GardenFlowProject; projectDir: string }> {
  ...
  const linkedWorkItem = input.workItemId
    ...
    : await workItemStore.createWorkItem({
      ...
      summary: '已升级为 GardenFlow 项目，进入持续创作/复盘链路。',
    });
  ...
  if (linkedWorkItem?.id) {
    ...
    await workItemStore.updateWorkItem(linkedWorkItem.id, {
      type: 'gardenflow-project',
      status: 'active',
      summary: `已创建 GardenFlow 项目 ${project.id}`,
    });
  }
...
export async function getGardenFlowProject(projectId: string): Promise<{ project: GardenFlowProject; projectDir: string }> {
  ...
  return {
    project: normalizeProject(JSON.parse(raw) as GardenFlowProject),
    projectDir: resolveProjectDir(normalizedProjectId),
  };
}

async function saveProject(project: GardenFlowProject): Promise<void> {
  await writeJson(resolveProjectJsonPath(project.id), project);
}

async function updateProjectStatus(projectId: string, status: GardenFlowProjectStatus): Promise<GardenFlowProject> {
  const { project } = await getGardenFlowProject(projectId);
  const next = {
    ...project,
    status,
    updatedAt: nowIso(),
...
export async function listGardenFlowProjects(limit = 20): Promise<Array<GardenFlowProject & { projectDir: string }>> {
  await ensureDirStructure();
  const entries = await fs.readdir(resolveProjectsDir(), { withFileTypes: true });
  const projects: Array<GardenFlowProject & { projectDir: string }> = [];
...
export async function saveGardenFlowCopyPack(input: {
  projectId: string;
  titleOptions: string[];
  finalTitle?: string;
  platform?: GardenFlowContentPlatform;
  ...
  sourceManuscriptPath?: string;
  publishPlan?: string;
}): Promise<{ project: GardenFlowProject; filePath: string; manuscriptPath: string }> {
  const normalizedProjectId = normalizeProjectId(input.projectId);
  const { projectDir, project } = await getGardenFlowProject(normalizedProjectId);
  ...
  const manuscriptPath = path.join('gardenflow', `${project.id}.md`).replace(/\\/g, '/');
  ...
  const manuscriptMetadata: Record<string, unknown> = {
    ...
    platform,
    taskType,
    gardenflowProjectId: project.id,
    gardenflowUpdatedAt: payload.updatedAt,
  ...
  const manuscriptBody = platform === 'wechat_official_account'
    ? [
        ...
        payload.publishPlan || '(无)',
        '',
        '> 该稿件由 GardenFlow 自动生成，可在稿件工作台继续编辑或执行公众号排版复制。',
      ].join('\n')
    : [
        '# 标题候选',
        ...(titleOptions.length > 0 ? titleOptions.map((title, index) => `${index + 1}. ${title}`) : ['(无)']),
        ...
        payload.publishPlan || '(无)',
        '',
        '> 该稿件由 GardenFlow 自动生成，可在稿件工作台继续编辑。',
      ].join('\n');
...
export async function saveGardenFlowImagePack(input: {
  projectId: string;
  images: GardenFlowImagePrompt[];
  coverPrompt?: string;
  notes?: string;
}): Promise<{ project: GardenFlowProject; filePath: string; plannedAssetCount: number }> {
  const normalizedProjectId = normalizeProjectId(input.projectId);
  const { projectDir, project } = await getGardenFlowProject(normalizedProjectId);
...
export async function saveGardenFlowRetrospective(input: {
  projectId: string;
  metrics?: GardenFlowRetrospectiveMetrics;
  whatWorked?: string;
  whatFailed?: string;
  nextHypotheses?: string[];
  nextActions?: string[];
}): Promise<{ project: GardenFlowProject; filePath: string }> {
  const normalizedProjectId = normalizeProjectId(input.projectId);
  const { projectDir } = await getGardenFlowProject(normalizedProjectId);
...
export async function getGardenFlowProjectContextPrompt(limit = 8): Promise<string> {
  const projects = await listGardenFlowProjects(limit);
