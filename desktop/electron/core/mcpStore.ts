...
/**
 * computer-use 原生电脑操作运行时。
 * 内置「小红书自动采集」任务依赖它驱动真实 Chrome，用户需自行在本机安装并授予系统权限。
 */
export const COMPUTER_USE_MCP_PRESET: McpServerPreset = {
  id: 'computer-use',
  name: 'computer-use',
  description: '原生桌面操作运行时（截图 / 键鼠 / 窗口 / Chrome 配方），内置小红书自动采集任务依赖该服务。',
  documentationUrl: 'https://github.com/Garden12138/computer-use',
  server: {
    name: 'computer-use',
    enabled: true,
    transport: 'stdio',
    command: 'python3',
    args: ['-u', '-m', 'computer_use', '--pacing', 'conservative', 'mcp'],
    env: {},
  },
};

export const MCP_SERVER_PRESETS: McpServerPreset[] = [COMPUTER_USE_MCP_PRESET];
...
/**
 * 幂等地写入预置 MCP 条目：已存在则只确保启用，不覆盖用户改过的 command/args。
 */
export const ensurePresetMcpServer = (presetId: string): {
  added: boolean;
  ...
  const existingIndex = servers.findIndex((item) => item.id === preset.id);
  if (existingIndex >= 0) {
    const existing = servers[existingIndex];
    if (existing.enabled) {
      return { added: false, server: existing, servers };
    }
    const next = servers.map((item, index) => (index === existingIndex ? { ...item, enabled: true } : item));
    const saved = saveMcpServers(next);
    return {
      added: false,
      server: saved.find((item) => item.id === preset.id) || { ...existing, enabled: true },
      servers: saved,
    };
  }

  const created: McpServerConfig = { id: preset.id, ...preset.server };
  const saved = saveMcpServers([...servers, created]);
  return {
    added: true,
    server: saved.find((item) => item.id === preset.id) || created,
    servers: saved,
  };
};
