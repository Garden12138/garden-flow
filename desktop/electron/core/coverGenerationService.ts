...
function resolveDefaultImageEndpoint(template: ImageProviderTemplate): string {
    switch (template) {
        ...
        case 'jimeng-images':
            return '';
        case 'openai-chat-completions-image':
        case 'openai-images':
        default:
...
function buildCoverPrompt(input: {
    ...
    const switchPromptBlocks = [
        ...
        switches.learnColorMood
            ? '- [学习颜色氛围=开] 学习模板图的标题配色与整体氛围色（主色/辅色/强调色/明暗对比），在底图中复用同款情绪与对比关系。'
...
export async function generateCoverAssets(input: GenerateCoverInput): Promise<GenerateCoverResult> {
    ...
    const openAiFallbackEndpoint = providerTemplate === 'openai-images' || providerTemplate === 'openai-chat-completions-image'
    ...
    const model = String(input.model || settings.image_model || resolveDefaultImageModel(providerTemplate)).trim();
