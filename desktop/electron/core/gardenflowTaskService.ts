...
import { getGardenFlowBackgroundRunner } from './gardenflowBackgroundRunner';
import { normalizeAutomationTaskSource } from '../../shared/automationTask';
...
function normalizeIntent(payload: Record<string, unknown>): TaskIntent {
  ...
  if (!text(intent.actionType)) intent.actionType = text(intent.type) || (text(intent.objective) ? 'long_cycle' : 'gardenflow_prompt');
  ...
  if (!text(intent.ownerScope)) intent.ownerScope = 'manual:gardenflow';
  return intent;
}
...
function taskListItem(task: Record<string, unknown>, kind: 'scheduled' | 'long_cycle'): Record<string, unknown> {
  ...
  const source = normalizeAutomationTaskSource(task.source) || 'manual';
  return {
    ...
    actionType: isLongCycle ? 'long_cycle' : 'gardenflow_prompt',
...
async function findTask(taskId: string): Promise<{ kind: 'scheduled' | 'long_cycle'; task: Record<string, unknown> } | null> {
  const runner = getGardenFlowBackgroundRunner();
...
export async function listGardenFlowTasks(payload: Record<string, unknown> = {}): Promise<Record<string, unknown>> {
  const runner = getGardenFlowBackgroundRunner();
...
export async function previewGardenFlowTask(payload: Record<string, unknown>): Promise<Record<string, unknown>> {
  ...
  return {
    ...
    policySignature: 'gardenflow-local-v1',
    normalized: intent,
  };
}

export async function createGardenFlowTask(payload: Record<string, unknown>): Promise<Record<string, unknown>> {
  ...
  const draftId = id('gardenflow-draft');
...
export async function confirmGardenFlowTask(payload: Record<string, unknown>): Promise<Record<string, unknown>> {
  ...
  if (text(intent.kind) === 'long_cycle') {
    const task = await runner.addLongCycleTask({
      ...
      source: normalizeAutomationTaskSource(intent.source),
    });
...
export async function updateGardenFlowTask(payload: Record<string, unknown>): Promise<Record<string, unknown>> {
...
export async function cancelGardenFlowTask(payload: Record<string, unknown>): Promise<Record<string, unknown>> {
...
export async function gardenFlowTaskStats(): Promise<Record<string, unknown>> {
  const result = await listGardenFlowTasks({ includeDrafts: true });
