 ...
 */
export interface RedClawBuiltinTaskState {
  enabled: boolean;
...
export interface RedClawBuiltinTaskProjection extends RedClawBuiltinTaskState {
  id: string;
  name: string;
  description: string;
  source: 'builtin';
...
interface RedClawBackgroundConfig {
  ...
  builtinTasks: Record<string, RedClawBuiltinTaskState>;
}
...
export interface RedClawBackgroundRunnerStatus {
  ...
  scheduledTasks: Record<string, RedClawScheduledTask>;
  longCycleTasks: Record<string, RedClawLongCycleTask>;
  builtinTasks: Record<string, RedClawBuiltinTaskProjection>;
}
...
function normalizeBuiltinTaskState(
  ...
  raw: Partial<RedClawBuiltinTaskState> | undefined,
): RedClawBuiltinTaskState {
...
function normalizeConfig(raw: Partial<RedClawBackgroundConfig> | null | undefined): RedClawBackgroundConfig {
  ...
  const builtinTasksRaw = (raw?.builtinTasks || {}) as Record<string, Partial<RedClawBuiltinTaskState>>;
  const normalizedBuiltinTasks: Record<string, RedClawBuiltinTaskState> = {};
...
function isBuiltinTaskDue(state: RedClawBuiltinTaskState, nowMs: number): boolean {
...
export class RedClawBackgroundRunner extends EventEmitter {
  ...
  private ensureBuiltinTaskState(definition: BuiltinAutomationDefinition): RedClawBuiltinTaskState {
    if (!this.config.builtinTasks) {
      this.config.builtinTasks = {};
    }
    const existing = this.config.builtinTasks[definition.id];
    if (existing) return existing;
    const created: RedClawBuiltinTaskState = {
      enabled: false,
  ...
  async listBuiltinTasks(): Promise<RedClawBuiltinTaskProjection[]> {
    await this.ensureLoaded();
    const nowMs = Date.now();
  ...
  async setBuiltinTaskEnabled(taskId: string, enabled: boolean): Promise<{
    task: RedClawBuiltinTaskProjection;
  ...
  async setBuiltinTaskSettings(taskId: string, input: {
    settings?: Record<string, unknown>;
    scheduleTime?: string;
  }): Promise<RedClawBuiltinTaskProjection> {
    await this.ensureLoaded();
    const definition = getBuiltinAutomationDefinition(taskId);
    if (!definition) throw new Error('Builtin automation task not found');
    const state = this.ensureBuiltinTaskState(definition);
  ...
  async runBuiltinTaskNow(taskId: string): Promise<RedClawBuiltinTaskProjection> {
    await this.ensureLoaded();
    const definition = getBuiltinAutomationDefinition(taskId);
    if (!definition) throw new Error('Builtin automation task not found');
    if (this.currentAutomationTaskId) {
      throw new Error(
        this.currentAutomationTaskId === definition.id
          ? '该采集任务正在执行中。当前可能在操作一个过小或被挡住的 Chrome 窗口，看起来像没打开浏览器。请等待结束，或完全退出 Bojin 后再重新运行。'
  ...
  private async executeBuiltinTask(
    ...
    const runtimeTaskId = this.createRuntimeTask({
      ...
      intent: 'automation',
      roleId: 'ops-coordinator',
      ownerSessionId: `redclaw-builtin:${definition.id}`,
    ...
    try {
      ...
      const result = await this.runAgentPrompt({
        contextId: `redclaw-builtin-${definition.id}`,
        title: `RedClaw Builtin ${definition.name}`,
        contextContent: [
