...
import { resolveAssetSourceToPath } from './localAssetManager.ts';
...
function resolveOpenAiChatCompletionsEndpoint(endpoint: string): string {
    const base = normalizeEndpoint(endpoint);
    if (base.includes('/chat/completions')) return base;
    return normalizeEndpoint(base, '/chat/completions');
}

function resolveOpenAiVersionedBaseFallback(endpoint: string): string | null {
    const base = normalizeEndpoint(endpoint);
    if (!base) return null;
    try {
...
function inferImageMimeType(buffer: Buffer, declaredMimeType?: string | null): string {
    const normalizedDeclared = String(declaredMimeType || '').split(';')[0].trim().toLowerCase();
...
async function fetchImageByUrl(url: string): Promise<GeneratedImageOutput> {
    ...
    const mimeType = inferImageMimeType(imageBuffer, response.headers.get('content-type'));
...
async function normalizeGeneratedImages(payload: any): Promise<GeneratedImageOutput[]> {
    ...
    const readDataArray = async (items: any[]) => {
        for (const item of items) {
            ...
            // new-api / 百炼常返回 b64_json: "" 并只在 url 里给图；空字符串不能当成已出图。
            const base64Payload = [item.b64_json, item.base64, item.image_base64, item.b64_image]
                .map((value) => String(value || '').trim())
                .find(Boolean);
            if (base64Payload) {
                pushBase64(outputs, base64Payload, item.mime_type || item.mimeType);
                continue;
            }
    ...
    const appendChatImageSource = async (raw: unknown, mimeType?: unknown) => {
        const value = String(raw || '').trim();
        if (!value) return;
        const decoded = decodeDataUrl(value);
        if (decoded) {
            pushBase64(outputs, decoded.base64, decoded.mimeType);
            return;
        }
        if (/^https?:\/\//i.test(value)) {
            outputs.push(await fetchImageByUrl(value));
            return;
        }
        pushBase64(outputs, value, mimeType);
    };

    const appendChatTextImages = async (raw: unknown) => {
        const text = String(raw || '').trim();
        if (!text) return;
        const sources: string[] = [];
        for (const match of text.matchAll(/data:image\/[a-z0-9.+-]+;base64,[a-z0-9+/=\r\n]+/gi)) {
            if (match[0]) sources.push(match[0]);
        }
        ...
        for (const source of Array.from(new Set(sources))) {
            await appendChatImageSource(source);
        }
    };
    ...
    for (const choice of chatChoices) {
        ...
        if (typeof content === 'string') {
            await appendChatTextImages(content);
            ...
            for (const item of content) {
                ...
                const imageUrl = typeof item.image_url === 'string'
                    ...
                    : item.image_url?.url;
                if (imageUrl) {
                    await appendChatImageSource(imageUrl);
                    continue;
                }
                const source = item.source;
                if (source?.data) {
                    await appendChatImageSource(source.data, source.media_type || source.mime_type);
                    continue;
                }
                if (item.b64_json || item.base64 || item.image) {
                    await appendChatImageSource(
                        item.b64_json || item.base64 || item.image,
        ...
        for (const image of messageImages) {
            if (typeof image === 'string') {
                await appendChatImageSource(image);
                continue;
            }
            const imageUrl = typeof image?.image_url === 'string'
                ? image.image_url
                : image?.image_url?.url || image?.url;
            if (imageUrl) {
                await appendChatImageSource(imageUrl);
                continue;
            }
            if (image?.b64_json || image?.base64) {
                await appendChatImageSource(
                    ...
                    image.mime_type || image.mimeType
                );
            }
        }
    }
...
function resolveDashscopeWanEndpoints(
    ...
    const isQwenImage = isQwenImageModel(normalizedModel);
...
function buildDashscopeWanPayloadVariants(
    ...
    const qwenImageModel = isQwenImageModel(request.model);
    const refs = pickReferenceImages(request, qwenImageModel ? 3 : 4);
    ...
    if (qwenImageModel && normalizedEndpoint.includes('/multimodal-generation/')) {
...
function buildOpenAiChatImagePrompt(request: ImageGenerationRequest): string {
    const requirements: string[] = [];
    if (request.aspectRatio && request.aspectRatio !== 'auto') {
        requirements.push(`aspect ratio ${request.aspectRatio}`);
    }
    const normalizedSize = normalizeImageSize(request.size);
    if (normalizedSize) {
        requirements.push(`preferred size ${normalizedSize}`);
    }
    const quality = String(request.quality || '').trim();
    if (quality && quality !== 'auto') {
        requirements.push(`quality ${quality}`);
    }
    if (requirements.length === 0) return request.prompt;
    return `${request.prompt}\n\nImage output requirements: ${requirements.join(', ')}.`;
}

const openAiChatCompletionsImageAdapter: ImageProviderAdapter = {
    template: 'openai-chat-completions-image',
    supportsMultiCount: false,
    async generate(request) {
        const refs = await normalizeReferenceImagesForTransport(
            pickReferenceImages(
                request,
                IMAGE_PROVIDER_CAPABILITIES['openai-chat-completions-image'].maxReferenceImages
            )
        );
        const prompt = buildOpenAiChatImagePrompt(request);
        const content: string | Array<Record<string, unknown>> = refs.length === 0
            ? prompt
            : [
        ...
        const endpoint = resolveOpenAiChatCompletionsEndpoint(request.endpoint);
        const response = await fetch(endpoint, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            ...
            signal: AbortSignal.timeout(resolveImageRequestTimeoutMs()),
        });
        const payload = await parseJsonSafe(response);
        if (!response.ok) {
            const message = String(
            ...
            logImageGenApiFailure(response, message);
            throw new Error(`Image generation failed (${response.status}): ${message}`);
        }
        const outputs = await normalizeGeneratedImages(payload);
        if (outputs.length === 0) {
            throw new Error('Chat Completions image generation returned no valid image payload.');
        }
        return outputs;
    },
};
...
const openAiAdapter: ImageProviderAdapter = {
    ...
    async generate(request) {
        ...
        // Accept both a provider base URL and a complete Images API URL. The SDK
        // appends /images/generations itself, so retaining that terminal path here
        // would produce /images/generations/images/generations.
        let activeBaseEndpoint = normalizeApiBaseUrl(request.endpoint);
        let endpoint = useEditApi
            ? resolveOpenAiImageEditsEndpoint(activeBaseEndpoint)
            : resolveOpenAiImagesEndpoint(activeBaseEndpoint);
        let preferredResponseFormat = resolvePreferredOpenAiResponseFormat(endpoint);
        const removedParams = new Set<string>();
        const maxRetryRounds = 4;
        let forcedSize = '';
        let attemptedEditFallback = false;
        let attemptedVersionedBaseFallback = false;
        let response: Response | null = null;
        let lastErrorText = '';
        const quality = mapQualityToOpenAi(request.quality);
        let client = createOpenAiSdkClient(activeBaseEndpoint, request.apiKey);
            ...
            try {
                for (let attempt = 0; attempt <= maxUpstreamTimeoutRetries; attempt += 1) {
                    try {
                        ...
                        const outputs = await normalizeGeneratedImages(sdkResult);
                        if (outputs.length > 0) {
                            return outputs;
                        }
                        const versionedBaseFallback = attemptedVersionedBaseFallback
                            ? null
                            : resolveOpenAiVersionedBaseFallback(activeBaseEndpoint);
                        if (versionedBaseFallback) {
                            attemptedVersionedBaseFallback = true;
                            activeBaseEndpoint = versionedBaseFallback;
                            endpoint = useEditApi
                                ? resolveOpenAiImageEditsEndpoint(activeBaseEndpoint)
                                : resolveOpenAiImagesEndpoint(activeBaseEndpoint);
                            client = createOpenAiSdkClient(activeBaseEndpoint, request.apiKey);
                            logImageGenDebug('openai-images', 'retry_with_versioned_base_url', {
                                endpoint,
                                model: request.model,
                                round: i + 1,
                                mode: useEditApi ? 'edit' : 'generate',
                                reason: 'successful response contained no image payload',
                            });
                            break;
                        }
                        return outputs;
                ...
                const versionedBaseFallback = attemptedVersionedBaseFallback || (status !== 404 && status !== 405)
                    ? null
                    : resolveOpenAiVersionedBaseFallback(activeBaseEndpoint);
                if (versionedBaseFallback) {
                    attemptedVersionedBaseFallback = true;
                ...
                if (!useEditApi && invalidValueParam === 'response_format' && preferredResponseFormat === 'url') {
                    preferredResponseFormat = 'b64_json';
                    logImageGenDebug('openai-images', 'retry_with_base64_response_format', {
                        endpoint,
                        model: request.model,
                        round: i + 1,
                        via: 'openai-sdk',
        ...
        for (const endpoint of endpoints) {
            ...
            for (let i = 0; i < payloadVariants.length; i += 1) {
                try {
                    ...
                    const useSse = isMultimodalEndpoint && !isQwenImageModel(request.model);
...
const ADAPTERS: Record<ImageProviderTemplate, ImageProviderAdapter> = {
    ...
    'openai-chat-completions-image': openAiChatCompletionsImageAdapter,
