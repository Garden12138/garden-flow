...
export interface CommandPermissionAnalysis {
  ...
  reason: string;
  /**
   * 标记该确认必须由人工点击完成，运行时不得静默放行。
   * 未标记的 confirm 保持既有行为（由宿主自行决定是否静默通过）。
   */
...
const readTokenFlag = (tokens: string[], names: string[]): string => {
...
export function analyzeAppCliCommand(command: string, options?: {
  ...
  if (namespace === 'mcp' && action === 'call') {
    const mcpServerId = readTokenFlag(tokens, ['id', 'server-id']).toLowerCase();
    if (options?.runtimeMode === 'background-maintenance' && mcpServerId === 'computer-use') {
      return {
        className: 'trusted-write',
        summary,
        reason: '后台内置采集允许调用已配置的 computer-use MCP。',
      };
    }
    return {
  ...
  // 无人值守链路沿用受信任写入清单，只有前台会话才要求人工确认。
  if (options?.interactive !== false && mapHasAction(USER_ACKNOWLEDGED_APP_ACTIONS, namespace, action)) {
    return {
      className: 'confirm',
      summary,
      reason: `${summary} 会创建或删除持久后台任务，需要你确认后才执行。`,
