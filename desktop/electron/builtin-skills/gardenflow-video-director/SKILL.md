...
# GardenFlow Video Director

Use this skill before calling `video_generate` for GardenFlow video work. Use `image_generate` for standalone storyboard/keyframe/reference images and pass their returned `absolutePath` values into `video_generate.referenceImages`.
...
10. Only after confirmation, call `video_generate`.
...
- Separate the human-facing storyboard from the provider prompt. For structured notes, write a dynamic `generationPrompt` for every storyboard item based on that item's actual subject, environment, materials, lighting, action, camera movement, and style.
- Keep `generationPrompt` visual-only, but preserve each shot's actual narration or dialogue in `voiceover`. For dialogue, include the speaker, delivery, and exact words instead of placeholders such as “continue narration” or “call the names quickly.” Structured note generation combines that field into a separate audio-only section for models that generate sound; it must never be rendered as visible text. Treat subtitles, on-screen copy, prices, step labels, icons, logos, UI, and calls to action as post-production data unless a structured `renderText` option was explicitly enabled.
- Keep each generated clip to one coherent scene and usually no more than 1–2 continuous visual beats. Split when the spatial layout, camera setup, or main action changes instead of dumping the entire narration into one prompt.
- Preserve a stable visual anchor across adjacent clips: subject identity, room geometry, object placement, important materials, palette, and lighting logic. Describe only the intentional change for the current clip.
...
- Always use `video_generate` for the actual video generation call.
