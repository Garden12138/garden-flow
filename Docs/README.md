...
本目录面向需要本地运行、使用或继续开发本仓库的人。基础文档继承自 Bojin 开源快照，GardenFlow 的迁移基线与当前验收范围见迁移记录。

## 文档导航

| 文档 | 适合读者 | 内容 |
| --- | --- | --- |
| [迁移记录](./MIGRATION.md) | 开发者、维护者 | RedClaw 迁移基线、保留范围、初始化调整和验收结果 |
...
当前仓库名为 **GardenFlow**。迁移暂时保留源项目的 **Bojin / RedClaw** 产品名称，以及 `beav-*`、`redbox-*` 和 `redconvert-*` 等兼容名称。它们通常出现在应用 ID、数据库文件、协议名、IPC channel、工作目录和历史迁移代码中，不代表存在多套产品。
