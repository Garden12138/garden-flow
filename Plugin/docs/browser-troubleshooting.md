...
```text
agent typed browser action -> Desktop Bridge -> GardenFlow Native Host -> Chrome native messaging -> GardenFlow extension -> page/content script
```
...
- 正式桌面端启动时会在 macOS / Windows / Linux 对账 Chrome、Edge、Brave 的 manifest；manifest 直接指向当前 GardenFlow 可执行文件，不依赖 GUI 浏览器的 Node/PATH。
...
- `extension_not_found`: GardenFlow is not loaded in a known Chrome, Chromium, Edge, or Brave profile.
- `no_native_host_manifest`: Chrome cannot launch `com.gardenflow.browser_control`.
- `host_missing` / `host_not_executable`: manifest 存在，但指向的 GardenFlow 可执行文件已移动或不可执行。
...
- `partial` with saved `gardenflow://browser-runs/<runId>/...` artifacts means browser evidence was preserved. Retry only Knowledge ingest when the failure is `knowledge_ingest_failed`; do not repeat paid download/OCR/ASR work.
...
pnpm smoke:browser-control -- --host-path <path-to-current-gardenflow> --fault-matrix --timeout-ms 30000
```

`smoke:browser-control` defaults to the built Rust `desktop/src-tauri/target/debug/gardenflow` Host and requires a running Desktop Bridge with the same App version. It has no JS Host fallback. The isolated fault matrix covers invalid control-token rejection, timeout with a late response, and MV3 service-worker restart with stable `extensionInstanceId`. Full extension reload is intentionally reserved for the installed-extension acceptance path: Chromium's `--load-extension` mode unloads the command-line extension when its developer reload control is used, so that result cannot represent a store-installed extension reload.

For development, load `Plugin/dist/extension` as an unpacked extension, start GardenFlow once so it reconciles the Native Host manifest, then run the connected diagnosis. Reload the unpacked extension after rebuilding `dist/extension`; Chrome does not reliably reload changed service-worker code just because files changed on disk.

Real Chrome acceptance:

- `pnpm smoke:browser-control` proves the isolated regression path only.
- A real Chrome acceptance run must use the user's installed Google Chrome profile, the official-ID GardenFlow extension, the GardenFlow Native Host binary, and `pnpm diagnose:browser-control -- --require-connected`.
...
- Active controlled tabs should show the in-page `GardenFlow 控制中` badge and an active favicon marker. If DOM actions work but the badge is missing, check `AGENT_CONTROL_BADGE`, `GET_AGENT_CONTROL_BADGE_STATE`, and tab lease events.
