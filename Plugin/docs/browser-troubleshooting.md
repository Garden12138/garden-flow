...
```text
agent typed browser action -> Desktop Bridge -> Bojin Native Host -> Chrome native messaging -> Bojin extension -> page/content script
```
...
- 正式桌面端启动时会在 macOS / Windows / Linux 对账 Chrome、Edge、Brave 的 manifest；manifest 直接指向当前 Bojin 可执行文件，不依赖 GUI 浏览器的 Node/PATH。
...
- `extension_not_found`: Bojin is not loaded in a known Chrome, Chromium, Edge, or Brave profile.
- `no_native_host_manifest`: Chrome cannot launch `com.redbox.browser_control`.
- `host_missing` / `host_not_executable`: manifest 存在，但指向的 Bojin 可执行文件已移动或不可执行。
...
For development, load `Plugin/dist/extension` as an unpacked extension, start Bojin once so it reconciles the Native Host manifest, then run the connected diagnosis. Reload the unpacked extension after rebuilding `dist/extension`; Chrome does not reliably reload changed service-worker code just because files changed on disk.

Real Chrome acceptance:

- `pnpm smoke:browser-control` proves the isolated regression path only.
- A real Chrome acceptance run must use the user's installed Google Chrome profile, the official-ID Bojin extension, the Bojin Native Host binary, and `pnpm diagnose:browser-control -- --require-connected`.
...
- Active controlled tabs should show the in-page `Bojin 控制中` badge and an active favicon marker. If DOM actions work but the badge is missing, check `AGENT_CONTROL_BADGE`, `GET_AGENT_CONTROL_BADGE_STATE`, and tab lease events.
