...
assert(!outputManifest.permissions.includes('cookies'), 'Content capture must not request the cookies permission');
...
const rustBrowserMcpPath = path.join(repositoryRoot, 'desktop/src-tauri/src/browser_control_mcp.rs');
const rustBrowserMcpSource = await fileExists(rustBrowserMcpPath)
  ? await readText(rustBrowserMcpPath)
  : '';
...
const browserClientUrl = pathToFileURL(browserClientPath);
browserClientUrl.searchParams.set('verify', String(Date.now()));
const browserClientModule = await import(browserClientUrl.href);
