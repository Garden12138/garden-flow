...
test('quality gates challenge pages and sparse pages before the legacy fallback', () => {
  ...
  assert.equal(assessCaptureQuality(blocked).accessErrorCode, 'BROWSER_SECURITY_CHALLENGE');

  const loginRequired = normalizeCaptureDocument({
    source: { url: 'https://example.com/login' },
    content: { title: '请先登录', text: '登录后查看完整正文。' },
  });
  assert.equal(assessCaptureQuality(loginRequired).accessErrorCode, 'BROWSER_LOGIN_REQUIRED');
