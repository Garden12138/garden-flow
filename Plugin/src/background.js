...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  void (async () => {
    try {
      ...
      if (isPluginCaptureMessageType(message?.type) && result?.success !== false) {
        void reportNativeCaptureAccessState(null).catch(() => {});
      }
      ...
      if (isPluginCaptureMessageType(message?.type)) {
        if (['BROWSER_LOGIN_REQUIRED', 'BROWSER_SECURITY_CHALLENGE', 'CONTENT_NOT_ACCESSIBLE'].includes(String(error?.code || ''))) {
          void reportNativeCaptureAccessState({
            code: error.code,
            platform: error.platform || inferCapturePlatform(sender?.tab?.url || message?.url || ''),
            origin: error.origin || safePageOrigin(sender?.tab?.url || message?.url || ''),
      ...
      sendResponse({
        ...
        code: typeof error?.code === 'string' ? error.code : 'CAPTURE_FAILED',
        phase: typeof error?.phase === 'string' ? error.phase : 'capture',
        recovery: typeof error?.recovery === 'string' ? error.recovery : '',
        platform: inferCapturePlatform(sender?.tab?.url || message?.url || ''),
        origin: safePageOrigin(sender?.tab?.url || message?.url || ''),
      });
    }
  })();
  return true;
...
function inferCapturePlatform(value) {
  const hostname = (() => {
    try {
  ...
  if (hostname.endsWith('xiaohongshu.com')) return 'xiaohongshu';
  if (hostname.endsWith('zhihu.com')) return 'zhihu';
  if (hostname.endsWith('youtube.com') || hostname === 'youtu.be') return 'youtube';
  return hostname || 'webpage';
}
...
function sanitizeXhsTaskContextForState(context) {
  ...
  const blogger = context.blogger && typeof context.blogger === 'object'
    ? {
        ...
        source: sanitizeCaptureSourceUrl(context.blogger.source),
...
function summarizeXhsTaskResult(result) {
  ...
  if (result?.noteId) {
    const identity = normalizeText(result.title || result.noteId);
    if (result.duplicate) return `重复内容已跳过${identity ? `（${identity}）` : ''}`;
    if (result.updated) return `已更新知识库${identity ? `（${identity}）` : ''}`;
    return `已写入知识库${identity ? `（${identity}）` : ''}`;
  }
  return '任务已完成';
}
...
function sanitizeCaptureCheckpoint(entry) {
  ...
  const source = sanitizeCaptureSourceUrl(entry.source || entry.sourceUrl);
...
async function readCaptureCheckpoints() {
  ...
  const raw = Array.isArray(stored?.[CAPTURE_CHECKPOINT_KEY]) ? stored[CAPTURE_CHECKPOINT_KEY] : [];
  const normalized = raw.map(sanitizeCaptureCheckpoint).filter(Boolean);
  if (JSON.stringify(normalized) !== JSON.stringify(raw)) {
    void setStorageLocal({ [CAPTURE_CHECKPOINT_KEY]: normalized }).catch(() => {});
  }
  return normalized;
}
...
async function getXhsTaskHistory() {
  ...
  const sanitizedHistory = history.map((entry) => ({
    ...entry,
    payload: sanitizeStoredCaptureMetadata(entry?.payload),
  }));
  if (JSON.stringify(sanitizedHistory) !== JSON.stringify(history)) {
    void setStorageLocal({ [XHS_TASK_HISTORY_KEY]: sanitizedHistory }).catch(() => {});
  }
  return {
    success: true,
...
async function writeXhsTaskHistory(history) {
  const nextHistory = Array.isArray(history)
    ? history.slice(0, XHS_TASK_HISTORY_LIMIT).map((entry) => ({
        ...entry,
        payload: sanitizeStoredCaptureMetadata(entry?.payload),
      }))
    : [];
...
function sanitizeStoredCaptureMetadata(value, depth = 0) {
  if (depth > 12 || value == null) return null;
  if (typeof value === 'string') {
    return /^https?:\/\//i.test(value) ? sanitizeCaptureSourceUrl(value) : value.slice(0, 20_000);
  }
  if (typeof value !== 'object') return value;
  if (Array.isArray(value)) {
    return value.slice(0, 5_000).map((item) => sanitizeStoredCaptureMetadata(item, depth + 1));
  }
  const result = {};
  for (const [key, child] of Object.entries(value)) {
    if (/^(?:cookie|cookies|password|passwd|verificationCode|captcha|authorization|accessToken|refreshToken|token|xsecToken)$/i.test(key)) {
      continue;
    }
    result[key] = sanitizeStoredCaptureMetadata(child, depth + 1);
  }
  return result;
}
...
async function initializeUpdateChecks(forceImmediateCheck) {
  void forceImmediateCheck;
  await configureUpdateAlarm({});
  await writePluginUpdateState(createDefaultUpdateState());
}
...
async function checkForPluginUpdates(options = {}) {
  void options;
  const update = await writePluginUpdateState(createDefaultUpdateState());
  return { success: true, update, disabled: true };
}
...
async function fetchKnowledgeJson(endpoint, path, init = {}) {
  ...
  const operationId = createBridgeOperationId(nativeMethod, payload);
...
function sanitizeCaptureSourceUrl(value) {
  const raw = normalizeText(value);
  if (!raw) return '';
  try {
    const parsed = new URL(raw);
    for (const key of Array.from(parsed.searchParams.keys())) {
      if (/^(?:xsec_token|access_token|refresh_token|auth|authorization|token)$/i.test(key)) {
...
function extractZhihuAnswerPayload() {
  ...
  if (!answerRoot) {
    const pageProbe = clean(document.body?.innerText || '').slice(0, 8_000);
    const accessErrorCode = /captcha|安全验证|人机验证|访问受限|verify you are human/i.test(pageProbe)
      ? 'BROWSER_SECURITY_CHALLENGE'
      : /请先登录|登录后查看|sign in to continue|login required/i.test(pageProbe)
  ...
  return {
    ...
    accessErrorCode: richText ? '' : accessErrorCode,
...
async function saveZhihuAnswerFromTab(tabId) {
  ...
  if (!payload.answerId || (!payload.answerText && !payload.answerHtml)) {
    throw captureAccessError(payload?.accessErrorCode, '知乎', payload?.source);
  }
...
async function saveZhihuArticleFromTab(tabId) {
  ...
  if (!payload.articleId || (!payload.articleText && !payload.articleHtml)) {
    throw captureAccessError(payload?.accessErrorCode, '知乎', payload?.source);
  }
...
async function saveXhsNoteFromTab(tabId) {
  ...
  if (!payload?.title && !payload?.content && !payload?.images?.length && !payload?.videoUrl) {
    throw captureAccessError(payload?.accessErrorCode, '小红书', payload?.source);
  }
  let commentsPayload = {};
...
async function fetchAccountsJson(path, init = {}) {
  ...
  const operationId = createBridgeOperationId(route.method, nativePayload);
...
async function extractXhsNotePayload() {
  ...
  function getCurrentNoteRoot() {
    ...
    const directRoots = Array.from(document.querySelectorAll(
      '#noteContainer.note-container[data-render-status], #noteContainer.note-container, #noteContainer',
    ));
    const directRoot = directRoots.find((candidate) => isNodeVisible(candidate))
      || directRoots[directRoots.length - 1]
      || null;
  ...
  function getNoteTitle(root) {
    return (
      ...
      root?.querySelector?.('.note-content #detail-title')?.innerText?.trim() ||
  ...
  const pageProbe = String(document.body?.innerText || '').slice(0, 8_000);
  const accessErrorCode = /captcha|安全验证|人机验证|访问受限|滑块验证|verify you are human/i.test(pageProbe)
    ? 'BROWSER_SECURITY_CHALLENGE'
  ...
  return {
    ...
    accessErrorCode: (!title && !content && images.length === 0 && !videoUrl) ? accessErrorCode : '',
  };
}
