...
export function assertNativeHostVersionCompatibility(handshake = {}) {
  const desktopBridge = handshake?.desktopBridge && typeof handshake.desktopBridge === 'object'
    ? handshake.desktopBridge
    : {};
  const bridgeProtocolVersion = Number(
    desktopBridge.bridgeProtocolVersion ?? handshake.bridgeProtocolVersion ?? 0,
  );
...
export async function reportNativeCaptureAccessState(problem = null) {
  if (!getNativeRegistration || !nativePort) return false;
  const registration = sanitizeNativeRegistration(await getNativeRegistration());
  if (!registration) return false;
  const source = problem && typeof problem === 'object' ? problem : null;
  const accessProblem = source && ['BROWSER_LOGIN_REQUIRED', 'BROWSER_SECURITY_CHALLENGE', 'CONTENT_NOT_ACCESSIBLE'].includes(String(source.code || ''))
    ? {
        code: String(source.code),
        platform: String(source.platform || 'webpage').slice(0, 100),
        origin: String(source.origin || '').slice(0, 2_000),
        recovery: String(source.recovery || '').slice(0, 500),
      }
    : null;
  await requestNativeHost('extension.register', {
    ...registration,
    accessProblem,
...
function nativeResponseError(error = {}) {
  ...
  if (error.data !== undefined) {
    ...
    if (typeof error.data?.recovery === 'string') nativeError.recovery = error.data.recovery;
  }
  return nativeError;
}
