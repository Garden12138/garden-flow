...
export async function ensureAgentTabGroup(sessionId, tabId, existingTabIds = []) {
  ...
  const existing = await findManagedGroupContainingTabs(existingTabIds) || await findAnyManagedGroup();
  if (existing) {
...
async function findAnyManagedGroup() {
  for (const group of groupMetadata.values()) {
    const live = await readGroup(group.chromeGroupId);
    if (!live) continue;
    const tabs = await chrome.tabs.query({ groupId: group.chromeGroupId }).catch(() => []);
    if (tabs.length > 0) return group;
  }
  const observed = await chrome.tabGroups.query({ title: DEFAULT_SESSION_GROUP_TITLE }).catch(() => []);
  for (const chromeGroup of observed) {
    const groupId = Number(chromeGroup?.id);
    if (!Number.isInteger(groupId)) continue;
    const tabs = await chrome.tabs.query({ groupId }).catch(() => []);
    if (tabs.length === 0) continue;
    let group = groupMetadata.get(groupId);
    if (!group) {
      group = {
        chromeGroupId: groupId,
        presentationColor: normalizeColor(chromeGroup.color),
        title: normalizeTitle(chromeGroup.title) || DEFAULT_SESSION_GROUP_TITLE,
      };
      groupMetadata.set(groupId, group);
    }
    return group;
