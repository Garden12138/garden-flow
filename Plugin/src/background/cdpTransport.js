...
export async function attachCdpTab(tabId) {
  ...
  try {
    ...
    emitCdpTransportTelemetry('attach.reused', { target, error: describeChromeError(error) });
    return { attached: true, alreadyAttached: true, target };
  }
  attachedCdpTabs.add(id);
...
function isAlreadyAttachedDebuggerError(error) {
  return /another debugger|already attached/i.test(describeChromeError(error));
}
