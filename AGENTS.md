...
- Use Node.js 22 and pnpm 10.28.2; keep the existing module lockfiles.
- `pnpm run setup` (at the repository root) installs root, extension, and desktop dependencies with frozen lockfiles.
- `pnpm dev` (at the root) prepares runtimes and starts Vite and Electron.
- `pnpm check` runs desktop parity/type checks and the extension's build, type checks, and regression tests.
- `pnpm test` runs the Node.js tests in `desktop/tests/`.
- `pnpm build` creates an unsigned desktop package; `pnpm build:plugin` builds the browser extension.
- `pnpm --dir desktop preview` serves a built renderer for smoke checks.
...
## Testing Guidelines
- Desktop tests use Node.js's built-in test runner; run them with `pnpm test` from the root.
- Extension tests and static checks run with `pnpm --dir Plugin check`.
